...
> taking these names for other purposes. (`with`, `:=`, `match`, `case` and
> `where` are **already** lexed — the first two carry today's transactions
> and mutation, §8, `match`/`case` carry tag dispatch, §4.10, and `where` is
> reserved ahead of the refinement syntax that will use it, §6.4 — so they

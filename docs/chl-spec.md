...
> **[Decided]**), and `import` (built-in modules — the `http` module
> surface, **[Decided]**; general modules remain future work, §9). Avoid
> taking these names for other purposes. (`with`, `:=`, `match` and `case`
> are **already** lexed — they carry today's transactions, mutation and
> tag dispatch, §8 and §4.10 — so they are not in this list.)
...
| `True`, `False` | `Bool` | |
| `None` | `Unit` | CHL's unit value. The `Option` constructor is the *variant* `.none` (§3.15) — a different thing, and already spelled distinctly. (Direction: unit becomes `()` — see §3.1.) |
...
nullary form `.tag` (equivalently `.tag()`) injects `None`, the value that
carries no information.

```python
x = .some(1)
y = .none
z = .ok((1, 2))     # one payload slot, so several values go in a tuple
```

Variants are **structural**, exactly as records are: a tag has no
declaration and no owner. `.some(1)` has the one-tag type `{some: Int}`,
and *width subtyping* — the dual of the record rule — lets it flow into
any variant type whose tag set contains it, so the same value is usable
as an `Option(Int)`. Two consequences worth stating outright:
...
- A tag's payload type is fixed by **where the tags meet**, not at the
  constructor. `.ok(1)` and `.ok("s")` are each fine alone; if both flow
  into one variant, the mismatch is reported where they join.

This is the polymorphic-variant model, and the **leading dot** plays the
role capitalization plays in languages that capitalize constructors.
§6.1 reserves `Caps` for types without exception, so a constructor is
lowercase — which makes `some(v)` indistinguishable from a call to a
function named `some`, and bare `none` indistinguishable from a variable
read. The dot is what resolves that, and it is only needed in
*constructor* position: after `case` a tag is the only thing that can
appear, so patterns take a bare tag (§4.10).
...
The pattern position takes a **bare** tag, without the constructor's dot
(§3.15): after `case` nothing else can appear, so the dot would be noise.
`case tag(binder):` binds the payload; `case tag:` matches without
binding, which is the natural spelling for a tag whose payload carries no
information. A binder is an ordinary local, scoped to its arm.
...
are simply unreachable, and are dropped. Writing the full `Option` match
over a scrutinee that inference has pinned to one tag is ordinary code,
not a mistake.

Patterns are still shallow: an arm matches one tag and binds the whole
...
> than given a form that would have to be retracted. Patterns are also
> shallow: a `case` matches one tag and binds the whole payload, with no
> nesting, no literal patterns, and no guard on an arm. Tuple
...
- **Surface refinement type syntax** — refinement types (§6) are
  ...
  statement.
- **Pattern matching** beyond a `match` arm's single tag — `match` /
  `case` tag dispatch over a variant is implemented (§4.10), but patterns
  are shallow: no nesting, no literal patterns, no per-arm guard, and no
  expression form of `match`. Tuple destructuring on assignment targets
  (§4.3) is the only other pattern-like form.

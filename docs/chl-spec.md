...
> choice.
>
> Entry iteration is **partly built**: a `map(…)`/`set(…)` source with a scalar or
> compound key works; an annotated `Map(K, V)`, a `groupby` result, a list
...
each output is a separate stream bound to its own sink.

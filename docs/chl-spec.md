...
> - `c[k]?` — **optional**, result `Option(T)` (matched with `some(v)` / `none`, as
>   in the north-star `txn_kv`). No prerequisite; always well-defined.
>
...
> This eliminates the not-defined lookup cases above (see *Partiality*, §3). The
> discharge mechanic is [collections.md, "Lookup: membership discharge"](../src/ccl/design/collections.md#lookup-membership-discharge).
...
- `{T₀, T₁, …} ⇒ U` — function type. A function takes exactly one
  ...
  types in annotations is **[Planned]**: the ASCII arrow is `=>`
  (`{x: T, y: U} => V`), with the compute/data kind (`⇒`/`⤇` in the CCL
  rendering) inferred, not written (§1.8, §6.3).

...
- **Validation and program branching [Open].** Referential transparency makes program versions *syntactically comparable with well-defined semantics*, and temporal functional mutation makes state a value over time domains — *branchable and pinnable by construction*. Together they are the substrate for branching a running application — logic and state — exercising the branch under a realistic workload, and diffing behaviour. The design of record for that model is [/src/ccl/design/program-evolution.md](/src/ccl/design/program-evolution.md).
- **Hot reload [Partial].** A running program can be replaced by a new version of its source over the control port (`--control`): `/diff` reports how the two versions differ, at a pipeline phase the caller picks, and `/reload` swaps the program. The new version inherits the running one's sources and sinks, may add to them, and retires a route it stops serving, and keeps the operator behind every `Let` binding, store and iteration input whose computation is unchanged — the structural diff of the two versions' trees says which node of the new one stands where each node of the old one did, so a fold caught partway through a collection keeps its place; a variable whose logic the edit did touch resumes from the value it held. A variable that moves to another loop, or into or out of a transaction, keeps its value and counts positions in whatever it now iterates. The one refused reload is one that cannot take over the state: a variable the new version neither declares nor loads with `@LoadFrom`, one it declares or loads at a type the held value does not fit, one that would move a value between two declarations the source tells apart only by position, and the two load-specific cases — a spelling the running program declares more than once under one chain, and a spelling nothing holds. See [program-evolution.md](../src/ccl/design/program-evolution.md#state-takeover). Running two versions at once is not implemented.
- **Observability.** The compilation pipeline preserves a legible chain from source to running state. The program inspector (`--inspect-only`, `src/inspector_model/` and `src/inspector_server/`) serves one pane per compiler stage, and the operator graph as a terminal pane, for a program it compiles and does not run; the deprecated web inspector (`--inspect`) serves live per-producer runtime state for a running program, which is the one view the program inspector still lacks.
...
| Hot reload of a running program (`--control`) | Implemented for logic, endpoint changes, and state resume; running two versions at once is **[Open]** |

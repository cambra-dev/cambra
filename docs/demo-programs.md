...
| [asset_cart](../tests/programs/asset_cart/) | A live asset cart: a filtered price stream, a transactional quantity per ticker, and a priced line served per view request | host channels (`channels.json`), record-valued sources, a comprehension filtering on a field, `Mut(Int, Txn)` written from a stream and read live in another loop, one sink per ticker | ✅ working (sink) | The gallery's first program whose meaning depends on a file beside it: `price_updates` is a name the *host* binds. Drives the pitch demo. Its reader is split one-per-ticker and its subtotal is left to the host because a block reading all six slots costs ~2 s per row — see the program text's TODO. `v1_single_line.cambra` is the rung above and ✅ runs: three keyed collections and three `wasm_serve` routes in place of six slots and a sink per ticker, one cart line per account, and a refined `Balance` the checkout's guard discharges. Two files are reload targets over that same wiring, each replacing the single `one_btc` with a divisor per asset. `v2_single_line.cambra` changes the arithmetic alone, so every collection keeps the name and shape v1 gave it and the swap inherits each value where it stands. `v2_migrating_single_line.cambra` also reshapes `cart` and `holdings` to carry a `scale` per entry, so each is declared under a new name and seeded from what v1 held through `@LoadFrom`; it is the demo's upgrade, and `scripts/asset-cart-upgrade.sh` drives it against the binary over the control port. Either way the cart, balances, holdings and quotes carry across and the new arithmetic takes effect. `v1.cambra` holds any number of lines per account and is 🚧 blocked, waiting on entry iteration (`for k -> v in m`) in a loop header and in a comprehension, and on a `for` inside `with begin():`. Their wiring is `v1_single_line.channels.json` and `v1.channels.json` beside them. |
...
| [order_ledger](../tests/programs/order_ledger/) | An order ledger totalled four ways: a loop-carried accumulator, a declarative aggregate, a transactional inventory, and a feed | `def` called from three sites, generator `def` + `yield`, `:=` over an induction extent, `Mut(Int, Txn)`, `with begin():` with two writes and a feed, `defer()`, filtered comprehension, record literals | ✅ working | Returns `7750`. The operator pane's shape-coverage program: every construct that pane draws differently, in one graph. 51 source lines convert to 222 operators — 52 `FanOutBranch`, 49 `Constant`, 41 `MapResult`, 24 `Memo` — which is the consolidation problem stated as a number. |

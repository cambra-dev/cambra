...
**drive** reads the store back and produces the body's input; the body sits between them, an
...
```
   source ──▶ drive ──▶ body ──▶ store ──▶ FanOut (cyclic) ──▶ readers
                ▲                                │
                └────────────────────────────────┘
...
| drive | `InductionDrive` | `TransactDrive` |
| readers | `StoreDenseRead` → `ExtractFinal` | `AsOf`, `StoreValueStream`, `StoreFinalRead` |

**Why the drive is a separate operator.** The accumulator has to reach the body, and the
only sanctioned route is a tile pulled along an edge. Splitting the roles puts it on one:
the store's output *is* the changelog, and the drive folds each position's previous value
out of it. Nothing is handed sideways.

It also means the induction drive needs no state of its own. The store's decided frontier
...
at the frontier. One `store_current` call answers both, so the drive's output is a pure
...
It is the rate every cyclic operator here runs at, and the drive re-arms on the wakeup queue
while a position remains to feed.

It also makes the cycle well-founded. The drive emits the position the store says is next,
...
*moves* the frontier — so the drive cannot tell from the store alone whether its item
...
moment it has *consumed* it, long before the attempt commits, which would advance the drive
past an item still in flight. So `TransactDrive` sits behind a `FanOut` with two branches —
...
```
  source ──▶ TransactDrive ──▶ drive fan ──▶ body ──▶ TransactWriter ──▶ CommitOperator
                   ▲               │                         ▲                  │
                   │               └─────────────────────────┘                  │
                   │                   the ack branch                           ▼
                   └────────────────── store fan (cyclic) ◀────────────────────┘
...
The drive's tile reaches the writer twice: once through the body, which turns it into a
...
| `InductionDrive` | only its own emitted window | a pull, from the store tile and the source tile | the body's `(prev…, item)` input |
| `TransactDrive` | the item cursor, the emitted rows with their items, the `(item, frontier)` retry key | a pull, plus **release** — which is what advances the cursor | the body's `(snapshot…, item)` input |

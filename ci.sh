...
ci_clippy() { cargo clippy -p cambra -p chl-parser --all-targets -- -D warnings; }
...
ci_clippy_release() { cargo clippy -p cambra -p chl-parser --release --all-targets -- -D warnings; }
...
# Both workspace members. `-p cambra` alone stopped covering the parser the moment it
# became its own crate, and its 74 tests would have gone on passing by not running.
ci_test() { cargo test -p cambra -p chl-parser -q; }
...
ci_doc() {
  RUSTDOCFLAGS="-A warnings -D rustdoc::broken_intra_doc_links" \
    cargo doc -p cambra -p chl-parser --no-deps
}
...
ci_fast() {
  ...
  # shellcheck disable=SC2310
  { cargo clippy -p cambra -p chl-parser -- -D warnings; } || failed=1

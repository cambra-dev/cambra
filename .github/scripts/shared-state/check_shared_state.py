...
EXPECTED_EXCEPTIONS = [
    ...
    # The subscribe scope, twice: the `thread_local!` and the cell inside it
    ...
    ("src/interpreter/types/extent.rs", "shared cell of `Restriction`"),
    # The probe table. The session that attaches it counts twice, for the
    # same reason the operator-graph recorder does. The handle is shared because
    # the driver reads one table while every producer writes it, and the producer
    ...
    ("src/interpreter/value_probe.rs", "ambient mutable state"),
    ("src/interpreter/value_probe.rs", "ambient mutable state"),
    ("src/interpreter/value_probe.rs", "shared cell of `ProbeTable`"),
]

  ...
  constructor(snapshot: Snapshot) {
    ...
    for (const pane of this.panes) {
      ...
      this.indicesByPane.set(
        pane.id,
        buildIndices(pane.root, pane.nodes, snapshot.definitions, snapshot.source.text),
      );
    }
...
function selectionsEqual(a: Selection, b: Selection): boolean {
  ...
  switch (a.kind) {
    case "source":
      return b.kind === "source" && a.from === b.from && a.to === b.to;
    // A `node` and a `locate` on one node ask different questions, so they are
    // different selections. Within either kind the pane and the node are the
    // whole selection: neither carries a region or a closure of its own.
    case "node":
      return b.kind === "node" && a.paneId === b.paneId && a.nodeId === b.nodeId;
    case "locate":
      return b.kind === "locate" && a.paneId === b.paneId && a.nodeId === b.nodeId;
    default: {
      // A kind added to `Selection` and left uncompared would be equal to
      // nothing, including itself, so every repeat of that gesture would
      // re-resolve.

...
// Whether a role is the named field `name`. The shape matters: a store key that
// renders as `fan` is not the fan edge.
export function isNamedRole(role: OperatorEdgeRole, name: string): boolean {
  return role.kind === "named" && role.name === name;
}
...
export interface OperatorEdge {
  ...
  // relation with no cycle guard; the share edges are the cross-references. A
  // cycle is a "value" edge with `deferred` set, which is what `isBackEdge`
  // tests and what dropping leaves the graph acyclic.
  kind: string;

...
export function drawGraphOf(pane: OperatorPane): DrawGraph {
  ...
  for (const n of pane.nodes) {
    if (isBranch(n) && n.inputs.some((e) => e.role === "fan") && outDegree(n.nodeId) === 1) {
  ...
  const resolve = (id: number, acc: number[]): number => {
    ...
    while (suppressed.has(at) && !seen.has(at)) {
      seen.add(at);
      acc.push(at);
      const fan = byId.get(at)?.inputs.find((e) => e.role === "fan");
  ...
  for (const n of pane.nodes) {
    ...
    for (const e of n.inputs) {
      ...
      const through = replaced.length
        ? byId.get(replaced[replaced.length - 1])!.inputs.find((x) => x.role === "fan")!
        : e;
      const id = `${from}>${n.nodeId}|${e.role}`;

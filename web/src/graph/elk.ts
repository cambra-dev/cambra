...
// promise on the calling thread, so a layout blocks the frame it runs in.
//
// That block is tens of milliseconds, not a few. Measured on the options below,
// against a dataflow-shaped graph: 24 ms at 45 nodes — the size the HTTP
// accumulator draws — 45 ms at 120, and 466 ms at 600. The first layout of a
// session adds roughly 60 ms of warm-up on top. Nothing bounds the node count,
// so a large program pays the tail of that curve on the main thread. The pane

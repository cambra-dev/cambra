...
./ci.sh fast     # Inner-loop gate: fmt + debug clippy (lib/bins) + the `z3` check + tests (`-p cambra`), plus the frontend's typecheck and vitest. Skips the release clippy pass, doc, shellcheck, doc-refs, formal, the fixtures gate, and the vendored-bundle freshness check. ~1/3 the time of the full gate — use this while iterating.
...
./ci.sh solver   # Checks for `z3` on PATH. Refinement subtyping runs queries against it, so without it the suite is red; this reports it once instead of as dozens of unrelated failures. Runs inside `fast` and the full gate.
...
- **Let**: `let x = e in body`.
- **MutDecl** (a mutable variable introduction — the `:=` operator's *declaring* half, the only node that binds a mutable variable): `x : Mut(V, D) := init` then `in body` on the next line, mirroring `Let`'s layout. The type suffix is omitted when the binder type is a bare `_`/`?N`, so a pre-inference introduction renders `x : Mut(_, _) := init`. Erased by the mutability-elimination phases, which turn it and its `MutWrite`s into a `LetRec` recurrence or a chain of shadowing `Let`s.
...
- **Σ type**: **always write the binder, and kind it with `:`.** `Σ (𝐷 : [𝐷₀, 𝐷₁]). 𝐷 ⤇ 𝑉` — a sum is a data arrow carrying its binders, so the body is always the arrow, and every binder scopes over the domain. A described witness kind goes in the same position by name: `Σ (𝐷 : UIntRanges). 𝐷 ⤇ 𝑇`. A listing kind is written in **brackets** — `[𝑇₀, 𝑇₁]`, or `[𝑇₀, …]` where the length is beside the point — because braces are the record type's and a listing whose only cardinality marker is an `ᵢ` subscript (`{𝑇ᵢ}`) does not say whether it is one candidate or many. Reserve `∈` for membership of a candidate in a kind (`𝑑 ∈ 𝐾`, in a rule premise), not for the binder. Do **not** use an anonymous shorthand (`Σ[𝐷₀, 𝐷₁] ⤇ 𝑉`) or a lattice-operator spelling (`⨆[𝑇ᵢ]`) — the binder is what makes the scoping readable.
...
After making code changes, run the formatter before running the code; prefer running the linter after ensuring the project builds. While iterating, `./ci.sh fast` (fmt + debug clippy + the frontend's typecheck and tests + the Rust suite) is the quick check — roughly a third of the full gate's time. **Before creating or pushing a PR, run the full `./ci.sh` and confirm it is clean** — GitHub CI gates on the same checks, and `./ci.sh` runs the parts no single `cargo` command covers and that `fast` skips: the other three clippy configurations (release, `serde`, lib-only), the doc build, the vendored-bundle freshness check in `ci_web`, and the `formal` gate (the Lean model plus its differential oracles). A green debug `cargo clippy` (or `./ci.sh fast`) is not enough (see Build Commands above).

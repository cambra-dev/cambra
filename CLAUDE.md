...
./ci.sh fast     # Inner-loop gate: fmt + debug clippy (lib/bins) + tests, all `-p cambra`. Skips the release clippy pass, the `wasm32` build, doc, shellcheck, doc-refs, formal, and the fixtures gate. ~1/3 the time of the full gate — use this while iterating.
...
./ci.sh wasm     # Builds the library for `wasm32-unknown-unknown`. Nothing else compiles that target, and the SMT solver is linked in rather than spawned so that it stays reachable. Runs inside the full gate only.

...
- **Let**: `let x = e in body`.
- **MutDecl** (a register introduction — the `:=` operator's *declaring* half, the only node that binds a register): `x : Mut(V, D) := init` then `in body` on the next line, mirroring `Let`'s layout. The type suffix is omitted when the binder type is a bare `_`/`?N`, so a pre-inference introduction renders `x : Mut(_, _) := init`. Erased by the mutability-elimination phases, which turn it and its `MutWrite`s into a `LetRec` recurrence or a chain of shadowing `Let`s.

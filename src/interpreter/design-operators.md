...
| `Materialize` | `SealedFunction(domain → *)` | `Scalar(Function(domain → codomain))` | Collects a streamed collection into one value — the whole bindings table in a single cell, which is the form a product value holds a collection-valued component in. Answers ⊥ until the input's domain is complete: a partial table is a different collection, not a smaller one. Releases each delivery as it takes it, so the source stays bounded; the only release it can take is the universal one. The inverse direction is a composition rather than an operator — `MapResult` of the table over an iteration of its domain. |

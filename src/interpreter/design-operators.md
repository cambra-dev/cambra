...
The **terminal** read is a different term, not a different classification of this one: `await_final(x)` (see `src/ccl/design/mutability.md`, "`await_final`") reduces the key's `StoreValueStream` to completion via `ExtractFinal` — the same stream this read samples, under a different reducer (fold-to-final vs. sample-at-trigger). The two are distinct terms all the way down from the surface — `as_of_read` and `final_or_default` — so which reducer a read gets never depends on the shape of the tree around it.

...
`CommitOperator`'s output is a [`Tile::Store`], not a `Function`: each tick carries only *that tick's* write-set delta, and a tick absent from the changelog is **decided-absent** — its value holds from the latest earlier change. Consumers must therefore **fold** the store (`store_current` / `store_value_at`), never index it. That is what makes a mutable variable readable while its store is still live: the current value is defined at the decided frontier, with no need for the history to end. Terminality is a flag separate from the frontier watermark, so a terminal store with trailing carries is not undercounted.

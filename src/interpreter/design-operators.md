...
The drive's own release runs the other way — outward, to the iteration source. `InductionStore`
reclaims the consumed prefix incrementally as `processed` advances, and releases the source in
full once it is complete and every arrived position is decided. That ends the drive: a drained

...
| `Function(bool)` | All-or-nothing interest in the lookup table. |
...
Refines interest in a `Function` tiling:
...
- **A function-tiled operator** (`Function`) — evaluate the function across a stream of inputs, essentially vectorising the pointwise definition. Used when the morphism sits downstream of an iteration or data source.
...
term produced: a scalar field stays a scalar, a collection field stays the sealed function it
already was, with the domain it binds. `MakeRecord` assembles it and [`SelectField`] reads one
field back out.

Keeping a field a tile is what lets it grow. A `Tile::Function` merges by appending its
...
| `IterateExtent` | None | `Function(extent → Scalar(extent))` | Enumerates all values in an `Extent`, producing an identity-mapping function (domain = codomain = extent) |
| `MapResultWithSource` | `Function(DataSourceDomain → Scalar(DataSourceDomain))` | `Function(DataSourceDomain → Scalar)` | Looks up each key of a data-source domain via `DataSourceDomainExtentImpl::get` to produce a function from keys to their output values. |
| `Zip` | `N` inputs of `Function(shared_extent → *)` tilings |  `Function(shared_domain → Record(_0, … _N))` | Merges N function operators that share a domain into one function whose codomain is a Record Tiling of all their codomains. Prefer the free `fan_in` factory at op-conversion call sites: it dispatches to `Zip` (function-tiled arms) or `ScalarFanIn` (scalar arms) based on the compiled arms' tilings, since the same CCL-level `zip` maps to either tile shape depending on upstream `input`. |
...
| `Converse` | `Function(domain → Scalar(codomain))` | `Function(codomain → domain)` | Inverts a function operator: each codomain value maps to the list of domain values that produced it. |
...
| `MapDomain` | `Function(A → *)` | `Function(A → Scalar(A))` | Replaces the codomain of a function with a copy of the domain values (identity codomain), producing an identity mapping from domain to itself. |
| `Filter` | Predicate: any tiling of type `A → bool` <br>Data: `Function(extent → Scalar(A))` | Same as input | Filters a function tile by a boolean predicate: keeps only domain elements where the predicate on the value evaluates to `true`. <br>TODO can probably remove this in favor of Restrict |
| `Restrict` | Predicate: any tiling of type `A → bool` <br>Data: `Function(A → *)` | Same as input | Filters a function tile by a boolean predicate: keeps only domain elements whose predicate evaluates to `true`. |
...
| `Aggregate` | `Function(* → Scalar)` | `Aggregation` | Reduces all codomain values of a `Function` input into a single running accumulator via an `AggregateKind` (e.g. Sum, Max). Currently, the aggregation is hardcoded in the graph, but we could add support for aggregate-kinds-as-data |
...
| `MapAggregate` | `Function(domain → codomain)` | `Function(domain → Aggregation)` | Performs a per-key aggregation |
| `MapExtractAggregate` | `Function(extent → Aggregation)` | `Function(extent → Scalar)` | Extracts terminal per-key aggregation results from a `Function(D, Aggregation)`, producing `Function(D, Scalar)`. |
...
- **`StoreValueStream` / `StoreValueStreamProducer`** — projects one key's commit-value stream `CommitTs ⇀ V` out of the store changelog, carrying the value forward across ticks that wrote other keys (the step interpolation), so its own output is a `Function` with a decided value at every tick. It backs the in-block reply tap (`carry_forward: false` — one entry per committed transaction) and the read-your-writes mutable variable carry (`carry_forward: true`).
...
`CommitOperator`'s output is a [`Tile::Store`], not a `Function`: each key carries only the ticks

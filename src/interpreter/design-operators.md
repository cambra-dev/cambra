...
## The universal-release promise

A **universal release is a promise never to request those positions again**, so a producer must not be pulled after one. `TileProducer::get` asserts this centrally (debug builds), because the failure is silent in both of its directions: an upstream that has gone quiet on release (`Constant`, `ExtractLast`) answers *empty*, so the puller computes over nothing; one that recomputes answers with the *same* data, which a caching consumer merges a second time — and a `Tile::Scalar`'s positions are implicit, so `merge` cannot tell "this position again" from "one more position" and appends, turning one value into two. Neither surfaces where it was caused.
...
| `ScalarFanIn` | `N` inputs with `Scalar` tilings | `Record(_0, … _N)` | Packs N scalar inputs into a single `Record` tiling where each field is a `Scalar` tiling. The scalar counterpart of `FanIn`; reachable from op-conversion via the `fan_in` factory. **Release semantics.** It re-reads every operand on every pull, so it forwards only a **universal** release — the one that says the consumer is done and there will be no next pull. Nothing narrower travels, since a scalar has no sub-region to free. Swallowing the universal one strands every producer beneath a binop operand or record field, and because `FanOut` forwards the *intersection* of its branches' guards, a single non-releasing branch blocks reclamation for all of them. |
| `MapResult` | Function: any tiling of type `A → B`<br>Data: `SealedFunction(extent → Scalar(A))` | `SealedFunction(extent → Scalar(B))` | Applies a function element-wise over a sealed-function input, transforming each codomain value. The function input can have many different tilings; currently supports `Scalar(ComputableFunction)`, `Scalar(Function)`, `CurriedFunction`, and `SealedFunction` tilings. When the **data** input is itself a `CurriedFunction`, it maps the function over each codomain list, producing a `CurriedFunction` with the same domain and transformed values. **Release semantics.** The input's guard is shaped from the *input's* tiling; the `function` operand is re-read on every pull, so it is released only on a **universal** release from the consumer — the one that says there will be no next pull. A narrower release must not reach it, or the next pull maps over an emptied function. |
...
| `Memo` | `*` | Same as input | Caches the output of an operator so it can be repeatedly read without recomputation. Immediately releases data from its input upon receipt so that the input can clear out any state — and once that release is **universal** (the input handed over a complete tile) it stops pulling the input altogether, serving the cache instead. A universal release is a promise never to re-request those positions, so a later pull would contradict it; and a pull that reaches an upstream which has not gone quiet returns the *same* data again, which `merge` appends, because a `Tile::Scalar`'s positions are implicit and "this position again" is indistinguishable from "one more position". One value silently becoming two is how that surfaces downstream, at whichever consumer broadcasts the result. |
...
A writer body returns one record per transaction: `commit : Bool`, the `writes` tuple, and one `to_<defer>_k` field per reply tap. A tap fed under one arm of cross-key *routing* carries a companion `to_<defer>_k__fire : Bool` gate holding that tap's own control-flow path (see [mutability.md](../ccl/design/mutability.md#general-in-transaction-conditionals-and-conditional-writes), "General in-transaction conditionals (and conditional writes)"). `body_decision_at` decodes the record and the grant path omits a non-fired tap from the commit delta, so a routed reply fires only on its own route. A tap whose path *is* the commit — a single-guard or spine feed — carries no gate and fires with its transaction, keeping unconditional programs at their gate-free shape.

### Convergence: the writer drives, one step per pull

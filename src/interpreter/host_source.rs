...
impl DataSourceDomainExtentImpl for HostSource {
    ...
    // A replacement takes over where the retired version left off. All three
    // are the buffer's, as they are for every source keyed by arrival order:
    // what a host pushed is held once and a new version of the program reads
    // the same buffer rather than the host's log again.

...
impl UnionOperator {
    ...
    fn flatten_domain(tiling: Tiling) -> Tiling {
        let (keys, values) = match tiling {
            Tiling::Function { keys, values } => (keys, values),
            other => return other,
        };
        ...
        .into_iter();
        let keys = arms.next().expect("at least one arm");
        for other in arms {
            assert_eq!(
                other, keys,
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        if self.flat {
            ...
            let (domain_extent, codomain_tiling) = match self.tiling() {
                Tiling::Function { keys, values } => (keys.clone(), (**values).clone()),
...
mod tests {
    ...
    fn a_fully_released_arm_is_not_pulled_again() {
        ...
        let Tile::Function { keys: domain, .. } = tile else {
            panic!("a union of functions is a function");
        };

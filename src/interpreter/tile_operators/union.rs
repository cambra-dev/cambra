...
impl TileProducer for UnionProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        let tiles: Vec<Tile> = self
            ...
            .iter_mut()
            .map(|p| {
                // An arm the consumer has fully released is not pulled again.
                ...
                // a universal release promises never to request those positions
                // again. The arm still has to *appear* here, since the arms are
...
mod tests {
    ...
    fn a_fully_released_arm_is_not_pulled_again() {
        ...
        producer.release(TileGuard::Function(FunctionGuard::Domain(
            Predicate::Union(vec![Predicate::True, Predicate::False]),
        )));
        ...
        let ColumnValue::Union { variants, .. } = domain else {

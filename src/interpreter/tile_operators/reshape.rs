...
impl PermuteRecordDomain {
    pub fn new(input: Box<dyn TileOperator>, permutation: Vec<usize>) -> Self {
        let Tiling::Function { keys, values } = input.tiling() else {
            panic!(
                "PermuteRecordDomain requires a collection input, got {}",
                input.tiling()
            );
        };
        let Extent::Record(input_fields) = keys else {
            panic!(
                "PermuteRecordDomain requires input with Record keys, got {}",
                input.tiling()
            );
        };
        let tiling = Tiling::Function {
            keys: Extent::Record(permute_record(input_fields.clone(), &permutation)),
            values: values.clone(),
        };
        Self {
...
impl TileProducer for PermuteRecordDomainProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        let ColumnValue::Records(input_fields) = keys else {
            unreachable!();
        };
...
impl FlattenTupleDomain {
    ...
    pub fn new(input: Box<dyn TileOperator>, indices_to_flatten: Vec<usize>) -> Self {
        ...
        let Extent::Record(outer_fields) = keys else {
            panic!(
                "FlattenTupleDomain requires Record keys, got {}",
                input.tiling()
            )
        };
        ...
        let tiling = Tiling::Function {
            keys: Extent::Record(flat_extent),
            values: values.clone(),
        };
        Self {
            base: OperatorBase::new(tiling),
...
impl TileProducer for FlattenTupleDomainProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        let Tile::Function {
            ...
            panic!("FlattenTupleDomain expected a collection");
        };
        let ColumnValue::Records(outer_cols) = keys else {
        ...
        Tile::grouped(row_starts, flat_domain, values, flat_pred, deleted)
    }
...
mod tests {
    ...
    fn flatten_tuple_domain_tiling_is_correct() {
        // IterateExtent produces `outer → outer`; the codomain is irrelevant here —
        ...
        let Tiling::Function { keys, .. } = op.tiling() else {
            panic!("expected a collection tiling");
        };
        let Extent::Record(fields) = keys else {
            panic!("expected a record key extent");
        };
    ...
    fn flatten_producer_get_flattens_nested_domain() {
        ...
        let input_tile = Tile::function(
            ...
            Box::new(Tile::Scalar(ColumnValue::Ints(vec![0, 0]))),
            Predicate::True,
            BitSet::new(),
        );
        ...
        let ColumnValue::Records(cols) = keys else {
    ...
    fn flatten_producer_get_passes_through_non_flattened_field() {
        ...
        let result = producer.get(producer.tiling().universal_guard());
        let Tile::Function { keys: domain, .. } = result else {

...
impl ExtractFinal {
    ...
    fn source_codomain_tiling(source: &dyn TileOperator) -> Tiling {
        match source.tiling() {
            Tiling::Function { values, .. } => *values.clone(),
...
impl TileProducer for ExtractFinalProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        if !source_tile.is_terminal() {
            ...
            if let Tile::Function { keys, deleted, .. } = &source_tile {
                let max_pos = (0..keys.len())
        ...
        let live_outer = (0..keys.len()).rev().find(|i| !deleted.contains(*i));
        if let Some(final_idx) = live_outer {
            let tile = if values.is_function() {
                ...
                let mut sub = *values;
                sub.retain(&BitVec::from_fn(keys.len(), |i| i == final_idx));
                let Tile::Function {
                    domain_predicate, ..
                } = &mut sub
                else {
                ...
                *domain_predicate = Predicate::True;
                ...
                let cv = scalar_tile_to_column_value(*values);
        ...
        let yields_a_level = self.tiling().is_function();

...
//! `DataFunction` off a join carries as many rows as the join produced — so
...
impl ValueRecorder {
    ...
    /// Forget a producer's recordings.
    ///
...
mod tests {
    ...
    #[test]
    fn a_collection_records_its_keys_values_and_watermark() {
        ...
        assert_eq!(recording.shape, "DataFunction");
    ...
    fn rows_survive_more_empty_calls_than_the_ring_can_hold() {
        let mut recorder = ValueRecorder::with_defaults();
        recorder.record(None, 1, "Restrict#1", &collection(&[0], &["a"], &[]));
        for _ in 0..DEFAULT_RECORDINGS_PER_PRODUCER * 4 {
            recorder.record(None, 1, "Restrict#1", &collection(&[], &[], &[]));
        }
    ...
    fn a_producer_answering_empty_on_a_second_pull_still_reads_as_its_data() {
        let mut recorder = ValueRecorder::with_defaults();
        recorder.set_tick(7);
        recorder.record(None, 1, "IterateExtent#1", &collection(&[0], &["a"], &[]));
        recorder.record(None, 1, "IterateExtent#1", &collection(&[], &[], &[]));
    ...
    fn a_producer_answering_twice_alike_is_not_doubled() {
        let mut recorder = ValueRecorder::with_defaults();
        recorder.set_tick(7);
        recorder.record(None, 1, "Memo#2", &collection(&[0], &["a"], &[]));
        recorder.record(None, 1, "Memo#2", &collection(&[0], &["a"], &[]));
    ...
    fn a_nested_collection_renders_its_group_size() {
        ...
        let mut recorder = ValueRecorder::with_defaults();
        recorder.record(None, 1, "GroupBy#1", &tile);
        let recording = recorder.recordings(None, 1).next().expect("recorded");
        assert_eq!(recording.shape, "DataFunction");
        assert_eq!(values(recording), vec!["<2 entries>", "<1 entries>"]);
    }

    #[test]
    fn a_store_records_its_changes_and_the_frontier_it_decided() {
        let mut recorder = ValueRecorder::with_defaults();
        recorder.record(
            None,
            1,
    ...
    fn the_recorded_count_advances_only_when_something_is_recorded() {
        ...
        recorder.record(None, 1, "P#1", &collection(&[], &[], &[]));

...
impl StoreDenseRead {
    pub fn new(
        ...
        let Tiling::Function { keys, .. } = trigger.tiling() else {
            panic!(
...
impl AsOf {
    ...
    fn build(
        ...
        let Tiling::Function { keys: b_ext, .. } = trigger.tiling() else {
...
impl TileProducer for AsOfProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        // compaction safe.
        if let (Tile::Function { keys: domain, .. }, Some(snap)) = (&trigger_tile, &snapshot) {
        ...
        let has_unlatched_position = matches!(&trigger_tile, Tile::Function { keys, .. }
        if (0..keys.len()).any(|i| {
            let b = keys.index_at(i);
...
fn newest_body_position(tile: &Tile) -> Option<usize> {
    let Tile::Function { keys, .. } = tile else {
        return None;
    };
    (0..keys.len())
        .filter_map(|i| match keys.index_at(i) {
...
fn body_decision_at(
    ...
    let Tile::Scalar(union_col) = values.as_ref() else {
        return None;
    };
...
mod tests {
    ...
    fn carry_dense_reader_does_not_over_release_store() {
        ...
        let mut read_values = |p: &mut Box<dyn TileProducer>| -> Vec<(usize, i64)> {
            ...
            let Tile::Scalar(col) = *values else {
                panic!("dense read values is a scalar column");
            };
            (0..keys.len())
                .map(|i| match (keys.index_at(i), col.index_at(i)) {
    ...
    impl TileProducer for DriverProbeProducer {
        ...
        fn get_impl(&mut self, projection_guard: TileGuard) -> Tile {
            ...
            if let Tile::Function { keys: domain, .. } = &tile {

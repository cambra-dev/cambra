...
impl TileProducer for CommitProducer {
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
        ...
        //
        // FAIRNESS / UNBOUNDED-STATE NOTE: this fixed low-to-high order can starve
        // a higher-index writer that contends with a lower-index one on a hot key
        // — the lower index always wins the tick and the higher index perpetually
        // retries. Output correctness is unaffected (every proposal is a pure
        // function of its snapshot, and `CommitProducer.consumed` blocks any
        // proposal from committing twice), but two resource costs bite a
        // *long-lived* contended store, not just throughput:
        //
        //   1. Starvation growth. `last_emit` suppresses a re-proposal only within
        //      one frontier; each time the winner advances the frontier the loser
        //      re-proposes at the new frontier, appending a fresh `emitted` entry.
        //      A stale proposal is reclaimed only when a *later* position commits
        //      and its cumulative prefix-release covers it (see `release_impl`), so
        //      under sustained contention `emitted` grows unbounded.
        //   2. Grant-then-deny orphans. An item that grants at frontier F (goes
        //      stale) then evaluates to *deny* on retry orphans its step-F proposal
        //      — never committed, never released; if no later item commits it is
        //      never swept.
        //
        // Bounding this needs the reclamation protocol to release *stale* proposals
        // (not only committed ones), which is delicate around the append-only
        // proposal positions and the `(item, frontier)` idempotency key — deferred.
        // A rotating start index (round-robin across pulls) would additionally make
        // the ordering fair.
    ...
    fn get_impl(&mut self, _projection_guard: TileGuard) -> Tile {
            ...
            && self.last_emit != Some((self.current, frontier))
        {
            ...
            match body_decision_at(&body_tile, pos, &self.tap_fields) {
                ...
                // The body producer maps the buffer synchronously on the pull
                // above, so it must have produced this position. A `None` here
                // means `body_decision_at` could not read a scalar/record
                // decision at `pos` — an unsupported decision-body tile shape
                // (e.g. a reply tap fed a list- or function-valued expression)
                // that should have been rejected earlier. Fail loudly: retrying
                // would re-read the same `None` forever (a busy-hang), which is
                // strictly worse to diagnose than an immediate panic.
                None => {
                    panic!(

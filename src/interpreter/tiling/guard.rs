...
impl TileGuard {
    ...
    pub fn check_from(&self, tiling: &Tiling) -> bool {
        match (self, tiling) {
            ...
            (TileGuard::Function(FunctionGuard::Domain(pred)), Tiling::Function { keys, .. }) => {
                pred.is_applicable_to(keys)
            }
            (TileGuard::Function(FunctionGuard::Codomain(g)), Tiling::Function { values, .. }) => {
                g.check_from(values)
            }

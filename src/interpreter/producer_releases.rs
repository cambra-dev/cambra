...
impl ProducerReleases {
    ...
    /// Forget `producer`'s record, because that producer is gone.
    ///
    /// A record left behind holds [`agreed`](Self::agreed) at wherever that
    /// producer stopped, forever.
    pub(crate) fn retire(&mut self, producer: &str) {
        self.per_producer.remove(producer);
    }

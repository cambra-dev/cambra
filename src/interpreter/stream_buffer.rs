...
//! common to every streaming data source keyed by arrival order (stdin, HTTP
//! server, host channels). Which producer has released what is
...
use crate::interpreter::{
    ColumnValue, Extent, Value, producer_releases::ProducerReleases, tiling::Predicate,
};
...
pub(crate) struct UIntStreamBuffer {
    ...
    /// What each producer has released, and where one registering from now on
    /// starts.
    pub(crate) releases: ProducerReleases,
}
...
impl UIntStreamBuffer {
    ...
    /// [`ProducerReleases::retire`](crate::interpreter::producer_releases::ProducerReleases::retire).
    pub(crate) fn retire_producer(&mut self, producer: &str) {
        self.releases.retire(producer);
    }
    ...
    /// this is `start_idx` unless the carried release runs past it. An induction
    ...
    pub(crate) fn release(&mut self, producer: &str, obsolete: Predicate) {
        ...
        // The agreement is every index no producer will read again, so it is what
        // the buffer may drop. A guard built for another extent is rejected the
        // first time one is read as an index set — here, or in `get_elements`.
        let agreed = Self::index_set(&agreed);
...
mod tests {
    ...
    ///
    /// This is a program's own producers, which all register during compilation:
    ...
    /// operators it rebuilds register as new producers, and a source hands a
    /// newly-registered one everything it still retains, so without the carry
    /// the replacement re-emits an output for every input its predecessor
    /// already answered.
    #[test]
    ...
    /// already handled.
    #[test]
    fn a_retired_producer_stops_holding_the_agreement() {
        let mut buf = buffer_with(3);
        ...
        buf.push(Value::String("e3".into()));
        buf.push(Value::String("e4".into()));
        buf.release("p2", covering(3, 4));
        assert_eq!(
            buf.start_idx, 5,
        ...
        assert_eq!(
            buf.get_elements("p3"),
            ColumnValue::from_uints(Vec::new()),
            "and the next version starts after what `p2` handled, not before it",
        );
    }

...
pub fn convert_record_fields_to_operators(
    ...
    match &expr.node {
        TypedExprNode::Let {
            ...
            // `let __store = Transact{…}`: build the shared store and register it
...
pub struct OpConversionContext {
    ...
    /// Transactional stores in scope, keyed by their `__store` binder. A
    /// `let __store = Transact{…}` builds the shared store once and registers
    /// it here; each variable read `__store.k` projects key `k` off the shared
...
impl OpConversionContext {
    ...
    /// Register a built transactional store under its `__store` binder.
    fn register_store(&mut self, name: Name, info: StoreReadInfo) {
        self.transactional_stores.insert(name, info);
    }

    /// Look up a transactional store by its `__store` binder.
...
fn convert_impl(
    ...
    let result: Result<Box<dyn TileOperator>, ConversionError> = match &expr.node {
        ...
        // `__store.k` — a read of variable `k` off a transactional store. The
        // shared store fan was built at `let __store = Transact{…}`; this
        ...
        TypedExprNode::Transact { .. } => Err(ConversionError::Unsupported(
            "Transact must be bound by a `let __store = …` (recognition invariant), \
        ...
        TypedExprNode::Let {
            ...
            // `let __store = Transact{…} in body`: build the shared store once
            ...
            if as_builtin(function) == Some(Builtin::Zip) =>
        {
            ...
            match &argument.node {
                TypedExprNode::Tuple(elts) => {
                    ...
                    //
                    // A **store-read arm** (`__store.k`) is a *leaf* source over
...
/// `__store.k` or an `as_of((trigger, store))` live read — rather than an

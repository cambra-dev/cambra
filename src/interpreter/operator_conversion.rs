...
fn compile_list_fn(
    ...
    let tiling = Tiling::function(domain.clone(), values_tiling);
    ...
    match elt_extent {
        ...
        Extent::Function {
            ...
            Ok((
                ...
                Tiling::function((**keys).clone(), values_tiling),
            ))
        }
...
fn build_product(
    ...
    // is no input here to read it off, so it is theirs — `main`'s rule, which required them
    // identical. A component that *is* a collection takes the value branch above, so what
...
/// must agree on it, which is the rule `main` enforced by requiring one tiling variant.
...
fn mutable_variables(expr: &Expr) -> Vec<MutableVariable<'_>> {
    ...
    fn go<'e>(
        ...
        if let TypedExprNode::Transact { keys, .. } = &e.node {
            ...
            for k in keys {
                ...
                let index = counts.entry((chain.clone(), name.clone())).or_insert(0);
        ...
        match binding_parts {
            Some((binding, definition, body)) => {
                let named = binding.name.source_spelling().map(str::to_string);
                if let Some(spelling) = named.clone() {
                    chain.push(spelling);
                }
                go(definition, chain, counts, out);
                if named.is_some() {
                    chain.pop();
                }
                go(body, chain, counts, out);
            }
            None => e.walk_children(|c| go(c, chain, counts, out)),
        }
    }
    ...
    let mut out = Vec::new();
    go(expr, &mut chain, &mut counts, &mut out);
    out
}
...
fn selects_a_tile(tiling: &Tiling, name: &str) -> bool {
    match tiling {
        ...
        _ => matches!(
            ...
            Tiling::Record(fields) if fields.contains_key(name) && fields.values().any(Tiling::has_a_level)
        ),
    }
}
...
fn reject_unanswerable_lookup_collection(tiling: &Tiling) -> Result<(), ConversionError> {
    let answerable = match tiling {
        Tiling::Function { values, .. } => matches!(values.as_ref(), Tiling::Scalar(_)),
...
fn result_extent(tiling: &Tiling) -> Extent {
    match tiling {
        ...
        Tiling::Function { values, .. } => result_extent(values),
...
mod variant_ctor_tests {
    ...
    fn single_row(tile: Tile) -> Value {
        ...
        assert_eq!(keys.len(), 1, "one-element scrutinee → one output row");
        let Tile::Scalar(cv) = *values else {
    ...
    fn variant_elim_outer_binder_zip() {
        ...
        assert_eq!(keys, ColumnValue::from_uints(vec![0, 1]));
        let Tile::Record(fields) = *values else {
            panic!("expected a Record values, got {values:?}");
        };
    ...
    fn variant_ctor_in_lambda_composes() {
        ...
        assert_eq!(keys, ColumnValue::from_uints(vec![0, 1]));
        let Tile::Scalar(cv) = *values else {
            panic!("expected a Scalar(Union) values, got {values:?}");
        };

...
/// (`Function`), which planning's group-by rewrite builds from `converse`. A
...
///
/// The caller enters the scope, because the body is converted in it.
fn compile_let_binding(
    ...
    ctx: &mut OpConversionContext,
) -> Result<Option<Box<dyn TileOperator>>, ConversionError> {
    ...
    ctx.bind_let(&binding.name, bound_expr, bound_input)?;
    Ok(body_input)
}
...
struct KeyReadInfo {
    ...
    /// the guard also reads ([`state_identities`]).
...
struct StoreReadInfo {
    ...
    /// the version that built it. Compared against the next version's sites to
...
pub struct OpConversionContext {
    ...
    /// What the version this one replaces left behind, to be kept where the
    /// computation is unchanged. Consulted at every `Let` binding
    /// ([`bind_let`](Self::bind_let)), every store
    ...
    /// The bindings this compilation built rather than kept, by binder name.
    ///
    /// What makes reuse hereditary. A term is eligible for reuse only when none
    /// of the names free in it is here, so a kept operator is never left reading
    /// a subgraph this compilation rebuilt. The node correspondence cannot answer
    /// this on its own: it matches a term's free variables by spelling, so an
    /// unchanged term can read a binding that changed.
    ///
    ...
    /// it, from [`state_identities`] over the tree about to be converted.
...
impl Inheritance {
    ...
    fn stores(&self) -> impl Iterator<Item = &StoreReadInfo> {
        self.entries.values().filter_map(|e| match e {
            Recorded::Store(info) => Some(info),
            Recorded::Operator(_) => None,
        })
    }
...
/// through this map — so an operator is kept exactly where the two versions'
/// trees correspond and the computation is unchanged.
///
/// Only [`Content::Same`](crate::ccl::diff::Content::Same) correspondences are
...
impl Correspondence {
    /// The unchanged correspondences of `diff`, whose `dst` is the tree about to
    /// be converted.
    fn of(diff: &Diff<'_>) -> Self {
        Self {
            previous: diff
                .shared()
                .map(|m| (m.dst.node_id(), m.src.node_id()))
                .collect(),
        }
    }

    /// The node `node` replaces, when the two versions compute the same thing
    /// there.
...
/// What the graph holds at one node.
///
/// One type for all three reuse sites, and a sum because a node is one thing or
/// the other: `bind_store` is reached exactly when the bound term is a
/// `Transact` and `bind_let` otherwise, so a store's reads and a bare operator
/// never both sit at a node. Two maps keyed the same way, one per site shape, is
/// what this replaces; they meant every traversal was written twice and every
/// field added to one had to be remembered for the other.
///
/// Keeping the entry keeps the whole subgraph under it. What that subgraph holds
/// is not recorded here: the region corresponds node-for-node, so
/// [`keep_region`](OpConversionContext::keep_region) finds it by walking the
/// tree.
#[derive(Clone)]
enum Recorded {
    /// A `Let` binding's operator, or a writer's iteration input.
    Operator(Rc<FanOut>),
    /// A `Transact` store, which every mutable-variable read projects.
    Store(StoreReadInfo),
}

impl Recorded {
    /// The fan-out every use of this node branches off. A store's is its own
    /// cyclic fan, which is why the two arms are not a fan plus a payload.
    fn fan(&self) -> &Rc<FanOut> {
        match self {
            Recorded::Operator(fan) => fan,
            Recorded::Store(info) => &info.fan,
        }
    }
}

/// What a recurrence iterates: the operator, and the first position it will
/// offer.
///
/// The two travel together because a position number means nothing without the
/// input it counts in. A drive told to start below what its input still offers
/// waits for an element that has gone; told to start above, it skips one nothing
/// has decided.
...
impl OpConversionContext {
    ...
    /// once, before conversion begins, and not at all for a first compilation.
    pub fn set_correspondence(&mut self, diff: &Diff<'_>) {
        self.correspondence = Correspondence::of(diff);
    }
    ...
    fn reads_only_kept(&self, bound_expr: &Expr) -> bool {
    ...
    fn bind_store(
        ...
        // ([`state_identities`]) rather than derived here, so a variable is
        ...
        let info = match correspondent {
            Some(Recorded::Store(info)) if !spent => {
                ...
                // this compilation does not walk to them, so they are re-recorded
                // here rather than at `iteration_input`.
                self.keep_region(bound_expr, &info.fan);
                info
            }
            Some(Recorded::Operator(_)) => unreachable!(
...
records one and the only site a `Transact` reaches"
        ...
        let info = StoreReadInfo {
            // The site of the version minting this store, kept or rebuilt, so the
            // next version compares against where its state actually sits.
            site: content_hash(bound_expr),
            ..info
        };
        self.record(node, Recorded::Store(info.clone()));
    ...
    /// ([`recomputable`](Self::recomputable)), and where it is not the spent operator is
    ...
    fn bind_let(
            ...
            && !spent_but_rebuildable
        {
            ...
            // compilation does not walk to it, so it is re-recorded here rather
            // than where it was first bound.
    ...
    /// term is recomputable ([`recomputable`](Self::recomputable)) and nothing recovers them
    ...
    /// unexplained — see
    /// [`record_kept_operators`](crate::interpreter::operator_graph::record_kept_operators).
    ///
    /// The caller records the root. This records what is under it.
    ///
    /// `walk_children` visits a subset of the children the differ pairs — it
    /// skips the predicates of a `Cast` target, which live in a `Type` — and that
    /// is sound in both directions here: conversion discards a cast target
    /// (`convert_impl`), so no entry is ever recorded inside one.
    fn keep_region(&mut self, term: &Expr, kept: &Rc<FanOut>) {
        kept.with_input(record_kept_operators);
        fn nodes_below(e: &Expr, out: &mut Vec<NodeId>) {
        ...
        for node in below {
            let Some(previous) = self.correspondence.previous(node) else {
                debug_assert!(
                    false,
                    "a kept region corresponds `Content::Same` throughout, so every node \
in it has a correspondent",
                );
                continue;
            };
    ...
    pub fn state_conflicts(&self, planned: &Expr) -> Vec<StateConflict> {
        ...
        for (site, target) in identities.read_load_from(&declared_by_predecessor) {
            let path = match target {
                LoadTarget::Variable(path) => path,
                // Refused rather than resolved by position: which of the
            ...
            if !held.contains_key(&path) {
                out.push(StateConflict::Undecided { path: path.clone() });
                taken_over.insert(path);
                continue;
            }
            // The same comparison a declaration gets, for the same reason: a
            // value used at another shape is a constant of the wrong extent, and
            // the operator built around it fails on its first pull rather than
        ...
        for info in self.minted.stores() {
            for (path, key) in info.carried_keys() {
                ...
                if let Some(now) = site_moved(path, info.site, &declared) {
    ...
    /// ([`recomputable`](Self::recomputable)) and nothing recovers them where it is not. This
    ...
    pub fn unreadable_inputs(&self, previous: &Expr, planned: &Expr) -> Vec<UnreadablePrefix> {
        ...
        for (node, vars) in state_identities(planned).stores() {
            ...
            let continues = fresh.len() < vars.len();
...
fn convert_impl_inner(
    ...
    let result: Result<Box<dyn TileOperator>, ConversionError> = match &expr.node {
        ...
        TypedExprNode::Let {
            ...
            let mut scope = ctx.enter_scope();
            let body_input =
                compile_let_binding(expr.node_id(), binding, bound_expr, input, &mut scope)?;
            ...
            if as_builtin(function) == Some(Builtin::Zip) =>
        {
            ...
            // The arms are each applied to `input`, so the levels they run over are its —
            // which is what the pair sits under. Their own shapes cannot say: two arms that
            // agree below the ambient (two projections of one grouped row, say) look
            // pairable all the way down.
            let ambient = level_count(input.tiling());
            match &argument.node {
        ...
        TypedExprNode::Builtin(Builtin::EmptyMap) => {
            ...
            if !matches!(extent, Extent::Function { .. }) {
                return Err(ConversionError::TypeError(format!(
                    "`empty_map()` is a collection, so its type is a function; this one is \
                     {extent:?}"
                )));
            }
        ...
        TypedExprNode::Builtin(b) => {
            ...
            match b {
                ...
                Builtin::VariantProject(tag) => {
                    ...
                    // `VariantCtor` shape) or a union *stream* `Function {
        ...
        // assigns the identities stores are built under, so the site and the
        // variable cannot disagree.
        //
        // The missing value is the cold start: a first compilation inherits
        // nothing, and nothing guards it, because there is no running program to
        // refuse on behalf of. A *reload* never arrives here without one —
        // `state_conflicts` has already refused that version while the running
        ...
        TypedExprNode::LoadFrom(name) => {
            ...
            let extent = ctx.extent_of(&expr.ty)?;
            // A loaded value is a CHL value of the type the declaration states, so a
            // function extent here is a `Map` — a collection, which is what decides
            // whether a comprehension over the binding transforms the table or its
            // outputs. Nothing applies this constant the way a list literal's table
...
/// must agree on it, which is the rule `main` enforced by requiring one tiling variant. A
/// scalar component contributes no level and the pairing is a plain record, which
/// [`fan_in_at`] routes elsewhere.
...
fn build_commit_store(
    ...
    for ((set_writer, w), taps) in setters.into_iter().zip(writers.iter()).zip(per_writer_taps) {
        ...
        // A drive counts commits against the positions its source offers, so
        // where it starts is whatever the source it is handed still has. A source
        // this version keeps has already passed some of them. A freshly-built one
        // starts at `0` whatever the source has done: this drive finds its next
        // item by scanning up from its cursor, so an advanced source costs it a
...
///
/// The address is the source's own: the bindings whose definitions enclose the
/// declaration, then the spelling. `a = count_by(src, 1)` and
/// `z = count_by(src, 10)` inline to two `Transact`s under the definitions of `a`
/// and `z`, so the two variables spelled `n` are `a`.`n` and `z`.`n` — distinct
/// without either of them having to be counted. Reordering the two bindings, or
/// adding a third, moves neither address.
///
/// Only a binder the author wrote enters the chain
/// ([`Name::source_spelling`]). A minted one is named by a uid whose value
/// depends on how many the passes before it minted, and the bindings a pass
/// interposes are not something the source says, so neither addresses anything
/// two versions of a program agree on.
///
/// The index counts among the variables sharing *both* the chain and the
...
/// one function can differ only in their writer bodies once arguments are
/// substituted, so any content-derived identity either fails to tell them apart
/// or changes under exactly the edit state has to survive. Content still decides
/// whether a *site* is the one that held a value, which is a narrower question
/// than which variable a value belongs to — see [`site_moved`].
#[derive(Clone, Debug, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct VarPath {
    /// The bindings whose definitions enclose the declaration, outermost first.
    chain: Vec<String>,
    /// The variable's own spelling — [`Name::field_key`].
    name: String,
    /// Its position among the variables sharing this chain and this spelling.
    index: usize,
}
...
///
/// A variable is named by the bindings enclosing its declaration, its spelling,
/// and — where the source supplies no name to tell two sibling declarations
/// apart — its position among them. The position is what two anonymous call
/// sites of one stateful function come down to, and it moves when they are
...
fn site_moved(
    path: &VarPath,
    site: ContentHash,
    declared: &HashMap<VarPath, DeclaredVariable>,
) -> Option<VarPath> {
    if declared.get(path).is_some_and(|d| d.site == site) {
        return None;
    }
    declared
        .iter()
        .find(|(other, d)| *other != path && d.site == site)
...
/// writer it has.
///
/// Keyed the way [`StateIdentities::stores`] is, so the two read together. That
...
/// A term that reads a data source, which is what makes it **unrecomputable**:
/// building it again does not yield what it held.
///
/// A source hands a new producer only what its retired ones had not finished
/// (`src/interpreter/producer_releases.rs`), so a rebuild sees a suffix where
/// the term's own meaning names the whole. A literal is recomputable, and so is
/// any term built only from recomputable terms.
///
...
/// term's answer is its children's. Propagated through the bindings between, a
/// term reading a mutable variable is unrecomputable through the `Transact` that
/// declares it — the store folds a source, and a rebuilt store folding the suffix
/// holds a different value.
pub fn reads_a_source(node: &TypedExprNode) -> bool {
    matches!(node, TypedExprNode::Source(_))
}
...
/// seeds ([`OpConversionContext::iteration_input`]).
///
/// Asked of every node rather than of one, because the seed reaches the store
/// through the bindings between: `@LoadFrom` binds a `let`, and the declaration
/// that uses it names that binding rather than containing the leaf.
pub fn is_a_load(node: &TypedExprNode) -> bool {
...
pub(crate) struct LoadFromSite {
    ...
    /// diagnostic names when nothing answers: a site that resolves to no
    /// variable has no address to report, and the spelling is what the source
    /// contains.
    name: String,
    /// The bindings enclosing the site, outermost first — the chain a
    /// declaration here would take.
    chain: Vec<String>,
    /// What inference concluded the value is, which the value the predecessor
    /// holds has to fit.
    ty: Type,
}
...
impl LoadFromSite {
    ...
    ///
    /// The binding the load seeds is not on that chain
    /// ([`state_identities`]), so the search starts outside it. Carrying it
    /// would make the innermost candidate one no source can name: a predecessor
    /// that declared a variable of this spelling inside an instantiation bound
    /// to the load's own target name would answer for it.
    ///
    /// A chain declaring the spelling once has nothing to tell apart and answers
    /// whichever site asks, which is a site per instantiation reaching one
    /// declaration of an enclosing scope. A chain declaring it more than once is
    /// [`LoadTarget::Anonymous`]: those declarations are the anonymous call sites
    /// of one stateful function, told apart by position alone, and a load carries
    /// no position that confirms one. [`site_moved`] catches a declaration moving
    /// between such positions because the retired site's content survives the
    /// edit that moved it; a load edits the body of every site it sits in, so no
    /// content corresponds and the value would move between them unremarked.
    fn resolve(&self, declared: &HashSet<VarPath>) -> LoadTarget {
        for n in (0..=self.chain.len()).rev() {
            let at = |index| VarPath {
                chain: self.chain[..n].to_vec(),
...
fn state_identities(expr: &Expr) -> StateIdentities<'_> {
    type Counts = HashMap<(Vec<String>, String), usize>;
    ...
    fn go<'e>(e: &'e Expr, w: &mut Walk<'e>) {
        ...
        // A binding's definition is an address the source names, so a `Transact`
        // inside one is addressed under that name. The binding's *body* is not:
        // what the body declares belongs to the enclosing chain. `walk_children`
        // visits exactly these two children for both node kinds.
        let binding_parts = match &e.node {
            TypedExprNode::Let {
                binding,
                bound_expr,
                body,
            } => Some((binding, bound_expr.as_ref(), body.as_ref())),
            TypedExprNode::MutDecl {
                binding,
                init,
                body,
            } => Some((binding, init.as_ref(), body.as_ref())),
            _ => None,
        };
        match binding_parts {
            Some((binding, definition, body)) => {
                // A `@LoadFrom`'s binding is not an address. Its definition is the
                // leaf and nothing else, so nothing is ever declared under it,
                // while the site inside it would otherwise start its outward
...
impl<'e> StateIdentities<'e> {
    /// Every store, as its declaring `Transact` with the variables it declares,
    /// in key order.
    ///
    /// Keyed by the declaration rather than by the variable because the questions asked
    /// of a store — whether it continues a recurrence, where its drive resumes — are
    /// answered for all of its keys at once.
    fn stores(&self) -> HashMap<NodeId, Vec<&MutableVariable<'e>>> {
        let mut out: HashMap<NodeId, Vec<&MutableVariable<'e>>> = HashMap::new();
        for v in &self.variables {
            out.entry(v.declared_by).or_default().push(v);
        }
        out
    }

    /// [`Self::stores`] carrying each variable's identity alone.
    ///
    /// Handed to conversion so that a store is built under the identities the
    /// guard checked, rather than under a second derivation of them. Owned,
    /// because the context holding it outlives the tree the walk borrows.
    fn variable_paths(&self) -> HashMap<NodeId, Vec<VarPath>> {
        self.stores()
            .into_iter()
            .map(|(node, vars)| (node, vars.iter().map(|v| v.path.clone()).collect()))
            .collect()
    }
    ...
    /// Deriving it twice would mean two computations that have to agree, and a
    /// disagreement reads as a variable that is still declared having been
    /// dropped.
    fn declared(&self) -> HashMap<VarPath, DeclaredVariable> {
        self.variables
            .iter()
            .map(|v| {
                (
    ...
    ) -> impl Iterator<Item = (&'s LoadFromSite, LoadTarget)> {
...
pub enum StateConflict {
    ...
    /// declarations are told apart by position, and the content that would
    ...
    /// Separate from [`Self::NoPredecessor`] because the name addresses a real
    ...
    /// it, so a record used one field at a time concludes a record of that one
    ...
    /// scope at that point.
    ///
    /// Carries the bare spelling rather than a [`VarPath`], because nothing
    /// resolved: an address with an empty chain would read as one the source
    /// named, and the source names only `x`.
    ///
    /// No `@LoadFrom(x, default)` escape hatch answers this. A default turns the
    /// hard error back into a silent wrong answer, which is the failure the
    /// guard exists to remove — so a version containing a `@LoadFrom` is an
...
impl std::fmt::Display for StateConflict {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            StateConflict::Dropped { path } => write!(f, "{path} is no longer declared"),
            StateConflict::Retyped {
                path,
                held,
                declared,
            ...
            StateConflict::Undecided { path } => write!(
                f,
                "{path} is declared but has decided no value, so there is nothing to load from it \
                 yet",
            ),
            ...
            StateConflict::Moved { path, now } => {
                ...
                // so they render identically and the caller's sort collapses them.
                let (first, second) = if path <= now {
                    (path, now)
                } else {
                    (now, path)
                };
                write!(
                    f,
                    "{first} and {second} are told apart only by where they appear, and this \
                     version moves a value between them",
                )
            }
        }
    }
...
fn reject_unanswerable_lookup_collection(tiling: &Tiling) -> Result<(), ConversionError> {
    let answerable = match tiling {
        Tiling::Function { codomain, .. } => matches!(codomain.as_ref(), Tiling::Scalar(_)),
...
/// `Function` over some domain (morphism context) or as a `Scalar`
...
///
/// `input` must have a `Function` tiling.
...
/// for `Function { codomain, .. }` returns `codomain.extent()`.
...
mod recomputability_tests {
    ...
    fn a_term_built_from_literals_is_recomputable() {
        ...
        assert!(
            nodes_reaching(&e, [reads_a_source])[0].is_empty(),
    ...
    fn reading_a_source_reaches_every_term_above_it_and_every_use_of_its_binding() {
        ...
        let [out] = nodes_reaching(&e, [reads_a_source]);
...
mod variant_ctor_tests {
    ...
    fn variant_elim_outer_binder_zip() {
        ...
        let stream_tiling =
            Tiling::function(Extent::Base(BaseType::UInt), Tiling::Scalar(stream_extent));

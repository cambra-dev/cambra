...
impl Name {
    ...
    /// mutable variable key projects this field of the mutable variable record
    /// (`__reg.field_key`).
    ///
    ...
    /// mutable variable record — every consumer resolves it against a `keys_map`
    ...
    /// of construction: a key spelling is the user's own variable name, distinct
    /// within the block or loop that declares it, or a writer's reply tap
    /// (`to_<base>_<n>`), which shares the record with them. Nothing in the type
    /// system rules a collision out, so each site that builds a record from these
    /// labels asserts distinctness in debug: `register_record` in
    /// `planning/loops.rs`, and the two `keys_map` inserts in

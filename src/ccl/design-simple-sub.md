...
`issues/type-checker-immutable-predicate-terms.md`.  The goal is replace all of this with
-  immutable predicate terms (bare
Rc<TypedExpr>)
- unique binders (Barendregt convention, α-uniquified at
lowering with structured names so display stays readable)
- one uniform capture-avoiding substitution over terms and types

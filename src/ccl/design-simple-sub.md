...
The inference engine drives the AST through two passes, defined in `infer_simple_sub.rs`, mirroring the academic paper's `typeTerm`/`constrain` and `coalesce` algorithms. A Cambra-specific lexical-scope propagation rides *inside* the coalesce walk (see Pass 2) rather than as a separate pass.
...
**Edges carry substitutions.** Each entry of a variable's bound lists is a `Bound { ty, subst }`, where `subst : ctx(ty) → ctx(holder)` rides the constraint edge (identity for ordinary bounds). `constrain_subtype` delegates to `constrain_go(lhs, rhs, sigma, cache)`: the **Fun/Fun arm derives the binder correspondence** `[𝑘 ↦ 𝑥]` for the codomain edge (contravariantly inverted on the domain), and the var arms record edges under `sigma` (lower) / `sigma⁻¹` (upper). The reverse of a discharge is taken as identity (`invert_or_id`) — sound because a discharge only rewrites predicates (opaque to `constrain`, §4) and the consumer bound it is recorded against never mentions the private binder. Under an identity `sigma` every arm reduces exactly to the substitution-free solver, so all monomorphic inference is byte-identical.

**Coalesce forces suspended substitutions.** `compact_go` threads a substitution accumulator: descending a bound edge composes its `subst` (`then(edge, acc)`), and the composite is applied — *forced* — at each refinement-predicate leaf. A bound reached transitively through `𝑣 → 𝑤 → …` thus arrives with every edge's morphism composed (the deferred transitive closure recovered by the walk). Identity accumulator ⇒ no-op.
...
The expected binder is *not* always fresh: when `fn_ty` is already a concrete Pi `(𝑘: 𝑑) ⇒ …` (the directly-applied case — a `λ 𝑘 → …` or a resolved dependent function), `apply` **reuses `𝑘`** as the expected binder, so the derived correspondence is the **identity** and the discharge is keyed on the real binder `𝑘`. This is what lets the discharge reach *contravariant* positions: a partition produced by `g(𝑖)` and fed to the `λ` of a `map`/aggregate lands in that `λ`'s parameter **domain** (a negative position, coalesced from upper bounds). A fresh expected binder `𝑥` would put a *rename* `[𝑘 ↦ 𝑥]` on the codomain edge whose **inverse** `[𝑥 ↦ 𝑘]` rides the contravariant edge, and a one-way discharge composed onto an inverse rename is a no-op on the predicate — leaving the binder undischarged at the parameter domain. The identity correspondence sidesteps that (resolved O8; only the extent-join case where two *distinct* dependent functions share a binder name is deferred, O1/O4).
...
| **`Subst` / discharge / rename** | Cambra-Specific | A context morphism over *term* binders (`ccl::subst`), riding a constraint edge in a `Bound { ty, subst }`. A **rename** `[𝑘 ↦ 𝑥]` is invertible; a **discharge** `[𝑥 ↦ arg]` (dependent application) is one-way. Composed along the coalesce walk and forced at refinement predicates. See §4.5. |

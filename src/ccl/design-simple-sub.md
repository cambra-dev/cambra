...
* **SMT-backed refinements.** Augmenting the lattice-carried refinement tags (today compared by id or structural predicate equality) with logical payloads (e.g. `v > 0`) reasoned about by *implication*, not just equality — via an external SMT solver such as Z3.

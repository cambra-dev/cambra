...
* **Lattice-carried refinements.** Refinement witnesses ride the lattice natively (compared by structural predicate equality) rather than being stitched on by a post-pass; see §4 and [`crate::ccl::simple_sub`]'s `# Refinements`.
...
**`let`-closing (codomain extraction).** A `let 𝑥 = 𝑣 in body` node's type is the body's type, which may close over `𝑥`. As that type is lifted out of the `let`'s scope, `coalesce_node` discharges `[𝑥 ↦ 𝑣]` into it (derived from the body's already-closed type, so chained `let`s close to fixpoint) — the design's `let`-closing refinement-move site. Together with the contravariant discharge above, every coalesced node's type is **well-formed in its lexical scope**, checked at the end of inference by `check_scope_valid` (§6.2) in debug builds: a free predicate variable must be bound by an enclosing Pi binder or AST binder, or be a source. A violation is a compiler bug (a substitution-descent miss leaving a dangling predicate binder), reported as an internal `InferError::ScopeViolation`. The check demoted from release-unconditional to a debug-build regression net when the uniform substitution retired the value-only contract — type-borne occurrences are rewritten in the same pass as the term, so the dangling-binder class it guarded is structurally unrepresentable; the per-substitution `debug_assert`s in `ccl::subst` remain as fast-path guards.
...
* **O2 (polymorphic case)** — `freshen_above` now *does* copy-and-freshen a refined value's predicate type slots through the shared cache (its `Refinement` arm), so a specialization's predicate is a proper freshen instance rather than a shared `Rc`. Immutable predicate terms are acyclic, so the refinement-cycle guard that once made this hard is unnecessary. A residual under-determination remains for one shape — a projecting inner comprehension over each group of a group-by (`[sum([s.amount for s in g]) for g in groupby(...)]`) leaves a predicate slot unresolved — but it is a feature that does not yet compile for an unrelated reason (incomplete `curry` support in op-conversion); no compiling program hits it.
* **O4** — two *different* discharges of one refinement (`g(0)` vs `g(1)`) are distinguished once forced — `force_refinement` rewrites the predicate term and witness equality is structural (§4) — but the constraint cache still keys on bare `(lhs, rhs)`, so two such discharges arriving at the *same* variable conflate at the edge level before either is forced (the extent-join corner, O1/O4). A σ-aware cache is tracked separately.
...
| **CompactType** | Simple-Sub | A flat, per-position bag of contributions (variables, atoms, an optional record shape, an optional function shape, and a witness set) produced for simplification and co-occurrence analysis. |

...
pub(crate) fn debug_assert_predicate_shape(predicate: &TypedExpr) {
    #[cfg(debug_assertions)]
    {
        fn offending(e: &TypedExpr) -> Option<&'static str> {
            ...
            // A cast's `target` is a type, so `walk_children` does not reach it, while
            // `eq_term_modulo_ty_slots` does compare that slot. A statement shape nested
            // one refinement deep is the same violation as one at the top, so the guard
            // has to reach as deep as the comparison it backs.
            if let N::Cast { target, .. } = &e.node
                && let Some(set) = ccl_utils::cast_target_refinement(target)
                && let Some(found) = set.iter().find_map(|r| offending(&r.predicate))
            {
                return Some(found);
            }
            let mut found = None;
...
/// [`crate::ccl::subst::Subst::eq_modulo_ty_slots`] asks it about a discharge's
/// captured argument, which is *any* expression the program wrote — a function body
/// among them. So every node a program can build needs an arm here, and the
/// restriction that a predicate is a boolean expression over
/// [`crate::ccl::REFINEMENT_BINDER`] is asserted where predicates are installed
/// ([`Refinement::born`]) rather than left implicit in this match.
///
...
/// stream (binder names, `Source` names, a [`TypedExprNode::MutWrite`]'s written
/// name, record field names, pattern tags, cast-target predicates) makes eq finer
/// than hash, preserving the `Eq`/`Hash` contract.
...
/// Whether [`eq_term_modulo_ty_slots_go`] has an arm for this node's shape.
///
/// Exhaustive, with no `_`: a variant added to [`TypedExprNode`] stops this compiling,
/// which is the signal to give it an arm. Without that, a new variant reaches the
/// pairwise match's catch-all and two copies of it compare unequal — a run-time failure
/// where the type system can give a compile-time one, and the way a variant goes missing.
...
fn eq_term_modulo_ty_slots_go(
    ...
    match (&a.node, &b.node) {
        ...
        // A `LoadFrom` name is not a reference to a binder the compared term introduces:
        ...
        _ => {
            ...
            //
            // `LetRec` and `Transact` are the shapes deliberately left out. A
            // predicate never acquires one: the phases that mint them do not rewrite
            // inside a predicate (`debug_assert_predicate_shape`). Neither does a
            // discharge's captured argument, taken during inference, before those
            // phases run. One reaching here is a violation of that boundary rather
            // than a missing arm.
            debug_assert!(
                std::mem::discriminant(&a.node) != std::mem::discriminant(&b.node),
                "two nodes of one shape compared unequal, so a rebuilt term no longer \
                 equals itself: {:?} is {}",
                std::mem::discriminant(&a.node),
                if eq_has_arm(&a.node) {
                    "handled above, so the arm did not match what it claims to"
                } else {
                    "a shape a predicate must not hold"
                },
            );
            false
        }
...
/// Structural hash of a term, the hashing counterpart of
/// [`eq_term_modulo_ty_slots`]: it hashes the term's node discriminant
...
/// types keeps the hash stable while inference resolves the term's
/// type slots in place, and keeps it non-recursive through refinements (so
/// it always terminates and adds no extra borrows).
///
/// A reference to a binder the term itself introduces hashes as that
...
mod tests {
    ...
    /// [`hash_term_modulo_ty_slots`] of `e`, for asserting the `Eq`/`Hash` contract on a
    /// term that is not a predicate. [`refinement_hash`] cannot serve: it installs the
    /// term through [`Refinement::born`], which rejects the statement shapes.
    fn term_hash(e: &TypedExpr) -> u64 {
        use std::hash::Hasher;
        let mut h = std::collections::hash_map::DefaultHasher::new();
    ...
    fn a_keyed_write_under_a_transaction_block_compares_its_key() {
        let build = |acc: &str, item: &str, key: i64| {
            ...
            TypedExpr::mut_decl(
                ...
                TypedExpr::begin(TypedExpr::for_loop(
                    item.clone(),
                    TypedExpr::var(Name::raw("xs")),
                    TypedExpr::mut_write_keyed(
                        acc,
                        TypedExpr::lit(Lit::Int(key)),
                        TypedExpr::var(item),
                    ),
                )),
            )
        };
        let (a, b) = (build("total", "x", 1), build("sum__2", "y", 1));
        assert!(
            eq_term_modulo_ty_slots(&a, &b),
            "\u{3b1}-variants through a `Begin` and a keyed write:\n  {}\n  {}",
            symbolic::symbolic(&a),
            symbolic::symbolic(&b),
        );
        assert_eq!(
        ...
        // The key is half of what a keyed write says, so two writes at different keys are
        // two terms.
        assert!(!eq_term_modulo_ty_slots(&a, &build("total", "x", 2)));
    }

    /// A statement shape reaching a predicate is reported where the predicate is
    /// installed, so the pass that built it is named.
    #[test]
    #[cfg(debug_assertions)]
    #[should_panic(expected = "which is a statement shape")]
    fn a_statement_shape_in_a_predicate_is_reported_at_the_boundary() {
        let offending = TypedExpr::for_loop(
            Name::raw("x"),
            TypedExpr::var(Name::raw("xs")),
            TypedExpr::mut_write(Name::raw("acc"), TypedExpr::lit(Lit::Int(1))),
        );
        let _ = Refinement::born(Rc::new(offending));
    }

    /// And one refinement deeper, which is as deep as [`eq_term_modulo_ty_slots`] compares:
    /// a cast's target predicate. The nested refinement is a struct literal, the one
    /// construction that does not pass the boundary itself, so the outer install is what
    /// has to find it.
    #[test]
    #[cfg(debug_assertions)]
    #[should_panic(expected = "which is a statement shape")]
    fn a_statement_shape_under_a_cast_target_is_reported_too() {
        let offending = Rc::new(TypedExpr::for_loop(
            Name::raw("x"),
            TypedExpr::var(Name::raw("xs")),
            TypedExpr::mut_write(Name::raw("acc"), TypedExpr::lit(Lit::Int(1))),
        ));
        ...
        let cast = TypedExpr::cast(TypedExpr::var("xs"), target);
        let _ = Refinement::born(Rc::new(cast));
    }

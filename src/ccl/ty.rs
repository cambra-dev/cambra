...
/// sum — a constructor, a coalesced result, an annotation — is [`Closed`](Self::Closed):
/// it knows its own tags. Only a consumer's expectation is open, so `Open` appears on
/// the right of a subtyping edge and nowhere else, and coalesce materializes closed.
/// It is inference-only: nothing downstream of the solver, and nothing in the runtime
/// [`Extent`](crate::interpreter::Extent), can observe it.
        ...
        match self {
            ...
            Type::Variant(tags, openness) => {
                ...
                if let Some(payloads) = synthetic_payloads(tags) {
                    ...
                    // payload is the nullary constructor and renders bare. An
                    // **open** arm set (the demand a `match` with a default arm
                    // makes) renders a trailing `| …`, so a demand never reads as
                    // an exact sum in a diagnostic.

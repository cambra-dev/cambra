...
/// Returns `true` if `name` is written by a store mutation (`MutWrite`) anywhere
/// inside `expr`. A store write advances the variable's history, so an alias
/// `let y = x` may not be substituted past one (the read would move to the wrong
/// position) — the store-era complement to [`is_let_bound`]'s lexical-rebind
...
fn inline_impl(expr: Expr) -> Expr {
    ...
    let new_node = match node {
        TypedExprNode::Let {
            ...
            // store write (`x += …`, a `MutWrite`), which advances `x`'s history
            ...
            // store write is a `MutWrite`, not a `let`, so `is_let_bound` alone

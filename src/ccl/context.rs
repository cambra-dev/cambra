...
/// [`DataSink`](crate::interpreter::DataSink)s such as `http_serve`'s response
/// channel.
...
pub struct CompiledProgram {
    ...
    /// one entry per output name for sink programs, in declaration order).
    ...
    for (name, mut op) in per_field_ops {
        ...
        if name == "main" {
            ...
            let main_consumer = main_consumer
                .take()
                .expect("multiple `main` outputs in the program's output list");

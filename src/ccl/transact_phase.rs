...
//!    `channelize`, rewritten to an `AsOf` live read by [`rewrite_live_reads`]
...
use crate::ccl::{
    BaseType, Builtin, Expr, F_COMMIT, F_DECISION, F_TIME, F_WRITE_TARGETS, F_WRITES, HistoryKind,
    Lit, Name, ProjKey, TransactWriter, Type, TypedBinding, TypedExprNode,
    ccl_utils::is_free_in_value, letrec::check_letrec_guarded, letrec_phase::hoist_feeds,
};

/// Recognize a **live cross-endpoint read** and rewrite it to an as-of join,
/// *before* lambda elimination. Run after `channelize`.
///
...
fn strip(expr: Expr, txn_stores: &HashSet<Name>, sites: &mut Vec<RawSite>) -> Expr {
...
pub fn check_no_nested_transactions(expr: &Expr, txn_stores: &HashSet<Name>) -> Result<(), String> {
    fn has_txn_for(e: &Expr, txn_stores: &HashSet<Name>) -> bool {

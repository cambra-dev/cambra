...
///
/// After defer-desugaring, a read-only reply is a chain of store reads feeding a
/// broadcast over a reading loop:
/// `let k₁ = last_or_default((store.f₁, _)) in … let kₙ = … in trigger ≫ (λ r → e)`,
/// where `e` reads the `kᵢ` and `store` is a commit log (`Txn`, a non-enumerable
/// domain). Every such read is an **as-of read at an arbitrary commit position** —
/// the reading transaction sees the store as of where it lands in the commit
...
/// `Txn` register has no last-value term (a future `await_last` would be it). The
...
/// One live-store read in a reply chain: its `let` binder, the history-binding
/// reference (the as-of source for a single-register read — recognition later
/// rewrites it to a store-record projection), the store-record field its
...
fn as_live_read(expr: &Expr) -> Option<Expr> {
    // Walk consecutive `let kᵢ = last_or_default((⟨histᵢ⟩, _))` bindings —
    // each a bare reference to a live history binding — down to the broadcast
    // body. Pre-recognition there is no shared store record: several
    // registers are several history bindings; the snapshot case rebuilds
    // their record below and recognition collapses it onto the store.
    ...
    {
        let Some((store_read, field)) = live_store_read(bound_expr) else {
            break;
        };
        reads.push(LiveRead {
    ...
    // position** — the store's value as of wherever the reading transaction lands
    ...
    // read (a program cannot request the store's last value — a future
    // `await_last` builtin would, but does not exist). So no finiteness or
    ...
    let used: Vec<&LiveRead> = reads
        ...
        .collect();
    // A reply that also reads the trigger element `r` (`e = f(r, store)`) is a
    // function of *both* the request and the store: `zip((trigger, as_of)) ≫ (λ p
    // → e[r ↦ p.0, kᵢ ↦ p.1])` — the request rides alongside the store snapshot.
...
/// A live read whose reply combines the **request element** with the store
/// read(s): `zip((trigger, as_of((trigger, source)))) ≫ (λ p → e[r ↦ p.0, kᵢ ↦
/// p.1(.fᵢ)])`. The `zip` pairs each request with its store snapshot; the reply
...
/// (store-only replies), the request element survives into the reply. The `as_of`
...
fn build_zip_read(
    ...
    // The as-of source and the store-snapshot value type: one register reads bare,
    // several fold into a record (as in `build_snapshot`).
    let (source, snap_ty) = if used.len() == 1 {
        (used[0].store_read.clone(), used[0].value_ty.clone())
    } else {
        ...
        let source_ty = Type::Record(
            used.iter()
                .map(|r| (r.field.clone(), r.store_read.ty.clone()))
                .collect(),
        );
        let source = Expr::new(TypedExprNode::Record(
            used.iter()
                .map(|r| (r.field.clone(), r.store_read.clone()))
                .collect(),
        ))
        .with_ty(source_ty);
        (source, record_ty)
...
/// Match `last_or_default((⟨hist⟩, _))` over a **live** store history — a bare
/// reference to a `Txn`-domained letrec history binding — returning the read
/// and the store-record field its history will occupy. `None` for a
/// non-matching bound expression or a batch (enumerable-domain) store.
fn live_store_read(bound_expr: &Expr) -> Option<(&Expr, String)> {
    ...
    let TypedExprNode::Var(hist) = &store_read.node else {
        return None;
    };
    Some((store_read, hist.field_key()))
}
...
/// record *literal* of the history-binding reads (the shared store record
/// does not exist pre-recognition); recognition rewrites each field to
/// `__store.f` and then collapses the literal onto the store variable itself,
/// so the engine latches one whole-store snapshot per request.
fn build_snapshot(
    trigger: &Expr,
    used: &[&LiveRead],
    lam_body: &Expr,
...
/// snapshot reply, and a reply that combines the **request element with a store
/// read** (`resp << store + req`, via a `zip(trigger, as_of)` read) into as-of
...
/// **hang-guard**. An *unrecognized* surviving `last_or_default` read falls
        ...
        && contains_live_trigger(body)
    {
        return Err(
            "unsupported live cross-endpoint read: a reply reads a transactional register as of \
             ...
             one register (`resp << store`, or computed `resp << store + 1`), several at one \
             snapshot (`resp << a + b`), and the request combined with a register \
             (`resp << store + req`). Restructure the reply to one of these."
                .to_string(),
        );
    }
    let mut result = Ok(());
...
pub fn collect_txn_stores(expr: &Expr) -> HashSet<Name> {
...
fn store_value_ty(ty: &Type) -> Type {
    fn under_mut(ty: &Type) -> Option<&Type> {
        match ty {
            // Only a mutable store peels to its value; a feed history reads as
...
pub fn run(expr: Expr, txn_stores: &HashSet<Name>) -> Expr {
    // Strip whenever a `with begin():` block is present. A block need not write a
    // transactional register — a read-only block (`out << store`) has no write
    // yet must still be unwrapped off the loop spine — so we cannot short-circuit
    // on `txn_stores` alone; but with neither a register nor a block there is
    // nothing to do.
    if txn_stores.is_empty() && !contains_begin(&expr) {
        return expr;
    }
    let mut sites: Vec<RawSite> = Vec::new();
    let stripped = strip(expr, txn_stores, None, &mut sites);
    ...
    // (`check_store_write_context`) should have rejected, and must never
...
/// - `final_var` — the `let`-bound final value (`last_or_default` over the
...
/// Every non-transactional-store `Var` a commit-decision block reads — the
...
fn cross_domain_reads(sites: &[RawSite], txn_stores: &HashSet<Name>) -> HashSet<Name> {
    fn collect(e: &Expr, txn_stores: &HashSet<Name>, out: &mut HashSet<Name>) {
        if let TypedExprNode::Var(n) = &e.node
            && !txn_stores.contains(n)
        {
            out.insert(n.clone());
        }
        e.walk_children(|c| collect(c, txn_stores, out));
    }
    let mut names = HashSet::new();
    for s in sites {
        collect(&s.block, txn_stores, &mut names);
    }
    names
}
...
/// is not a transactional store (including induction writes lifted from inside a
...
fn loop_induction_writes(body: &Expr, txn_stores: &HashSet<Name>) -> HashSet<Name> {
    fn go(e: &Expr, txn_stores: &HashSet<Name>, out: &mut HashSet<Name>) {
        if let TypedExprNode::MutWrite { name, .. } = &e.node
            && !txn_stores.contains(name)
        {
            out.insert(name.clone());
        }
        e.walk_children(|c| go(c, txn_stores, out));
    }
    let mut out = HashSet::new();
    go(body, txn_stores, &mut out);
    out
}
...
fn contains_txn_write(expr: &Expr, txn_stores: &HashSet<Name>) -> bool {
...
fn strip(
        ...
        && matches!(&effect.node, TypedExprNode::Begin { .. })
    {
        ...
        if block_writes_txn(&block, txn_stores) {
            ...
            let (txn_block, lifted) = partition_block(*block, txn_stores);
            let (read_keys, write_keys) = collect_footprint(&txn_block, txn_stores);
            sites.push(RawSite {
                target: target.clone(),
                source: source.clone(),
                block: txn_block,
    ...
    if matches!(&expr.node, TypedExprNode::For { .. }) {
        ...
        let source = strip(*iter, txn_stores, enclosing, sites);
        ...
        let enclosing_writes = loop_induction_writes(&body, txn_stores);
        let body = strip(
            *body,
            txn_stores,
    ...
    let mut expr = expr;
    expr.map_children(|c| strip(c, txn_stores, enclosing, sites));
    expr
}
...
/// Partition a writing block by store domain: remove each induction `MutWrite`
/// (a target *not* in `txn_stores`) from the block spine and return it in the
...
/// accumulator out of the atomic commit. Only top-level spine writes are lifted;
/// a guarded/nested induction write, or an induction write reading a
/// transactional value, is out of scope (cross-domain sequencing is a later
/// stage) and stays in the block, where it surfaces downstream if unsupported.
fn partition_block(block: Expr, txn_stores: &HashSet<Name>) -> (Expr, Vec<Expr>) {
    let mut lifted = Vec::new();
    let txn_block = partition_spine(block, txn_stores, &mut lifted);
    (txn_block, lifted)
}

fn partition_spine(expr: Expr, txn_stores: &HashSet<Name>, lifted: &mut Vec<Expr>) -> Expr {
    let Expr {
        node,
        ty,
        user_annotation,
    ...
    match node {
        TypedExprNode::ExprStmt { expr: effect, body } if matches!(&effect.node, TypedExprNode::MutWrite { name, .. } if !txn_stores.contains(name)) =>
        {
            lifted.push(*effect);
            partition_spine(*body, txn_stores, lifted)
        }
        TypedExprNode::ExprStmt { expr: effect, body } => {
            let body = partition_spine(*body, txn_stores, lifted);
            Expr {
                node: TypedExprNode::ExprStmt {
                    expr: effect,
                    body: Box::new(body),
...
/// an induction store (`Mut[…]`, non-`Txn`) but no transactional register.
///
...
/// so the `with begin():` is either a misuse (the user believes the store is
...
/// domain. Store-ness is a type, so this cannot be caught at lowering; it is
...
pub fn check_no_induction_only_transactions(
    ...
    if let TypedExprNode::Begin { body } = &expr.node
        ...
        && let Some(name) = first_non_txn_write(body, txn_stores)
    {
    ...
    expr.walk_children(|c| {
        if result.is_ok() {
            result = check_no_induction_only_transactions(c, txn_stores);
        }
    });
    result
}

/// The first `MutWrite` target in a block that is *not* a transactional store,
/// in first-occurrence order — the induction store to name in the error above.
fn first_non_txn_write(block: &Expr, txn_stores: &HashSet<Name>) -> Option<Name> {
    ...
    block.walk_children(|c| {
        if found.is_none() {
            found = first_non_txn_write(c, txn_stores);
        }
    });
    found
}

/// Whether a block (a `Begin` body) writes any transactional store — marks it a
/// committing transaction site rather than a read-only block.
fn block_writes_txn(block: &Expr, txn_stores: &HashSet<Name>) -> bool {
    if let TypedExprNode::MutWrite { name, .. } = &block.node
        && txn_stores.contains(name)
    {
        return true;
...
fn collect_footprint(block: &Expr, txn_stores: &HashSet<Name>) -> (Vec<Name>, Vec<Name>) {
    let mut reads = Vec::new();
    let mut writes = Vec::new();
    fn walk(e: &Expr, txn_stores: &HashSet<Name>, reads: &mut Vec<Name>, writes: &mut Vec<Name>) {
        match &e.node {
...
fn build_writer(
    ...
    //    to the `last_or_default` `final_var` (in scope via `cross.reads`), a
    ...
    // keys (transactional stores `collect_footprint` recorded). `walk_block`
        ...
        TypedExprNode::ExprStmt { expr, body } => {
            match &expr.node {
                TypedExprNode::MutWrite { name, value } => {
                    ...
                    // target a transactional store the site records as a write
                    ...
                    // failure for a DB substrate. The lowering gate
                    // (`transactions::write_or_let`) already rejects a non-txn
                    // `:=`/`+=` inside a block; assert that invariant here too.
                    debug_assert!(

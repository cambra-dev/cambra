...
fn elim_lambda_impl(
    ...
    let result = match body_node {
            ...
            if matches!(value.node, TypedExprNode::Lambda { .. }) =>
        {
            ...
            // the refinement onto the inner lambda's binder — `cast(λ 𝑦 : 𝐷 → body,
            // {𝐷 | 𝑝} ⤇ 𝑉)` and `λ 𝑦 : {𝐷 | 𝑝} → body` are the same function — and
            // hand the result to the nested-lambda rule, which carries the binder's
            // type into the curried morphism. The refinement ends up on the inner
            // collection's domain, under the Pi that binds `param`, and planning
            // materializes it there as a `map_filter`.
            //
            ...
            if is_free(param, &value) {
                ...
                let refined = Expr::new(TypedExprNode::Lambda {
                    param: crate::ccl::expr::TypedBinding {
                        ty: target.domain().unwrap_or_else(|| inner_binding.ty.clone()),
                        ..inner_binding
                    },
                    body: inner_body,
                })

...
/// return its mutable type `Mut[V, D]`. Returns `None` for any non-`Mut`
...
/// generalized per call site): the mutable variable's domain is the loop it accumulates
/// over, resolved by the unified phase per call site.
fn mut_param_history_type(param: &Param) -> Option<Result<Type, LoweringError>> {
...
pub(super) fn lower_function_body(
    ...
    // lowers. Mutability of a `Mut[…]` parameter is its `Type::History` (stamped

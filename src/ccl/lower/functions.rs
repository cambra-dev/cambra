...
pub(super) fn uncurry_params(
    ...
    if params.len() == 1 {
        ...
        let mut lam = with_binder_site(
            Expr::lambda(params[0].name.as_str(), param_ty.clone(), body_expr),
            params[0].name_span,
        );
        ...
        .any(|p| mut_param_history_type(p, ctx).is_some())
    {
        return params.iter().rev().try_fold(body_expr, |acc, param| {
            ...
            // A curried parameter stays a named binder, so it keeps its own
            // site like a single parameter does.
            let mut lam = with_binder_site(
                Expr::lambda(param.name.as_str(), param_ty.clone(), acc),
                param.name_span,
            );

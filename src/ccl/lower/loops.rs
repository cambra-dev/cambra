...
pub(super) fn lower_direct_mirror_loop(
    ...
    let chain = ctx.with_shadowed([iter_var.clone()], |ctx| {
        lower_loop_body_chain(body_stmts, acc_names, yield_defer, false, ctx)
    })?;
...
fn lower_loop_body_chain(
    ...
    for stmt in body_stmts.iter().rev() {
        chain = match &stmt.node {
            ...
            // `design/mutability.md` §"Value-selecting Case and conditional
            // induction writes".
            ChlStmt::If {
                branches,
                else_body,
...
fn build_loop_if_case(
    ...
    for br in branches {
        ...
        let body = lower_loop_body_chain(&br.body, acc_names, yield_defer, true, ctx)?;
        out.push(Branch {
            pattern: None,
            guard,
            body,
    ...
    let else_chain = match else_body {
        Some(eb) => lower_loop_body_chain(eb, acc_names, yield_defer, true, ctx)?,
        None => Expr::lit(Lit::Unit),
    };
    out.push(Branch {
        pattern: None,

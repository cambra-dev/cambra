...
/// as `for x: bump(cnt)`, whose only possible effect is a hidden store write
...
pub(super) fn lower_generator_for(
    ...
    if for_body_has_yield(body) {
        ...
        // phase (src/ccl/design-mut-txn-feed.md) builds the recurrence and
        // hoists the yield-feed to an ordinary feed of the loop's history.
        // Detect loop-carried writes (`:=` / `+=` to an outer-scope name) so we
        // can dispatch on them; store-ness of each write is checked
...
/// update. To mutate a store, introduce it with `:=` before the loop and write
/// it with `:=` / `+=`.
    ...
    LoweringError::unsupported(
        ...
        format!(
            "assignment to `{name}` is mutation: `{name}` is bound outside the \
             ...
             immutably; to mutate a store, introduce it with `:=` before the \
...
/// an immutable binding, never a store write — a loop body's `x = e` is a
...
/// name — the author must use `:=` to write a store.) `<<=` is a
...
///
/// `=` binds immutably, so such a write is not a store mutation — it is a
...
/// unified letrec phase (src/ccl/design-mut-txn-feed.md).
///
...
pub(super) fn lower_direct_mirror_loop(
    ...
    for stmt in body_stmts.iter().rev() {
        chain = match &stmt.node {
            // `x = value` — a plain immutable binding. Inside a loop body it is
            // a per-iteration shadowing `let`, *never* a store write: `=` is not
            // a mutation operator (accumulators are written with `:=` / `+=`).
            ...
            // `x := value` — a mutable write inside the loop body. A write to an
            // accumulator is a `MutWrite` (the recurrence the phase threads); a
            // `:=` to any other name is a per-iteration local `let`. An annotated
            // `:=` intro inside the loop is rejected, like the `Mut[…]`
            // declaration below — accumulators are declared before the loop.
            ChlStmt::MutAssign {
                target,
                annotation,
                value,
...
running_totals";
        ...
        // guarded `LetRec` and hoists the yield-feed (src/ccl/design-mut-txn-feed.md).
        assert!(
    ...
    /// iterations. The error points at `:=` (introduce a store to mutate).
    #[test]

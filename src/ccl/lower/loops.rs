...
/// Rejection for a **register introduced inside a for-loop body**, at any
/// spelling: `x := e`, `x: T := e`, or `x: Mut(V) = e`.
///
/// The register would have to be scoped to one iteration, and its sequencing
...
/// annotation says nothing about whether it introduces a register, so gating on
/// the annotation accepted `x := e` and rejected `x: Mut(V) := e` for the same
/// construct.
fn in_loop_register_error(span: Span, name: &str) -> LoweringError {
    LoweringError::unsupported(
        span,
        format!(
...
fn lower_loop_body_chain(
    ...
    for stmt in body_stmts.iter().rev() {
        chain = match &stmt.node {
            ...
            // recurrence.
            //
            // A `:=` naming anything *else* introduces a register, and a
            // register declared inside a loop body is not supported at any

...
impl LoweringContext {
    ...
    /// Every `http_serve` route this context knows, as
    /// `(source name, sink, port, method, path)`.
    pub fn registered_routes(
        &self,
    ) -> impl Iterator<Item = (&str, &Arc<dyn DataSink>, u16, &str, &str)> {

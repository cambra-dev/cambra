...
/// [`lower_tx_block`]). The block is lowered with `in_tx_body = true` (bare store
/// reads are snapshots).
///
...
fn lower_tx_block_inner(
    ...
    // src/ccl/design/mutability.md "Implementation status"), reject a second
...
/// `Let`). Store-ness carries no lowering registry, so the target is *not*
...
/// (the two run on independent domains). A write to a genuine non-store surfaces

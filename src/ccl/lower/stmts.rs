...
/// outside those inner bindings, making its names unbound.
///
/// The `ExprStmt` node "drives the feed": [`crate::ccl::channelize`] collapses
/// it to its body once it has extracted all `Feed` nodes, leaving a clean
/// `Let* Record{…}` shape that `compile_program` compiles one output at a
/// time.
...
pub(super) fn lower_middle_stmt(
    ...
    match &stmt.node {
        ...
        ChlStmt::LoadFrom {
            ...
            // A load binds an ordinary `let`, so a `Mut(…)` annotation on one is a
            // contradiction rather than an unknown type. Named here for the same reason
            // `x: Mut(…) = e` is: falling through to `lower_type_annotation` reports
            // `Mut` as a type application nothing defines, which is wrong about a type
            // constructor the language has.
            if mut_annotation_parts(&annotation.ty, ctx).is_some() {
                return Err(LoweringError::unsupported(
                    stmt.span,
                    format!(
                        "`{name}: Mut(…)` under `@LoadFrom` declares a mutable, but a load \
                         binds a plain value; annotate it with the value type (e.g. \
                         `{name}: V`) and introduce a mutable from it with `:=`"
                    ),
                ));
            }
            let annotation_ty = lower_type_annotation(annotation, ctx)?;

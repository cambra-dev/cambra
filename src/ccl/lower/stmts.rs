...
pub(super) fn lower_stmts_inner(
    ...
    // A nested block is its own scope: snapshot the transactional registry so a
    // `Mut[…, Txn]` register declared local to this block reverts on exit and
    // does not leak into a sibling scope. Induction store-ness carries no
    // lowering-time registry (it is the `Type::Mut` on the binding, checked
    // post-inference), so only the transactional set is scoped here. Restored on
    ...
    // innermost-first. (Store-ness is carried by `Type::Mut` and checked
...
pub(super) fn lower_middle_stmt(
    ...
    match &stmt.node {
        ...
        ChlStmt::MutAssign {
            ...
            // the unified phase resolves. See src/ccl/design-mut-txn-feed.md.
        ...
        // shared commit store (see src/ccl/design-mut-txn-feed.md).
...
/// their store-ness is the `Type::Mut` on the binding, checked post-inference —
...
pub(super) fn pre_register_txn_decls(stmts: &[Spanned<ChlStmt>], ctx: &mut LoweringContext) {
    for stmt in stmts {
        match &stmt.node {
            ...
            // the name here so [`lower_call`] picks the curried shape.

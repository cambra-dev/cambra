...
/// The canonical 0-based tag position of `Commit` in the decision variant
/// `[.Commit(𝑃) | .Abort]`. [`decision_variant_ty`] builds the variant with
/// `Commit` first, and the runtime decode (`commit_operator::COMMIT_TAG`) reads
/// the union by this position. It is a *positional* convention (tag names are
/// erased at op-conversion), so it deliberately does **not** follow the
/// BTreeMap tag order inference materializes inferred variants in
/// (`Abort` < `Commit`) — asserted wherever a decision type is turned into an
/// index, so a coalesced-in-sorted-order decision variant fails loudly.
pub(crate) const COMMIT_TAG_INDEX: usize = 0;
...
/// The 0-based tag index of `tag` in a decision variant type (peeling outer
/// refinements). Panics if the type is not a variant carrying that tag.
fn variant_tag_index(decision_ty: &Type, tag: &str) -> usize {
    ...
    match t {
        Type::Variant(tags) => tags
            .iter()
            .position(|(k, _)| matches!(k, FieldKey::Name(n) if n == tag))
            .unwrap_or_else(|| panic!("decision variant lacks a `{tag}` tag: {decision_ty}")),
...
pub fn commit_project(decision_ty: &Type) -> Expr {
    let idx = variant_tag_index(decision_ty, V_COMMIT);
    // `decision_ty` may be a coalesced history type; guard the positional coupling
    // the runtime decode relies on (see [`COMMIT_TAG_INDEX`]) — a decision variant
    // materialized in BTreeMap tag order would place `Commit` at 1 and silently
    // desync the read from `commit_operator::COMMIT_TAG`.
    debug_assert_eq!(
        idx, COMMIT_TAG_INDEX,
        "decision variant must place `Commit` at the canonical tag index"
    );
    Expr::builtin(Builtin::VariantProject(idx)).with_ty(Type::fun(
        decision_ty.clone(),

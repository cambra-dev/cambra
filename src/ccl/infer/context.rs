...
///   however it is spelled. A `groupby` lowers to a `Lambda`, so the node test
///   alone let it generalize, and each use then rebuilt the whole grouping —
///   the duplication this predicate exists to avoid, reached through the one
///   collection that happens to be written as a function.

...
impl Typing for CheckCtx {
    ...
    fn as_function(
        ...
        // value it wraps: a defer's `Feed` to its channel, and a `Mut` store to
        ...
        loop {
            peeled = match peeled {
                Type::Feed(payload) => peel_refinements_outer(payload),
                Type::Mut { value, .. } => peel_refinements_outer(value),
                _ => break,
            };
        }
        match peeled {

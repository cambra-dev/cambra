...
use crate::ccl::{Expr, InferVarId, Name, Type, TypedBinding, TypedExprNode};
...
pub struct TypeInferenceContext {
    ...
    /// Monomorphization provenance remap produced by the last [`infer`] run:
    /// `(fresh_clone_id, source_clone_id)` pairs for every node a
    /// specialization cloned (see `infer::specialize_use` / S4).
    /// Inference stays ignorant of the `ProvenanceTable`; it only surfaces
    /// these `NodeId` pairs, and `compile_program` drains them via
    /// [`take_mono_remap`](Self::take_mono_remap) to tag each clone
    /// `Derived { via: Mono }`.
    mono_remap: Vec<(
    ...
    /// is the blame node for error `i`, `None` when unknown). Mirrors
    /// [`mono_remap`](Self::mono_remap): inference stays ignorant of the
    /// `ProvenanceTable`; it only surfaces these ids, and `compile_program`
    /// drains them via [`take_infer_error_nodes`](Self::take_infer_error_nodes)
    /// to resolve each to a source span while the table is in scope. This is
    /// the side-channel that lets the public [`infer`] keep its
...
impl TypeInferenceContext {
    ...
    /// Drain the monomorphization provenance remap accumulated by the most
    /// recent [`infer`] run. See [`mono_remap`](TypeInferenceContext::mono_remap);
    /// `compile_program` calls this once after inference to apply the remap to
    /// the provenance table.
    pub fn take_mono_remap(
        &mut self,
    ) -> Vec<(
        crate::ccl::provenance::NodeId,
        crate::ccl::provenance::NodeId,
    )> {
        std::mem::take(&mut self.mono_remap)
    }
...
pub enum InferError {
    ...
    /// A [`Type::Mut`]-typed expression appears in a value position but is not
    /// a bare variable reference — the second-class discipline's rule 1 (see
    /// `src/ccl/design-mut-txn-feed.md`, "No aliasing"). A mutable value must
    ...
    /// A [`Type::Mut`] appears nested inside a composite type — the
    ...
    /// An *unannotated* binding was inferred to have [`Type::Mut`] type — the
    ...
    /// (peeled) type is not [`Type::Mut`]. The second-class discipline's rule 1:
    ...
    /// [`Type::Mut`] — a write to something that was never introduced as a
...
/// source [`Span`](crate::chl_parser::ast::Span) via the `ProvenanceTable`
...
/// may still carry `Type::Feed` types with `Type::Infer` channel domains
...
pub fn infer(expr: &mut Expr, ctx: &mut TypeInferenceContext) -> Result<Type, Vec<InferError>> {
    ...
    match super::run(expr, &ctx.source_types) {
        Ok((ty, mono_remap)) => {
            // Surface the monomorphization remap to `compile_program` (drained
            // via `take_mono_remap`). Inference itself never touches the
            // `ProvenanceTable`; it only hands back these `NodeId` pairs.
            ctx.mono_remap = mono_remap;
            Ok(ty)
        }
        Err(located) => {
...
fn collect_type_errors(
    ...
    match ty {
        ...
        Type::Feed(payload) => {
            // A feed handle downstream of desugar is a compiler bug — desugar
            // erases every defer construct along with its feed type.
            if strictness == Strictness::Strict {
                errors.push(InferError::Unsupported(format!(
                    "feed handle type survived desugar_defers at `{context_sym}`"
                )));
            }
            collect_type_errors(payload, context_sym, strictness, errors, seen_refinements);
        }
        Type::Mut { value, domain } => {
            // A `Mut` reference at the strict wall is a compiler bug — the
            // unified phase (`transact_phase` / `letrec_phase`) rewrites every
            // mutable read/write and erases the `Mut` types before this point.
            if strictness == Strictness::Strict {
                errors.push(InferError::Unsupported(format!(
                    "mutable-reference type survived the unified phase at `{context_sym}`"
                )));
            }
...
/// `Define` node, **or** a transient [`Type::Feed`] / [`Type::Mut`] in any
/// reachable type slot — and hence the channel `Feed`/`Infer` (and `Mut`
/// `Infer`-domain) types the pre-desugar check tolerates.
///
...
fn has_pre_desugar_artifacts(expr: &Expr) -> bool {
    fn ty_has_transient(ty: &Type) -> bool {
        if matches!(ty, Type::Feed(_) | Type::Mut { .. }) {
            return true;
        }
        let mut found = false;
...
/// Strip outer [`Type::Refinement`] layers and, if a [`Type::Mut`] is
/// underneath, return its `(value, domain)` children.
///
/// Outer refinements are transparent to mutability: a witness a store's
/// reference acquired during solving does not change that it *is* a store.
/// Returns `None` for any non-`Mut` type.
fn peel_mut(ty: &Type) -> Option<(&Type, &Type)> {
    match ty {
        Type::Mut { value, domain } => Some((value, domain)),
...
/// `src/ccl/design-mut-txn-feed.md`, "No aliasing: `Mut` values are
...
fn check_no_nested_mut(
    ...
    match ty {
        ...
        // Tuple / Record / Variant / Feed payloads (and inert leaves): a `Mut`
        // anywhere below here is nested, so drop `allow_mut` to `false`.
...
mod tests {
    ...
    /// store-ness (see src/ccl/design-mut-txn-feed.md, "Store-ness is the type").
    #[test]

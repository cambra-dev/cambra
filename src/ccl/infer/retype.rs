...
//! # ⚠️ Temporary bridge — retired in the `channelize` follow-up PR
//!
//! **Do not invest in this pass; it is scaffolding.** `retype` exists only so
//! the feed foundation type-checks. After `desugar_defers` rewrites the feed
//! tree, some nodes still carry residual `Hole` / `Infer` types — most
//! importantly a bound lambda's *parameter domain*, which the main inference
//! pass leaves as a `Hole` and which this walk resolves from the lambda's
//! **call sites** (`collect_call_arg_types` + `specialize_lambda_domain`). So
//! it is a post-hoc *inference-completion* pass, not merely a feed re-typer —
//! which is why even defer-free programs (plain generators, higher-order UDFs)
//! depend on it here.
//!
//! The stacked `channelize` PR retires `retype` outright: it moves that
//! lambda-domain resolution **upstream** into lowering / inference, leaving the
//! feed pass type-preserving by construction (it then needs only the bounded,
//! solver-free `channelize::resolve_read_types` read-fixup — no re-synthesis
//! walk). This file is deleted there. Weigh any change here against the fact
//! that the whole pass is about to be removed.
//!
//! ---
//!
...
use crate::ccl::{BaseType, Expr, HistoryKind, Name, ProjKey, Type, TypedExprNode};
...
mod tests {
    ...
    fn feed(domain: Type, value: Type) -> Type {
        Type::History {
            value: Box::new(value),
    ...
    fn rebinds_feed_read_to_channel_type() {
        ...
        let read = TypedExpr::var("x").with_ty(feed(fresh_var(0), int()));
        let mut expr = TypedExpr::let_bind("x", channel, read);
        expr.ty = feed(fresh_var(0), int());
    ...
    fn reports_unbound_residue_var() {
        let mut expr = TypedExpr::var("ghost").with_ty(feed(fresh_var(0), int()));

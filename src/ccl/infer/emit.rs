...
pub(super) fn emit_let<C: Typing>(
    ...
    let scheme_ty = match &binding.user_annotation {
        // Only a *store* annotation (`x: Mut[V] = init`) binds the variable at
        // the history type and constrains `init <: V`. A `Feed`-kind history
        // annotation is deliberately excluded: it is not a store introduction,
        // so it falls through to the generic `Some(ann)` arm below (there is no
        // `Feed[…]` initializer surface today, but gating on `Store` keeps this
        ...
        Some(ann) => {
            ...
            // A non-store annotation on a *store* bound expression is a
            // **deref-copy** (`y: int = x`): bind `y` at the annotation's value
            // type, not the store type. Binding it at the store type makes `y` an
            ...
            // `Mut` alias (rule 3), and `y += 1` is *accepted* as a store write.
            ...
            // non-store bound expression annotation and inferred type agree, so
            // this stays `bound_ty`.
...
pub(super) fn emit_transact<C: Typing>(
    ...
    for w in writers.iter_mut() {
        ...
        // virtual store key the consumer reads as `__store.to_…`. Its stream

...
fn emit_node_inner(expr: &mut Expr, ctx: &mut InferCtx) -> Result<Type, LocatedInferError> {
    ...
    let mut ty = match &mut expr.node {
        ...
        TypedExprNode::MutWrite { name, value } => {
            ...
            // register rule; naming `V` here is the whole of it.
            let register_value = var_ty.as_register();
            // The *target* is a handle, resolved by name above. The written **value** is
            // an ordinary value operand, so a register mention there reads: `b := a`
            // between two registers writes `a`'s current value into `b`.
            let value_ty = read_operand(value, ctx)?;
...
fn complete_annotation(ann: &Type, inferred: &Type) -> Type {
    match (ann, inferred) {
        ...
        (Type::Refinement(base, claims), _) => Type::refined(
...
pub(super) fn emit_lambda<C: Typing>(
    ...
    // Deliberately *not* `read_operand`: a lambda's result is where rule 2 catches a
    ...
    let body_ty = match denoted_expr(body).ty.as_register() {
...
pub(super) fn emit_cast<C: Typing>(
    ...
    if let Type::Fun { domain, .. } = target
        && let Type::Refinement(_, claims) = domain.as_mut()
    {
        for r in claims.iter_mut() {
    ...
    let domain = match cast_target_refinement(target) {
        Some(claims) => Type::refined(d, claims),
        None => d,
    };
...
pub(super) fn emit_apply<C: Typing>(
    ...
    // **The one position where a register's handle survives.** If the parameter is a
    // register the argument is passed *by reference* and the handle must reach the
    ...
    // register is the parameter itself and never nested inside it, so there is no
    // composite to walk.
    let param_is_register = parameter_type(function, &fn_ty)
        .and_then(Type::as_register)
        .is_some();
    let arg_ty = if param_is_register {
        raw_arg_ty
    } else {
        deref_mut(&raw_arg_ty)
    };
...
pub(super) fn emit_binop<C: Typing>(
    ...
    let left_ty = read_operand(left, ctx)?;
    let right_ty = read_operand(right, ctx)?;
...
pub(super) fn emit_unary<C: Typing>(
    ...
    let inner_ty = read_operand(inner, ctx)?;
...
pub(super) fn emit_tuple<C: Typing>(
    ...
    for (i, e) in elts.iter_mut().enumerate() {
        ...
        fields.insert(FieldKey::Index(i), read_operand(e, ctx)?);
    }
    Ok(product(fields))
}
...
///
/// A tail position reports a value where a register is read — a program ending in a
...
///
/// Every tail is walked, [`TypedExprNode::MutDecl`] included: a register does not
...
/// Emit a **value operand**: a register mention here is a *read*, so its handle
/// derefs to the value it holds.
///
/// This is the ordinary case. A register's handle survives in exactly two positions —
...
/// `Mut(V) <: τ` reads as "a register is a subtype of its value", which is a coercion
...
fn read_operand<C: Typing>(e: &mut Expr, ctx: &mut C) -> Result<Type, LocatedInferError> {
    Ok(deref_mut(&ctx.subexpr(e)?))
}
...
pub(super) fn emit_feed<C: Typing>(
    ...
    // discipline forbids it), so a register mention here reads.
    //
...
pub(super) fn emit_aggregate<C: Typing>(
    ...
    let input_ty = read_operand(input, ctx)?;
...
/// `Mut` and a read derefs to `V` at the rule that emits it ([`read_operand`], not
/// the subtyping relation — see `src/ccl/design/mutability.md`, "A register read is
...
pub(super) fn emit_mut_decl<C: Typing>(
    ...
    let init_ty = ctx.in_let_rhs(|ctx| read_operand(init, ctx))?;
    ...
    let body_ty = ctx.scoped(&binding.name, &history, |ctx| read_operand(body, ctx))?;
...
pub(super) fn emit_for<C: Typing>(
    ...
    let iter_ty = read_operand(iter, ctx)?;
...
pub(super) fn emit_case<C: Typing>(
    ...
    if let Some(scrut) = scrutinee {
        let scrut_ty = read_operand(scrut, ctx)?;
...
fn emit_case_branch<C: Typing>(b: &mut Branch, ctx: &mut C) -> Result<Type, LocatedInferError> {
    let guard_ty = read_operand(&mut b.guard, ctx)?;
...
pub(super) fn emit_variant_ctor<C: Typing>(
    ...
    let payload_ty = read_operand(payload, ctx)?;
    let mut tags = BTreeMap::new();

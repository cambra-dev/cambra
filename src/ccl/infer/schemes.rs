...
pub struct OperatorSchemes {
    ...
    /// `∀ν. ((Txn → {time: Txn, write: ν}), Txn, ν) → ν` — the write carried
    ...
    /// Only the value `ν` is quantified; the history domain, the `time` field,
    /// and the position are all the concrete commit-time domain `Txn`. `ν` is
    /// shared across the `write` field, the default, and the result, so this is
    /// inline-built like `get_prev_seq`.
...
impl OperatorSchemes {
    ...
    pub fn new() -> Self {
        ...
        // GetPrevTxn: ∀ν. ((Txn → {time: Txn, write: ν}), Txn, ν) → ν. The
        // history stream and position are the concrete `Txn` domain; only the
        // carried write value `ν` is polymorphic, shared across the record's
        // `write` field, the default, and the result — hence inline-built.
        let nu = fresh_var(BODY_LEVEL);
        ...
        let mut tup: BTreeMap<FieldKey, Type> = BTreeMap::new();
        tup.insert(FieldKey::Index(0), fun(Type::Txn, commit_record));
...
mod tests {
    ...
    /// `((Txn ⇒ {time: Txn, write: ν}), Txn, ν) ⇒ ν`: the history domain, the
    /// `time` field, and the position are all the concrete `Txn` domain, and
    /// the carried write value / default / result share one variable `ν`.
    #[test]
    fn get_prev_txn_scheme_shape() {
        ...
        assert_eq!(**hist_dom, Type::Txn, "history domain is Txn");
        ...
        // the result all share the same freshened variable ν.

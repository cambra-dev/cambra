...
fn rename_witness(
    ...
    CompactType {
        ...
        var: ct.var.as_ref().map(|v| CompactVariant {
            tags: rename_map(&v.tags),
            openness: v.openness,
        }),
...
fn subst_witness(
    ...
    let mut out = CompactType {
        ...
        rec: ct
            ...
            .clone(),
        var: ct.var.as_ref().map(|v| CompactVariant {
            tags: subst_witness_keyed(&v.tags, binder, candidate),
            openness: v.openness,
        }),
        fun: ct.fun.as_ref().map(|f| CompactFun {
            name: f.name.clone(),
...
fn kind_of(a: &KindMerge, b: &KindMerge) -> KindMerge {
    use KindMerge::*;
    match (a, b) {
        // Nothing *required* a kind on one side, so the other side's answer stands.
        (Unknown, k) | (k, Unknown) => *k,
        (Data, Data) => Data,
        (Compute, Compute) => Compute,
        // The two kinds are unrelated by subtyping, so neither stands in for the other
        ...
        Type::Sigma(s) => {
            ...
            CompactType {
                ...
                sigma: Some(CompactSigma::closing(s.binder(), kind, {
                    ...
                    let body = compact_go(&s.body, pol, subst_acc, None, st);
                    if fresh {
        ...
        Type::Infer(state) => {
            ...
            let no_concrete = bound.as_ref().is_none_or(|b| {
                let CompactType {
                    ...
                    history_slot,
                    var,
                    // A sum and a witness are shapes: the first is what the position
                    // *is*, the second is the name a consumed sum gave it, and both
                    // determine the position as much as an atom does.
                    sigma,
                    witness,
                    // Not shape. All three are carried across the fallback explicitly
...
mod tests {
    ...
    /// **Factoring carries the candidate's Pi binder.** Splitting `Σ σ ∈ {(𝑘: 𝐷) ⤇ 𝑉[𝑘]}. σ`
    /// into the kind `{𝐷}` and the body `σ ⤇ 𝑉[𝑘]` orphans `𝑘` unless the rebuilt body
    /// declares it — and it declares it on the *witness* domain, `(𝑘: σ) ⤇ 𝑉[𝑘]`, which is
    /// what the binder meant: it ranges over elements of whichever domain the witness
    /// picked. A dropped binder is not reported as anything about sums; it surfaces as
    /// `𝑘` free in the body, which the escape check calls an out-of-scope binder.
    #[test]
    fn factoring_carries_the_candidates_pi_binder() {
        let dependent = CompactType {
            fun: Some(CompactFun {
                name: Some(Name::from("__gb_k")),
                kind: KindMerge::Data,
                domain: Box::new(CompactType::from_atom(AtomKey::UIntRange(3))),
                codomain: Box::new(CompactType::from_atom(AtomKey::Prim(BaseType::Int))),
            }),
            ..Default::default()
        };
        let sum = CompactSigma::closing(
            crate::ccl::infer_var::fresh_witness_binder_id(),
            CompactWitnessKind::enumerated(vec![dependent]),
            CompactType::default(),
        );
        let factored = sum
            .factored_view()
            .expect("a data-function candidate factors");
        assert_eq!(
            factored.body.fun.as_ref().and_then(|f| f.name.clone()),

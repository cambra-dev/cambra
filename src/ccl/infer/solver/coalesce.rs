...
///
/// **Only a collection is closed.** A sum binds the witness of the collection it *is*, so
/// the position being closed has to be one — a data function, whose domain is its data.
/// Everything else mentioning a witness is inside that collection's scope rather than
...
///
/// Stated on the *collection* rather than on the shapes to exclude: an index record is not
/// a special case of a bare index, and enumerating index shapes would have to grow with
/// every position a witness can occupy.

...
use crate::ccl::{
    ...
    TypedExprNode,
    ccl_utils::{COMMIT_SELECTOR, synthesize_arm_predicate, typed_compose},
...
fn splice_after_unit(chain: Expr, tail: Expr) -> Expr {
    match chain.node {
        ...
        TypedExprNode::Let {
            ...
            bound_expr,
            body,
        } => let_in(binding, *bound_expr, splice_after_unit(*body, tail)),
...
fn attach_feed_fields(decision: Expr, feeds: &[FeedSite]) -> Expr {
    ...
    match decision.node {
        TypedExprNode::Let {
            ...
            let mut e = let_in(binding, *bound_expr, new_body);
            e.ty = ty;
            e
        }
        TypedExprNode::Record(fields) => {
...
mod tests {
    ...
    fn recognition_builds_the_transact_carrier() {
        ...
        assert!(
            s.contains("variant_wrap(0)") && s.contains("writes:") && s.contains(".Abort"),

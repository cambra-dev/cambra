...
fn rewrite(mut expr: Expr) -> Expr {
    ...
    // A register introduction becomes an ordinary value binding. After
    // elimination the register *is* its seed: every read and write in the body
    // has been rewritten into the recurrence (inside a loop) or a shadowing
    // advance (sequentially), so nothing is left that needs the register to be
...
mod tests {
    ...
    /// A register's history rides the **binder's `ty`** — that is what a `MutDecl`
    ...
    fn phase_erases_mut_from_a_binder_ty() {
        ...
        assert!(
            ...
            "sanity: the register introduction is bound at its history"
        );
    ...
    fn direct_mirror_sum() -> (Expr, Name, Name) {
        ...
        init.ty = int.clone();
        // The accumulator is a register *introduction*, so it is a `MutDecl` bound
        ...
        // (`debug_assert_no_register_let`), and would leave the binder slot the

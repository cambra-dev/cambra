...
`Let` survives the whole pipeline, while a mutable variable introduction is compiled away.
And it is the **only** node that binds a mutable variable — a `Let` cannot, because
`emit_let` reads through an initializer that is a mutable variable, so `b = a` binds `b` at `a`'s value.
That is what makes mutability structural: a pass asks which node it is looking
...
Lowering ([`ccl_utils::make_cast`]) emits it for list-comprehension filters, for-loop `if`-guards, and `groupby`. Every `target` lowering produces today is `Fun(Refinement(_, 𝑝), _)` — a fact about those lowerings rather than a contract on `cast`, since only `lambda_elim`'s cast-wrapped-lambda arm requires the shape and it asserts so itself: a function type whose domain carries the predicate `𝑝`, so the cast attaches a refinement to a collection function's domain. `target` is the lowering-time *specification* (its domain/codomain are typically `Type::Hole`, carrying only the refinement); the resolved cast type lands on `expr.ty` after inference — the same `user_annotation`-vs-`ty` split used elsewhere.

`Cast` is an **upcast**: the refined-domain function it produces is a supertype of `value`. The refinement is attached *constructively* — `emit_cast` decomposes `value`'s type and re-wraps its domain with `𝑝` — rather than discharged as a `value_ty <: target` edge, because no such edge exists to discharge: the refinement lattice is strict (`unrefined ⊀ refined`), and the target lowering emits for a comprehension is a **data** function, whose domain is invariant rather than contravariant. A *covariant* refinement (casting `Int` to `{Int | 𝑝}`) correctly *fails* the check — acquiring a value-level refinement is a runtime/SMT-checked narrowing, not an upcast. How the solver discharges the refinement obligation — flowing the demanded refinement onto the target-domain variable and stacking it so chained casts compose — is covered in [type-inference.md](type-inference.md#45-dependent-refinements-via-pi-types).

`lambda_elim`, planning, and operator conversion carry the domain refinement through to a runtime `Restrict`: a `Cast` around a group-by lambda becomes a **Pi-const** form whose refinement planning's pointful group-by recognizer reads off the predicate, while a `Cast` wrapping point-free filter/guard code survives lambda elimination unchanged and is consumed as a domain refinement at planning — see [optimization.md](optimization.md). Every `Cast` lowering emits today carries a domain-refinement target, and the cast-wrapped-lambda arm of `lambda_elim` requires one and asserts so; `cast` itself accepts any target, and the direction for a general `𝑈 ⇒ 𝑇` cast is in [type-inference.md](type-inference.md#6-future-work).
...
CHL `match` needs no IR node of its own, and it did not need the `Let(__scrut)`-plus-equality-guards desugaring once sketched here. It *is* the `scrutinee: Some(_)` case: each `` case `tag(binder): `` arm becomes a `Branch` carrying a `Pattern { tag, binding }`, with `guard: Lit(true)`. That is what makes tag dispatch and the guarded `if`/`elif` chain the same first-match rule over one node, and it means a tag pattern and a boolean guard can eventually appear on the *same* arm (`` case `tag(x) if p: ``) with no new structure — the three `Branch` shapes (guard only, pattern only, both) are already spelled out in its docs. A trailing `pattern: None` branch in a scrutinee-`Case` is the default arm `case _:`. See [lowering.md](lowering.md), "Variants and match".
...
`Transact { keys, writers, domain, parameter }` denotes a **transactional mutable variable**: a set of scalar-variable `keys` sharing one sequencing `domain`, driven by concurrent `writers` that read the shared mutable variables and propose per-position writes. It denotes a *pure value* — the history record `{key: Fun(domain, V)}`, each field a key's history — so a variable read is the projection `__hist.key`; the mutable variable↔writer cycle is the operator's runtime behaviour, not the node's denotation (exactly as the induction/commit store realizes a recurrence). Each key carries its `init`, the seed: the value it holds before the domain's first position. An induction-domain carrier has exactly one writer (a `mut` loop, whose footprint is all its accumulators).

`parameter` is `None` for a top-level carrier, whose writers take the snapshot tuple alone. A **nested** carrier — an inner loop's — sets it to the `(enclosing, position)` pair `lambda_elim` merged the two loops' binders into, and is the top-level carrier lifted pointwise over that parameter `𝑃`: each `init` is `𝑃 ⇒ 𝑉`, so the inner loop seeds from wherever the enclosing one had got to, and each writer's body takes `(𝑃, (snap…, item))`. The source is curried instead, `enclosing ⤇ (domain ⤇ item)`, one collection per enclosing position. The carrier then denotes one history record per enclosing position, `enclosing ⇒ {key: Fun(domain, V)}`, and a variable read is the morphism `__hist ≫ .key` of the enclosing parameter, as every other value of the enclosing writer's point-free body is. Op-conversion realizes it as one store per enclosing row (see [*Induction stores as a changelog*](../../interpreter/design-operators.md#induction-stores-as-a-changelog-inductionstore-and-storedenseread)). The parameter is stated rather than rebuilt from `enclosing` and `domain` because the pair carries refinements neither component does. Symbolic rendering: `transact under 𝑃 (k = init, …) { … }`.
...
Rationale: planning post-elim retires the old pointful/point-free double representation (one `LetRec`, not a pointful group re-derived into a point-free one); the `Transact` carrier makes the mutable variable structure available to planning and op-conversion, and unifying loops and transactions on one node lets both domains share the one planning→conversion path.
...
- **The unified letrec phase** (`ccl/mut_elim.rs` for induction, `ccl/transact_phase.rs` for `Txn`; after inlining, before `channelize`) emits one `LetRec` per mutation loop / transaction: the accumulators (or per-key histories) become a history binding `𝐷 ⇒ {step, __to_<feed>*}` made causal by `get_prev_seq` (induction) or `get_prev_txn` (`Txn`), trailing reads become `final_or_default`, and each in-loop feed is hoisted to an ordinary `Feed(defer, __hist ▷ .__to_<feed>)`. `planning::plan_loops` then lowers each group onto the `Transact` carrier above, so planning and op-conversion run unchanged.
...
| `Type::History { value, domain, history_kind }` | Type checker only | The unified history handle: a collection `domain ⤇ value` plus a `HistoryKind` marker (`Overwrite` = mutable variable, displayed `Mut(value, domain)`; `Append` = deferred-output channel, displayed `feed(domain ⤇ value)`) | `Overwrite` by mut_elim (overwrite) / `transact_phase` (Txn); `Append` by `channelize` — both erased to a bare `Type::Fun` before `operator_conversion` |

...
`Transact` is **born in `letrec_phase::recognize`** (from the guarded `LetRec` the mutability phase emits — see below) and consumed at operator conversion, which **dispatches on `domain`**: a concrete iteration extent → the sequential `Recurse` (the induction case — one always-commit writer, all the pipeline emits today); `Type::Txn` → the concurrent commit operator (the transactional slice — a later PR on this stack). Recognition runs **after `lambda_elim`**, on the group's point-free normal form — it anchors on the guard builtins (`get_prev_seq` / `get_prev_txn` / `begin_<site>`), which survive elimination, so one `LetRec` travels from the mutability phase through `channelize` and `lambda_elim` and is recognized point-free; `Transact` is then recognition's *output* carrier from there to op-conversion (op-conversion is lambda-free, and planning stages the carrier's writer sources). There is no `Jump` node: `while` loops with explicit restart/break are future work.

Rationale: recognizing post-elim retires the old pointful/point-free double representation (one `LetRec`, not a pointful group re-derived into a point-free one); the `Transact` carrier makes the store structure available to planning and op-conversion, and unifying loops and transactions on one node lets both domains share the one recognition→conversion path.
...
- **The unified letrec phase** (`ccl/letrec_phase.rs` for induction, `ccl/transact_phase.rs` for `Txn`; after inlining, before `channelize`) emits one `LetRec` per mutation loop / transaction: the accumulators (or per-key histories) become a history binding `𝐷 ⇒ {step, to_<feed>*}` guarded by `get_prev_seq` (induction) or `get_prev_txn` (`Txn`), trailing reads become `last_or_default`, and each in-loop feed is hoisted to an ordinary `Feed(defer, __hist ▷ .to_<feed>)`. `letrec_phase::recognize` then lowers each group onto the `Transact` carrier above, so planning and op-conversion run unchanged.
...
| `Type::History { value, domain, kind }` | Type checker only | The unified store/feed handle: a function `domain ⇒ value` plus a `HistoryKind` marker (`Store` = mutable register, displayed `Mut[value, domain]`; `Feed` = deferred-output channel, displayed `feed(domain ⇒ value)`) | `Store` by the unified phase (`letrec_phase` / `transact_phase`); `Feed` by `channelize` — both erased to a bare `Type::Fun` before `operator_conversion` |
...
**`Type::History`** collapses what were once two separate variants (`Type::Mut` and `Type::Feed`) into one `domain ⇒ value` function distinguished only by a two-valued `HistoryKind` (`Store | Feed`) — see [mutability.md](mutability.md) §"Collapse `Mut` / `Feed` into one `History` variant". A store derefs on read to the scalar `value` (carry-forward terminal, scalar-last read); a feed reads as its whole stream `domain ⇒ value`. Both are erased before `operator_conversion`, so no history type survives to planning.

**`Type::ChanDom`** carries its introduction level (an identity-transparent `ChanLevel`) and freshens like a quantified variable, so a defer minted inside a generalized generator names a distinct channel per specialization. `channelize` closes the tree by the pure whole-tree substitution `erase_chan_domains` — the feed-side analog of the store phase's `erase_mut`.

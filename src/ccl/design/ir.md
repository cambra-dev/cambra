...
Why they cannot be one node with a mode flag: a flag would mean one node with **two typing rules**, and the rules are what differ. `emit_copair` builds a coproduct domain; `emit_disjoint_join` requires one shared domain. The disjointness precondition belongs to the second alone, and is checked where it is relied on (`flat_merge`). `is_index_partition_of` is then simply the second one's reconciliation — refined arms `{D | π̂ᵢ}` partitioning a declared `D` — rather than a patch over a declared/structural mismatch.

Why "union" was the wrong name for either: in type theory a *union type* is **untagged**, which neither of these is — one is a tagged sum, the other a merge of partial maps. Naming both `CollectionUnion` is what let op-conversion end up *guessing* which operation a node meant (from whether a fed input was present, a property that correlates with the split but is not it), even though every construction site already knew.

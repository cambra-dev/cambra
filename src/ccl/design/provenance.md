...
compare-chain operand freshens, inference's two freshens — a refinement
predicate per scheme instantiation, and a trait obligation's stored operand
expressions per obligation copy — and the guard a `Case` arm copies for the arms
below it to complement against. Each captured copy carries its own origin from the
...
`infer.branch_condition`), `infer/context`

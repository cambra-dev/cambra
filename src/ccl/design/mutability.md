...
- **`Txn` — the sharp case: commit order = *arrival* order, not program order.** Two `with begin():`
  ...
  *when the runtime observes its trigger*. A programmer's default assumption (lexical order) is wrong.
  ...
  snapshot). "Arrival order" here means the oracle is arrival-**monotone** (it never places a later
  arrival before an earlier one — the basis of the external-consistency guarantee in (5)); it does
  **not** mean a program may predict or rely on the relative order of two transactions it did not
  causally sequence itself. That stronger non-guarantee is the [deliberately-unordered design
  commitment](#reads): the runtime picks *a* commit order and a program may not assume one transaction
  serializes before another.
...
- **Reply-after-commit / cross-endpoint monotonicity.** A reply fed from *inside* a block (or data-
  ...
  another endpoint observes `≥ 2`. This holds *only* for the inside-block (commit-riding) reply. A
  read-only block's fed-out read is by contrast an **arbitrary as-of sample** (the
  [deliberately-unordered commitment](#reads)): it may observe an early position, so *freshness is
  not* among the guarantees — a definite latest-committed read awaits the future `await_last`.
...
- **`InductionStore` (+ `StoreDenseRead`)** — the induction loop. It drives the loop *sequentially
  ...
  changelog over the loop extent to the dense `𝐷 ⇀ 𝑉` stream (serving both a scalar-final

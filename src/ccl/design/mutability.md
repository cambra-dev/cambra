...
`Type::History { history_kind: Overwrite }` (displayed `Mut(𝑉, 𝐷)`), and is therefore known
...
`Type::History { value: 𝑉, domain: 𝐷, history_kind: HistoryKind::Overwrite }` (displayed `Mut(𝑉, 𝐷)`) — a
wrapper variant carried on the introduction's binding and on every reference to it. It shares the
`Type::History` variant with a feed channel, distinguished by `history_kind`; a mutable variable is the
...
- *`zip` conversion.* An arm of a `zip` that reads a mutable variable (`__cnt.acc`) is a **leaf** source over its own
  ...
  input (rather than fanning the shared iteration input into it, which it would reject);
  `zip_arms_at` then co-aligns the leaf accumulator stream with the input-driven request stream by
  domain position.
...
existing `zip`/`zip_arms_at` co-iteration plus a wider source decode.
...
**That check is per store and the rule is per variable.** `await_final(𝑥)` completes on 𝑥's own
writers, so a block writing only a store-mate of 𝑥 is well-founded and refused anyway: one store is
one recurrence, and `final_read` is not a causal accessor into it. Reaching the stated rule through
𝑥 itself is what `check_await_final_linearity` forbids first, so every program this check rejects is
one the stated rule permits. Narrowing it means splitting a store at a key's closure point; until
then the refusal is a compile error rather than a wrong answer
(`a_block_writing_only_a_read_only_store_mate_of_an_await_rejected`).
...
- **Data-typed selection** (`zs = xs if c else ys`) — *implemented*
  ...
  aggregate, program result, or comprehension — by one rule: the sum pins the consumer's kind
  variable, so the result is a sum over the same witness
  (type-inference.md, "Consuming a sum: pinning the consumer's kind").

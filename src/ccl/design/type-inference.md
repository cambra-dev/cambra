...
A literal is typed by *which* literal it is: `5 : {Int | __elem == 5}`, its base refined by the singleton predicate. Not a `Literal(base, value)` constructor — an ordinary witness, so every rule above applies unchanged and none has to learn a new case.
...
* **A `Case` takes its arms' type stripped** — it selects one arm at runtime, so it cannot carry any one arm's refinement. (Derived from an arm rather than a fresh join variable, so a `Mut` arm does not deref-coerce and lose the constructor the second-class check keys on.)
* **A list's elements and a `Case`'s arms *join*** rather than being required to equal the first one, which two elements carrying different singletons never could.
...
A feed handle is `Type::History { value: 𝑇, domain: 𝐷, kind: HistoryKind::Append }` (displayed `feed(𝐷 ⇒ 𝑉)`) — a function `𝐷 ⇒ 𝑇` carried as two children plus a two-valued `kind` marker. It **shares the `Type::History` variant with a mutable variable** (`kind: Overwrite`, displayed `Mut(𝑉, 𝐷)`); the two were unified from the former `Type::Feed(ρ)` / `Type::Mut{…}` pair (see mutability.md §"Collapse `Mut` / `Feed` into one `History` variant"). `let 𝑑 = Defer in body` gives `𝑑` a `Feed`-kind history whose channel `𝐷 ⇒ 𝑇` is the *post-desugar result type* of the binding (a `𝐷 ⇒ 𝑇` channel for fed defers, the defined value's type for `<<=`-defined defers). Like `Hole` and `Infer` the `Feed` kind is **transient**, scoped to inference: `channelize` (which runs after inference) eliminates every defer construct along with its feed histories, and no pass downstream of it may observe one. (This is the feed-handle type of mutability.md §"`Feed` is a CCL type" — what a defer-mediating UDF parameter carries.)

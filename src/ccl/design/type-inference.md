...
**A refinement never changes a type's shape.** It is a claim about the value at a position, not part of the structure carrying it, so `{(𝐷 ⇒ 𝑉) | 𝑝}` *is* a function and `{Mut(𝑉, 𝐷) | 𝑝}` *is* a mutable register. Every rule that dispatches on or destructures a shape therefore looks *through* the outer layers first — `Type::peel_refinements`, and the handle accessors `Type::as_register` / `as_feed` / `is_handle` built on it, which are what the typing rules and the second-class `Mut` discipline both ask "is this a register?" with. This is the same claim-versus-structure distinction that [type functions](#47-type-functions) draw by *polarity* when reducing: what a computed type must not inherit is the claim, and what it must keep is the structure (a collection domain's extent).
...
* A **deref-copy** `y : 𝑉 = 𝑥` off a register `𝑥` binds `y` at the value type `𝑉`, while its initializer is a *history*. Recording the initializer's type made `y` an alias of `𝑥` in the type system — so the second-class `Mut` discipline, which keys on types, then misfired on a variable the user declared immutable: `z = y` was rejected as an unannotated `Mut` alias, and `y += 1` was *accepted* as a write.
* A **register introduction** `𝑥 : Mut(𝑉) := init` binds `𝑥` at the history `Mut(𝑉, 𝐷)`, while its initializer is a plain value. Recording the initializer's type left the register's own slot reading `𝑉`, so the transaction-register scan could not classify it from the slot.
...
Nothing has to outlive the annotation. The one fact a later pass used to need from it — *is this binder a register?* — is answered structurally instead: only `MutDecl` (a `:=` introduction) and a pass-by-reference `Lambda` param bind one, and a `Let` cannot, because `emit_let` derefs a register-typed initializer. That retired the `Mut` discipline's rule 3 along with the bit that stood in for the declaration (see `src/ccl/design/mutability.md`, "No aliasing: `Mut` values are second-class (downward-only)").
...
The consequence is that no *typing* rule may take a `Below`. There is nothing to subtype against, nothing to reduce, nothing to compact — the solver asserts this rather than inventing a rule (`constrain::extrude`, `compact`, `reduce::strip_value_claims`). Only the structural walks that rewrite every slot uniformly — substitution, free-variable collection, refinement stripping — pass through one, and they do so because they are indifferent to what a slot means.
...
**Law 5 is polarity-directed.** A refinement at a *covariant* position is a claim about the value produced, which a computed type must not inherit. At a *contravariant* position it is not a claim at all — on a collection domain it is the **extent**, which the interpreter compiles to a `Restrict` at the iteration boundary, so dropping it changes which collection the type denotes rather than relaxing an assertion. Reduction therefore strips covariantly and preserves contravariantly (`strip_value_claims`), which is also what keeps the relation variance-stable: relating a refined domain to a stripped one is what manufactures the illegal `𝐷 <: {𝐷 | 𝑝}` obligation. No *program* reaches the contravariant case today — every operand of an arithmetic or comparison operator the runtime accepts is a scalar, which is also why the agreement test can be a plain `==` — so the rule is pinned by a unit test on `strip_value_claims` rather than by an end-to-end case. It is stated because the alternative is a rule that is wrong the moment a compound operand exists, which `FieldOf(ρ, 𝑘)` and `CollectionUnion` both are.

A **register** keeps its extent for a simpler reason: nothing strips it. A seed contributes verbatim, so `c := [v for v in xs if 𝑝]` types as the filtered collection it is. That shape then fails downstream — it does not terminate in join planning's `insert_iterate_markers`, while everything before completes (inference, the post-letrec `typecheck`, group-by recognition, `simplify`) — and it is the same unfinished area that makes a *write* to a collection register panic in `planning::loops`'s `split_decision_compose`. Mutable collections are unsupported either way. The loop is a planning bug to fix alongside mutable-collection support, not a restriction to encode in the type system.
...
|---|---|---|
| `Arithmetic` | none — the result rule checks the operands | `Add(α, β)` and siblings, a [type function](#47-type-functions) |
| `Concat` | both operands constrained to `String` | `String` |
| `Compare` | none — the result rule checks the operands | `Greater(α, β)` and siblings — a type function reducing to `Bool`, so the check is reachable |

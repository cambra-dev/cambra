...
2. **`Feed(a) <: 𝑇`** for non-feed `𝑇` ⇒ `a <: 𝑇` — transparent read (`sum(d)`, `d + 1`, `x <<= y` chains discharge through the handle). The post-inference structural check mirrors this, at both of the sites an application reaches: `CheckCtx::as_function` peels `Feed` like outer refinement tags, and `constrain_argument` takes the same read view (`Type::feed_read_view`) before looking for a domain. A comprehension over a channel applies it, so a handle unpeeled at either site leaves them disagreeing about whether a channel is a function.

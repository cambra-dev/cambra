...
* **Lattice-carried refinements.** Refinement witnesses ride the lattice natively (compared by structural predicate equality) rather than being stitched on by a post-pass; see §4 and [`crate::ccl::infer::solver`]'s `# Refinements`.
...
* **SMT-backed refinements.** Augmenting the lattice-carried refinement witnesses (today compared by structural equality only) with logical payloads (e.g. `v > 0`) reasoned about — implication, not just equality — by an external SMT solver such as Z3.
...
The algorithm walks the AST top-down. It normalizes each node's expected type into a solver-ready `ccl::Type` (via `normalize_annotation`: `Hole` → fresh `Type::Infer`; `Refinement` wrappers are *kept* — they ride the lattice as refinement witnesses, see §4), generates constraints, and writes each node's resulting `Type` directly onto `expr.ty`.
...
So the two passes share one description of the language's structure; they differ only in whether a rule's obligations are *emitted as constraints* or *checked against the recorded solution*. Adding or changing a structural rule updates both at once. Both contexts treat refinements strictly and identically; see §4 for how the check stays refinement-aware (adjacency flow checks) and why the passes that introduce refined types must keep each node reconstructable.
...
#### Feed handles as an invariant constructor (`Type::Feed`)

`Type::Feed(ρ)` is the type of a feed handle: `let 𝑑 = Defer in body` gives `𝑑 : Feed(ρ)` where `ρ` is the *post-desugar result type* of the binding (a `𝐷 ⇒ 𝑇` channel for fed defers, the defined value's type for `<<=`-defined defers). Like `Hole` and `Infer` it is a **transient** variant scoped to inference: `desugar_defers` (which runs after inference) eliminates every defer construct along with its feed types, and no pass downstream of desugar may observe one. (This is the feed-handle type of design-mut-txn-feed.md §"`Feed` is a CCL type" — what a defer-mediating UDF parameter carries.)
...
* **Extrusion** (`extrude_invariant`): a payload variable crossing a level boundary gets a *single* fresh proxy linked to the original by **both** a lower and an upper bound (an equality link through the standard lower×upper closure), instead of the polar one-way link. The proxy registers under both `ExtrudeCache` polarity keys.
* **Compaction/coalesce**: the payload occupies a dedicated `CompactType::chan` slot, recursing at the **same polarity** — by compaction time the constraint-level invariance has already propagated both directions, so this is materialization only, not a second polarity analysis. `simplify_type` walks the slot at the same polarity; refinement/co-occurrence behavior is unchanged.
* **Transparent read at joins** (`dissolve_read_feeds`): rule 2 covers a feed handle meeting a concrete consumer *directly*, but a read can also meet other contributions through a shared join variable (`x + 1` flows `Feed(Int)` and `Int` into the binop's `∀α.(α,α)→α`). At coalesce, a position carrying a `chan` **alongside** non-feed contributions dissolves the handle into its payload before the contribution count; a feed handle alone (or two handles merged) keeps its constructor. Feeding-then-scalar-reading still errors correctly: the dissolved payload is the channel `Fun(?, T)`, which genuinely collides with a scalar.
...
* **Refinements:** are **kept** (recursing to normalize the inner) — they ride the lattice as refinement witnesses (above). A `Refinement(Hole, r)` source annotation thus becomes `Refinement(?fresh, r)`.
...
* **Refinements:** the witness set carried at a position is re-wrapped as nested `Type::Refinement` layers around the materialized inner type (in first-insertion order — deterministic and, since consumers strip at all depths, order-independent).
...
* **O2 (polymorphic case)** — `freshen_above` copy-and-freshens a refined value's predicate type slots through the shared cache (its `Refinement` arm), so a specialization's predicate is a proper freshen instance rather than a shared `Rc`. Immutable predicate terms are acyclic, so no refinement-cycle guard is needed. A residual under-determination remains for one shape — a projecting inner comprehension over each group of a group-by (`[sum([s.amount for s in g]) for g in groupby(...)]`) leaves a predicate slot unresolved — but it is a feature that does not yet compile for an unrelated reason (incomplete `curry` support in op-conversion); no compiling program hits it.
...
| **CompactType** | Algebraic subtyping | A flat, per-position bag of contributions (variables, atoms, an optional record shape, an optional variant shape, an optional function shape, and a witness set) produced for simplification and co-occurrence analysis. |

...
asks it about a predicate, where it is the same-restriction relation; `Subst::eq_modulo_ty_slots`
asks it about a discharge's captured argument, which is any expression the program wrote, a function
body included. Its domain is the wider of the two, so every node a program builds has an arm — the
mutable-variable introduction and the loop among them, each binding over its body the way a `let`
does. `LetRec` and `Transact` have none: the mutability-elimination phases mint them and inference
precedes those phases.

That a predicate is a boolean expression over `__elem` is asserted where a predicate is installed,
at `Refinement::born` and at the rewrite path's `point_at`. Leaving it implicit in a missing match
arm made one walk answer for the narrower domain while its other caller used the wider one, and the
report named a predicate losing reflexivity for a term that was never a predicate.

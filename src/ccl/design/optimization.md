...
| `Builtin::Iterate` | `iterate` | planning-introduced chain-head iteration source: `iterate(p) : {𝑑: 𝐷 \| 𝑝(𝑑)} ⇒ {𝑑: 𝐷 \| 𝑝(𝑑)}` (or `𝐷 ⇒ 𝐷` when `p` is the trivially-true `true ▷ const`). `planning` emits one at the head of every iteration site (aggregate arguments, the stream side of `LastOrDefault`, top-level function-valued results, sink-bound record fields, mutation-loop sources, …). Op-conversion's Iterate arm requires `input=None`; it compiles `Apply(p, Iterate)` to an `IterateExtent` tile (plus a `Restrict` filter when `p` is non-trivial). Mid-chain filtering is the separate `Restrict` variant. |

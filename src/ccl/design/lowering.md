...
- `` `tag(e) `` lowers to `VariantCtor { tag, payload }`. The bare `` `tag `` lowers to the same node with a `Lit(Unit)` payload, so there is one constructor shape rather than a nullary/unary pair.

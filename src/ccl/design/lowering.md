...
- ``match s: case `tag(w): body …`` lowers to `Case { scrutinee: Some(s), branches }`, one `Branch` per arm carrying a `Pattern { tag, binding, empty_payload }`. Every arm's guard is the literal `true`, which is what makes tag dispatch and the guarded `if`/`elif` chain the *same* first-match rule over one node. An arm that names no payload — `` case `tag(_): `` or `` case `tag: `` — still binds, to a minted `__match_payload_N` the body cannot spell, so downstream passes need no separate no-binder case.
- The two of those are **not** the same claim, and `empty_payload` is the one bit that separates them. `` case `tag(_): `` says the arm does not read its payload, which is not a statement about the type; `` case `tag: `` says the tag carries nothing, which is one — `` `some{Int} `` and `` `some `` are different types, so it must not match a tag carrying a payload. `emit_case` records the claim as `payload <: Unit` on that arm, *after* the scrutinee constraint (so the payload has already flowed in) and per arm rather than folded into the expected variant, which is what lets the rejection name the arm instead of reading as a whole-scrutinee mismatch.
...
Nothing prunes them. `variant_project` names the tag it reads, and a tag the column does not carry yields an empty restriction that contributes nothing to the union — so a dead arm is simply inert. The one thing it needs is a payload *type*, and no value reaches the position to supply one, so coalesce pins it (`pin_unobservable_arm_payload`): to the base a demand already recorded there determines, else to one every requirement the arm's own body states accepts, else to `Unit` when the body does not read it at all. The pin constrains in **both** directions, which is what makes the arm's slot and the merge over the arms agree — a bound above the position alone does not participate in the merge. See [type-inference.md](type-inference.md), "Requirements are read together, once".

...
`𝐷 ⇒ 𝑉` (a capability) by its [`FunKind`](type-inference.md#46-data-vs-compute-functions-and-conditional-collection-domain-joins).
...
§4.6](type-inference.md#46-data-vs-compute-functions-and-conditional-collection-domain-joins)) are
...
> between branches — the five types, the Σ arms, `reify`, the constructors —
...
rule](type-inference.md#46-data-vs-compute-functions-and-conditional-collection-domain-joins)
...
[type-inference.md §4.6](type-inference.md#46-data-vs-compute-functions-and-conditional-collection-domain-joins),
...
§4.6](type-inference.md#46-data-vs-compute-functions-and-conditional-collection-domain-joins).

### Lowering realization: `reify` over a present-key refinement

The keyed Σ is not written by the programmer — lowering builds it, and the term
must be **well-formed at every compilation stage** (no free variables, valid pre-
and post-inference). Two changes to today's `groupby` lowering realize it:

1. **Refine the outer binder** with the present-key predicate `𝑘 ∈ (c ≫ key)`. The
   key-set `𝐸` is not a free variable — it is the concrete **key-image** `c ≫ key`
   (`𝐼 ⤇ 𝐾`, in scope), inlined into the refinement. A `Compose` takes its data
   kind from `c`, so the key-image is a `Collection(𝐾)` and `𝑘 ∈ (c ≫ key)`
   type-checks via the membership atom. This is the finite present-key domain that
   the old total `(𝑘: 𝐾) ⇒ …` domain lacked.
2. **Wrap the outer lambda in `reify`** to cross `Compute → Data`: the refined
   lambda `λ (𝑘 : {𝐾 | 𝑘 ∈ (c ≫ key)}) → group(𝑘)` is a capability (`⇒`), and the
   keyed collection is a data function (`⤇`). `reify` is the only thing that can make
   that crossing.

So lowering emits (with the inner group unchanged from today):

```
groupby(c, key)  ⟶  reify( λ (𝑘 : {𝐾 | 𝑘 ∈ (c ≫ key)}) → cast(λ 𝑖 → c(𝑖), {𝐼 | key(c(𝑖)) == 𝑘} ⇒ 𝐴) )
```

which infers as the concrete keyed collection `{𝐾 | 𝑘 ∈ (c ≫ key)} ⤇ ({𝐼 | key(c(𝑖)) == 𝑘} ⤇ 𝐴)`
and **injects into** `Map(𝐾, Collection(𝐴))` by discharging the key-set witness
`𝐸 := c ≫ key` — exactly as a list literal `[0, 𝑘) ⤇ 𝑇` injects into `List(𝑇)`.
There is no live abstract witness in `groupby`'s own type: `𝐸` is the concrete
key-image, and `Map`'s Σ is the abstraction supplied by entering it.

**`reify` never reaches op-conversion; the membership refinement stays faithful.**
Two stages, and which does what matters:

- **Lambda elimination** eliminates the inner cast-lambda to the point-free
  group-by source `const(cast(c)) : (𝑘) ⇒ group` and then performs `reify`'s only
  runtime-visible act: it flips that outer arrow's kind to `Data`, so the
  eliminated type equals the inferred keyed collection. The `reify` atom is
  consumed there — it never survives the pass.
- **Planning** is what produces `Converse`: `convert_groupby_pointful` recognizes
  the Pi-const source (`recognize_groupby_sites`) and emits the group-by. This
  ordering is forced, not incidental — the recognizers must run on the *bare*
  pointful predicate form, before `compile_refinement_predicates`
  ([type-inference.md, "4.5 Dependent refinements via Pi types"](type-inference.md#45-dependent-refinements-via-pi-types)).
  `Converse`'s **key-extraction** morphism `c ≫ key : 𝐼 ⇒ 𝐾` produces plain keys,
  so the recognizer strips refinements from that morphism's codomain — the
  "present" identity is emergent from `Converse`, not carried on the extracted key.

The **key-domain refinement on the result** `{𝐾 | 𝑘 ▷ keydom#id} ⤇ …` is **kept** — it
faithfully expresses that the domain is *this* group-by's present keys, so it rides
through to op-conversion. Precisely: planning *does* compile it to point-free form
along with every other surviving predicate, so a `keydom` atom appears in
post-planning types; what never happens is its **execution** — it is never lowered to
a `Restrict`,
because `Converse` discharges it structurally. That distinction is load-bearing: a
compiled-but-unexecutable atom riding a type is only safe while nothing lowers it,
so op-conversion asserts no `Member` reaches it rather than relying on that
holding by construction.

**Invariant — no iteration markers inside predicates.** The membership predicate
`𝑘 ∈ (c ≫ key)` embeds a *collection* (the key-image), unlike a scalar filter
predicate. That collection also lives in the term tree, where planning wraps it in
`iterate` markers; a move-site discharge of a `let`-bound source into the
predicate would otherwise drag those markers into the type, where they make
`simplify` churn (nested-`Compose` vs `flatten_compose`) and diverge from
inference's pre-marker copy. So the term→type substitution boundary strips the
...
invariant that keyed collections are the first to exercise, not a group-by
special-case.
...
reify-`groupby` eliminates to a *dependent* `const(cast) ≫ collapse`, whose key binder
...
`reify(λ 𝑘 → cast(…))` eliminates to a Pi-const source that planning recognizes as
`Converse` (see [Lowering realization](#lowering-realization-reify-over-a-present-key-refinement)),
...
- **The runtime witness.** The load-bearing item, and the one the others reduce to.
  ...
  **What forces it is producer-visibility, not abstraction.** An abstract *parameter*
  ...
  concrete producer whatever the type says. (Today a second mechanism also hides it —
  a `Collection(𝑇)` annotation is a *bound*, so the type monomorphizes too. Exact
  annotations retire that one and not inlining.) What no call-site instantiation

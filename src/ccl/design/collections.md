...
#### Runtime realization and its limits

A `Map` or `Set` value is a runtime tile the existing operators already build. `groupby`'s
`λ 𝑘 → cast(…)` eliminates to a Pi-const source that planning recognizes as `Converse`
([The key domain is the key morphism's image](#the-key-domain-is-the-key-morphisms-image)),
...
consumption (`for v in m`) need **no new `Domain` and no hash store**: a keyed domain
dispatching `extent_of` to a hash store is what a lookup (`𝑚[𝑘]`) needs, and it arrives with
that feature rather than with immutable construction.

`set([1,2,2,3])` produces the sealed keyed tile over keys `{1, 2, 3}`, and
`map([(1, 10), (2, 20)])` the sealed keyed tile carrying `10` and `20`.
[`AggregateKind::Drain`] is `(𝐷 ⤇ 𝛾) ⇒ unit` with accumulator `Units(1)` and `accumulate` a
no-op, which is what absorbs a key's duplicates into one `unit`.

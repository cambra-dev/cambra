...
([chl-spec, Direction: collection types](../../../docs/chl-spec.md#63-direction-collection-types-decided)).
`Map(𝐾, 𝑉)` is what remains after forgetting which key set: a Σ over `SubtypesOf(𝐾)`, whose
...
would ([The kind is declared](#the-collection-type-is-declared-not-read-off-the-shape)), so today's
...
**characteristic-predicate** form won: `Map(𝐾,𝑉) = Σ (𝐷: SubtypesOf(𝐾)). 𝐷 ⤇ 𝑉`, where a
...
**What `SubtypesOf` adds to the kind system: a parameter.** `Enumerated` *lists* its
candidates and `UIntRanges` *describes* its, so containment against either is a
decidable question about one kind. `SubtypesOf(𝐾)` carries a **type parameter**, and
...
solver](type-inference.md#kind-containment)).
...
[kind-representation question](#the-collection-type-is-declared-not-read-off-the-shape), not merely
...
rather than relating the two by a typing rule ([type-inference.md, Planning asserts
the type it replaces](type-inference.md#planning-asserts-the-type-it-replaces)).

...
| [branching.md](branching.md) | The design space of running two versions of a program at once: the fragment *execution model*, the *evaluation* and *effect ownership* axes, the derived per-register *history* mode, the two laws and four well-formedness conditions, and the deployment patterns (upgrade, live experiment, shadow, canary, reverse shadow) that fall out. |

...
/// [`Builtin::GetPrevSeq`] (induction domain). A reference consumed only
/// through the accessor's history slot depends only on strictly earlier
/// positions, so it is guarded (design doc `src/ccl/design/mutability.md`,
/// "New builtins").
...
fn collect_unguarded_refs(e: &TypedExpr, live: &BTreeSet<Name>, out: &mut BTreeSet<Name>) {
    ...
    // earlier positions). Written so a future guard accessor only has to

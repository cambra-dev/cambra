...
//! condition (design doc, "`LetRec`") additionally requires every *unguarded*
//! reference to be **position-non-increasing** — it must pass the ambient
//! position through (as `store(𝑡)` does inside the commit record for time
//! `𝑡`), never consult a strictly-later position. This check verifies only the
//! *acyclicity* of the unguarded subgraph; it performs no position analysis, so
//! a position-*increasing* unguarded edge (e.g. `a = λ i → b(i+1)`) closing a
//! cycle through a guarded back-edge would be accepted here. That shape is not
//! reachable today — the phases emit only position-passing unguarded references,
...
/// is guarded (design doc `src/ccl/design/mutability.md`, "New builtins").
...
/// through the accessor. The guarded grammar:
///
/// - a bare `Var(h)`;
...
///   parameter only);
/// - a **`⧺`-union** of guarded slots ([`TypedExprNode::CollectionUnion`]) —
...
/// directly) is an ordinary, unguarded reference.
fn is_guarded_history_slot(history: &TypedExpr, live: &BTreeSet<Name>) -> bool {
    match &history.node {
        ...
        TypedExprNode::Compose(elts) => {
            ...
            // `collect_unguarded_refs` respects the step's own binders). A
            // step that *does* reference a binding reads it outside the
            // accessor, so the slot is unguarded.
            matches!(elts.first().map(|e| &e.node), Some(TypedExprNode::Var(_)))
                && elts[1..].iter().all(|e| {
                    let mut refs = BTreeSet::new();
                    collect_unguarded_refs(e, live, &mut refs);
                    refs.is_empty()
                })
        }
        TypedExprNode::CollectionUnion(ops) => {
            !ops.is_empty() && ops.iter().all(|o| is_guarded_history_slot(o, live))
        }
        _ => false,
    }
}
...
fn collect_unguarded_refs(e: &TypedExpr, live: &BTreeSet<Name>, out: &mut BTreeSet<Name>) {
    ...
    // references are guarded (the accessor consults only strictly earlier
    // positions) — see `is_guarded_history_slot` for the exact grammar.
    if let TypedExprNode::Apply { function, argument } = &e.node
        ...
        && is_guarded_history_slot(history, live)
    {
    ...
    // guarded exactly as in the pointful shape; every other zip slot and any
    // middle compose element is an ordinary reference position. This is what
    // lets guardedness re-check at op-conversion entry, after `lambda_elim`
    ...
    if let TypedExprNode::Compose(elts) = &e.node
        ...
        && matches!(&last.node, TypedExprNode::Builtin(b) if is_guard_builtin(*b))
        ...
        && is_guarded_history_slot(view, live)
    {
        for s in rest_slots {
            collect_unguarded_refs(s, live, out);
        }
        for m in mids {
            collect_unguarded_refs(m, live, out);
        }
        return;
    }
    match &e.node {
...
mod tests {
    ...
    /// commit streams — is a guarded history slot: `store = λ t →
    /// get_prev_txn((c1 ≫ (λ r → {…})) ⧺ (c2 ≫ (λ r → {…})), t, 0)` with
    /// each `cᵢ` reading `store` bare crosses the guard on every cycle.
    #[test]
    fn union_of_pointwise_mapped_commit_streams_is_guarded() {
    ...
    /// NOT a guarded view — the map consults that binding outside the
    /// accessor, so the reference is ordinary and the cycle unguarded.
    #[test]
    fn map_body_reading_a_group_binding_is_unguarded() {
        ...
        let errs = check_letrec_guarded(&bindings).expect_err("map body reads x unguarded");
        assert_eq!(errs[0].cycle, vec![Name::raw("x")]);
    }

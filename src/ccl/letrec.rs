...
/// is causal (design doc `src/ccl/design/mutability.md`, "New builtins").
...
mod tests {
    ...
    /// `get_prev_*`, `incr_commits` reads `store` bare. The non-causal
    ...
    /// history slot — causal), and `incr_commits` reads `store` bare. The trip
    ...
    /// commit streams — is a causal history slot: `store = λ t →

...
/// Compile each claim of a set against the element type it sees in the restrict
/// pipeline planning will build for this domain — `base` narrowed by the claims
...
/// agree. For a cast target's claims, `base` is the cast value's domain (the
/// assertion base — see [`compile_refinement_predicates`]).
fn compile_claims(claims: &mut RefinementSet, base: &Type, memo: &PredMemo<Type>) {
    ...
    let elem_tys = crate::ccl::application_elem_types(claims.as_slice(), base);

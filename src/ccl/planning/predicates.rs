...
fn compile_predicates_in_type(ty: &mut Type, memo: &PredMemo<Type>) {
    if let Type::Refinement(base, refinements) = ty {
        // Each refinement is compiled against the element type it sees in the
        // restrict pipeline planning will build for this domain — the base
        // narrowed by the refinements applied before it. `wrap_with_iterate` builds
        // that pipeline from the same application order, so the compiled

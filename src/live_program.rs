...
use crate::ccl::{
    context::{CompileError, CompileStage, CompiledProgram, GlobalContext, compile_program},
    diff::diff,
};
use crate::interpreter::{
    Consumer, Value,
    operator_conversion::ReuseTally,
    operator_conversion::{StateConflict, TRANSACTION_STORE_ID},
...
impl LiveProgram {
    ...
    pub fn diff_against(
        ...
        let old = ctx.sources_and_sinks().compile_to(self.source(), stage)?;
        let new = ctx.sources_and_sinks().compile_to(code, stage)?;
        let d = diff(&old, &new);
        if d.is_identical() {
    ...
    ///
    /// Rejected when the new version would **drop state**: every mutable
    /// variable the running program holds a value for must be one the new
    /// version still declares, so that its value is seeded rather than
    /// discarded. Everything else is allowed: logic freely, and sources and
    /// sinks by addition.
    ///
    ...
    /// answers 404 rather than hanging. Only losing a variable's accumulated
    /// value is refused, because that is the one outcome an author cannot see
    /// having happened.
    ///
    /// On `Err` the running program is untouched and still serving: both the
    /// staged compile and this check run before anything is torn down.
    ///
    /// # Panics
    ///
    /// Panics if the real compile fails after the staged one succeeded. The two
    /// run the same passes over the same sources and sinks, so disagreement is a
    ...
    pub fn update(
        ...
        let planned = ctx
            .sources_and_sinks()
            .compile_to(code, CompileStage::Planned)?;

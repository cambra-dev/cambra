...
//! [`Expr`](crate::ccl::Expr) snapshot and reads the
//! [`ProvenanceTable`](crate::ccl::provenance::ProvenanceTable). It is
//! deliberately **serde-free**: serialization lives in the `cambra-inspector`
//! workspace crate behind the optional, default-off `serde` feature on
//! `cambra`, so plain `ccl`/interpreter builds (`cargo build -p cambra`) never
//! compile it.
//!
...
//! [`CompiledProgram::provenance`](crate::ccl::context::CompiledProgram::provenance)
//! table — the two are projections of the same nodes.
//!
//! # Scope
//!
...
//!   + the [`ProvenanceTable`](crate::ccl::provenance::ProvenanceTable).
...
//!   done at the *source* level, not over the lowered tree (decision D5): some
...
//! # Query handlers
//!
//! [`Snapshot`] bundles the two indices + the borrowed snapshot projections
//! (source text, IR, [`ProvenanceTable`](crate::ccl::provenance::ProvenanceTable), surface AST) and exposes the
...
//! always-`None`-in-M1 live seams (`tick`, `value_summary`).
//!
...
mod tests {
    ...
    /// Compile a CHL program and return its post-inference snapshot + table.
    fn compile_snapshot(code: &str) -> (crate::ccl::Expr, ProvenanceTable) {
        let mut ctx = GlobalContext::default();
        let consumer: Box<dyn Consumer> = Box::new(|| {});
        let compiled = compile_program(&mut ctx, code, consumer).expect("program compiles");
        (compiled.post_inference_ir, compiled.provenance)
    }

    /// Round-trip: a position resolved to a node via `SpanIndex` (span → node)
    /// must resolve back through the `ProvenanceTable` (node → span) to a span
    /// that actually contains that position. The backward and forward
    /// directions agree.
    #[test]
    fn span_index_round_trips_with_provenance_table() {
        // A trailing arithmetic expression bound to `x`. Lowering tags the
...
x
";
        let (snapshot, table) = compile_snapshot(code);
        let index = SpanIndex::build(&snapshot, &table);
        ...
        // node → span agrees with span → node: the resolved node's origins
        // include a span that contains the position we started from.
        let origins = table
            .origins(node)
            .expect("resolved node is known to the table");
        assert!(
            origins.iter().any(|s| s.start <= pos && pos < s.end),
            "node→span must point back at a span containing pos {pos}; origins: {origins:?}"
        );
    }

    /// Set semantics (D4): a position inside the `1 + 2` sub-expression returns
...
";
        ...
        let spans: Vec<_> = chain
            .iter()
            .map(|&n| {
                // Tightest matching origin span for this node.
                table
                    .origins(n)
...
";
    ...
    // -----------------------------------------------------------------------
    // NameBinderIndex (D5 — source-level lexical resolution)
    // -----------------------------------------------------------------------
...
";
    ...
    /// The D5-motivating case: goto-definition on a **multi-param `def`
    /// parameter** use resolves to that param's `name_span`. A lowered/uniquify
...
";
    ...
    /// `Module`, exactly the point of D5.
    #[test]

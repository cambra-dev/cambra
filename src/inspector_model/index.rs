...
//!
//! This is the *backward* direction of the provenance substrate: the
//! [`ProvenanceTable`](crate::ccl::provenance::ProvenanceTable) maps a
//! [`NodeId`] forward to the source spans it came from (`resolve`: node → span);
...
//! `(snapshot, table)` pair — see the round-trip in this module's tests.

use std::collections::HashMap;

use crate::ccl::Expr;
use crate::ccl::provenance::{NodeId, ProvenanceTable};
...
/// the coincident-span tie-break (D4): when two nodes carry byte-identical
...
impl SpanIndex {
    /// Build the index from the post-inference snapshot and its provenance
    /// table. Each node is indexed under every span in
    /// `provenance.origins(node)`; nodes with no origins (synthetic) or unknown
    /// to the table contribute nothing.
    pub fn build(snapshot: &Expr, provenance: &ProvenanceTable) -> Self {
        let mut entries = Vec::new();
        Self::collect(snapshot, provenance, 0, &mut entries);
        SpanIndex { entries }
    }

    fn collect(expr: &Expr, provenance: &ProvenanceTable, depth: u32, out: &mut Vec<Entry>) {
        let node = expr.node_id();
        if let Some(origins) = provenance.origins(node) {
            for &span in origins {
                out.push(Entry { node, span, depth });
            }
        }
        expr.walk_children(|c| Self::collect(c, provenance, depth + 1, out));
    }

    /// All nodes whose origin span contains `pos`, ordered outermost → innermost
    /// (the containment chain, D4). "Innermost" = narrowest span, with
    ...
    ///
    /// Coincident-span policy (D4): when the innermost span is shared by several

    ...
    I: ValueInput<'src, Token = Token, Span = Span>,
{
    recursive(|expr| {
        ...
        //
        // The two payload brackets are both accepted here and sorted out in
        // lowering, where the position is known: parens carry a term, braces
        // carry the tag's field list. Parsing only one of them per position
        // would need the parser to know which it was in, which it does not.
        ...
        let variant_ctor = just(Token::Backtick)
            ...
            .then(
                choice((
                    expr.clone()
                        ...
                        .map(PayloadBracket::Paren),
                    brace_type.clone().map(PayloadBracket::Brace),
                ))
                .or_not(),
            )
            .validate(|((tag, tag_span), bracket), e, emitter| {
                let payload = match bracket {
                    None => None,
                    // `` `tag() `` is the nullary form written the long way:
                    // there is no payload between the parens to carry.
                    Some(PayloadBracket::Paren(args)) if args.is_empty() => None,
                    Some(PayloadBracket::Paren(mut args)) => {
                        // A tag carries exactly one payload, so a multi-argument
                        // constructor has nothing to lower to. Point at the
                        // explicit tuple, which does.
                        if args.len() > 1 {
                            emitter.emit(Rich::custom(
                                e.span(),
                        ...
                        Some(VariantPayload::Term(Box::new(args.remove(0))))
                    }
                    Some(PayloadBracket::Brace(fields)) => {
                        Some(VariantPayload::Fields(Box::new(fields)))
                    }
                };
                Spanned::new(
                    e.span(),
...
/// position wanted that one. The paren form arrives as an argument *list* so a
/// multi-argument `` `tag(a, b) `` can be diagnosed rather than mis-parsed.
#[derive(Clone)]
enum PayloadBracket {
    Paren(Vec<Spanned<Expr>>),
    Brace(Spanned<Expr>),
}
...
mod tests {
    ...
    fn variant_arm_takes_a_brace_field_list() {
        for (src, positional) in [("`some{Int}", true), ("`pair{a: Int, b: Bool}", false)] {
            match parse_e(src).node {
                Expr::VariantCtor { payload, .. } => match payload.expect("payload") {
                    VariantPayload::Fields(f) => assert_eq!(
                        matches!(f.node, Expr::BraceGroup(_)),
                        positional,
                        "{src} field-list shape"
                    ),

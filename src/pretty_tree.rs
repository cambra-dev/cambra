...
/// converts the `ccl::provenance::RewriteTag` (`Phase`/`Nature`/label) into these.
#[derive(Debug, Clone)]
#[cfg_attr(feature = "serde", derive(serde::Serialize))]
pub struct RewriteInfo {
    /// The phase that performed the rewrite (the `Phase` debug name, e.g.
    /// `"Infer"`).
    pub via: String,
...
pub struct InspectNode {
    ...
    /// Children with edge labels, e.g. `("left", child_desc)`.
    ///
    /// The inspector's tree builder labels a **value** child with its positional
    /// index (`"0"`, `"1"`, …) and a **refinement predicate** riding one of the
    /// node's type slots with `"where.N"`. A non-numeric label is therefore how a
    /// consumer tells a subtree that lives inside a type from one that is an
    /// operand — see `crate::inspector_model` for why predicates are in the tree
    /// at all.

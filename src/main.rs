...
use cambra::{
    ccl::{
        context::{Endpoints, GlobalContext, eprint_errors, render_errors},
        symbolic::symbolic,
    },
            ...
            Ok(report) => {
                ...
                *new_data.borrow_mut() = true;
                ...
                ControlReply::ok(format!(
                    "updated: {reused}/{bound} bindings reused\n\n{}",
                    report.diff
                ))
            }
            Err(errs) => ControlReply::rejected(render_errors(&errs, "<new>", code)),
...
fn run_program(
    ...
    let mut live = match LiveProgram::start(&mut ctx, code, Endpoints::Open, &main_consumer) {
        Ok(p) => p,
        Err(errs) => {

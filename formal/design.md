...
This is the third member of the non-canonical-identity family (with the trivial-equality/find-first duplicate-key disagreement and refinement-layer order), and the repair is the one the codebase already applied to refinements themselves: **canonical Pi binder names** (`REFINEMENT_BINDER`'s treatment, positionally — one reserved name per Pi depth, since predicates may reference an outer binder). That change would make α-equivalence coincide with structural equality, collapse both defects above, retire `without_pi_names` (whose three call sites all patch the `Some`-vs-`None` erasure by rebuilt arrows) and the rename half of `bridge_holder_gap` — and it would make the σ-gap below unrepresentable, so the non-dependent transitivity proof *becomes* pure transitivity with no new metatheory. (In the model: `Ren`, `codRen`, and the `NoPi` hypothesis all disappear.) A correction from this probe: `SpecKey` was previously cited here as exposed to refinement-layer order — it is not; it compares refinement sets as sets. The exposure is specifically α-variance via predicates.

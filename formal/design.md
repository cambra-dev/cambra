...
**Canonical binders open a cheaper route than the σ-gap.** With the solver's flattening canonicalizing binders, every coalesced type is α-canonical — and a chain of α-canonical types has *diagonal* binder correspondences (`__piK ↦ __piK`) at every function edge, which act as the identity on rename environments. The transitivity proof's machinery already handles exactly that shape (`codRen_diag_isId` powers `Sub.refl`), so the next Lean step is `sub_trans` with `NoPi` relaxed to "all binders canonical": pure transitivity for every type the solver emits, with no σ-generalization at all. The σ-gap below then only concerns pre-coalesce, source-named types, and retires entirely with the minting-level stratum.
...
**Finding: refinement layers stack in constraint arrival order — and the representation serves three incompatible views.** Two refined upper bounds meeting at one variable coalesce to `{{𝑇 | 𝑞} | 𝑝}` or `{{𝑇 | 𝑝} | 𝑞}` depending on which arrived first (≈1 in 10k generated sets; one case showed both orders across two variables of a single run). Subtyping is indifferent — the deficit machinery compares layers as a set — but `Type`'s derived `PartialEq` is order-sensitive, and structural equality is load-bearing where types are *identities*: the trivial-equality short-circuit, cache keys, and recorded-vs-recomputed walls. (`SpecKey` itself is *not* exposed to layer order — it compares refinement sets as sets; its α-variance exposure is a separate finding under M0.)

The obvious fix was tried and **reverted, twice sharpening the finding**. Canonically sorting `CompactType::refinements` at the merge/collection sites fails exactly one test (`comprehensions::case_6`, the post-planning wall), and root-causing that failure exposed the real structure. Planning inserts no casts — the `cast(cast(..))` in the failing types is legitimate nested-comprehension lowering. The actual mechanism is **predicate vintages**: a dependent-application discharge embeds the *argument term* into refinement predicates, so the same filter exists in several vintages — `𝑝(xs)`, `𝑝(cast(xs))`, `𝑝(cast(cast(xs)))` — one per comprehension layer it was discharged through. `eq_refinement_predicate` distinguishes vintages (deliberately: a cast target carries a semantic filter, and the pinned test `refinement_eq_distinguishes_cast_target_predicates` records why conflating them could drop a runtime `Restrict`). Layer order doesn't create the vintages; it *selects which vintage* each type ends up carrying, and the recorded-vs-recomputed wall demands the two paths select consistently — which arrival order happens to deliver and canonical order happens to break, for this case.

So the blocker is a **predicate-identity question**, not an ordering mechanism: are two vintages of one refinement the same refinement? Vintage-blind equality (peel embedded casts before comparing) is non-transitive as stated (`xs ≡ cast{𝑎}(xs)` and `xs ≡ cast{𝑏}(xs)` but `cast{𝑎}(xs) ≢ cast{𝑏}(xs)`) and was implicitly rejected by the pinned test's soundness rationale; full transparency (ignore casts *and* their targets) is transitive but conflates semantically distinct embedded collections. The remaining candidate is **canonical discharge** — normalize what discharge embeds so vintages never diverge — which touches the substitution pipeline and needs a semantics ruling. Until then: pinned as `refinement_layer_order_depends_on_arrival_order`, and the confluence fuzz quarantines exactly this class (comparison after canonical layer sorting) so any new class still fails.

With that class quarantined: **100k constraint sets × 8 permutations across seeds, zero violations** — acceptance, coalesced structure, and kind-var resolution are confluent within the generated vocabulary.

...
**Model divergence pending the grammar update.** The Rust now carries refinements as an unordered `RefinementSet` (see the repaired finding under M4); the model's `refined` still carries one predicate per layer. The two agree on the subtype relation — the model peels layers into a set anyway — but *not* on partition collapse, whose `legUnder` peels one layer where `partition_domain` now takes the claims common to every leg. So the wire emitter **refuses** a multi-claim refinement rather than mis-serializing it as nested layers, and the differential harness counts and reports what it skipped. Restoring full coverage is the model's grammar update: `refined` carrying a claim set, with `Sub.refl` / decidability / transitivity re-established over it — which also *simplifies* the transitivity proof, since `Ty.peel` becomes trivial and the peel route collapses.

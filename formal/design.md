...
**Finding: refinement layers stack in constraint arrival order — and the representation serves three incompatible views.** Two refined upper bounds meeting at one variable coalesce to `{{𝑇 | 𝑞} | 𝑝}` or `{{𝑇 | 𝑝} | 𝑞}` depending on which arrived first (≈1 in 10k generated sets; one case showed both orders across two variables of a single run). Subtyping is indifferent — the deficit machinery compares layers as a set — but `Type`'s derived `PartialEq` is order-sensitive, and structural equality is load-bearing where types are *identities*: `SpecKey` (monomorphization sharing), the trivial-equality short-circuit, cache keys. Two uses that should share a specialization can split on arrival order.

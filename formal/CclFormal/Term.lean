...
- **`cast` checks its refinements at runtime, and progress is stated modulo
  ...
  scalar small-step mirrors that as: `cast` steps through when every refinement
...
- **`refineV` — checked values inhabit the refinement.** `cast` is the only
  ...
  value-level introduction: `cast p v` (refinements holding) steps to `v`, and
  `v`'s refined typing must come from somewhere — `Sub` deliberately forbids
  refinement conjuring. `refineV` says a refinement that *evaluates true on a
  value* types that value: the term-model face of literal-singleton typing
  (a refinement is a fact about a value). Runtime-checked (`refinementsHold`),
...
carries the refinement set it asserts; it is the only refinement introduction,
...
inductive Tm where
  ...
  | cast (refinements : List Pred) (e : Tm)
deriving Repr
...
def Tm.refinementsHold (refinements : List Pred) (v : Tm) : Bool :=
  refinements.all fun p => Pred.eval v p == some (.bool true)
...
inductive Blocked : Tm → Prop
  | castV {refinements v} : IsVal v → refinementsHold refinements v = false → Blocked (.cast refinements v)
...
predicate is `Pred.elemOnly`**, the non-dependent fragment. Off it, a refinement
references an enclosing binder the term judgment does not scope — the
element-wise checks (`refinementsHold`, `Pred.eval`) hold no frame parameter to
supply for a `piBound` index or a free name, so a dependent refinement's truth on
a value is not even stated here. The dependent fragment enters when the
judgment scopes its types' frames, not before.
...
innermost binder). Subsumption is the ground relation directly — with refinements
closed at construction it carries no environments to instantiate.

`cast` is the refinement introduction a *program* writes: it asserts its
refinements on top of the value's type, and the small-step checks them — the
pairing refinement soundness rests on. `refineV` is the value-level
introduction preservation forces (see the module docs): refinements that
...
inductive HasTy : List Ty → Tm → Ty → Prop
  ...
  | cast {Γ e T} {refinements : List Pred} :
      HasTy Γ e T →
      refinements ≠ [] → (∀ p ∈ refinements, p.elemOnly = true) →
      T.isRefined = false →
      HasTy Γ (.cast refinements e) (.refined T refinements)
  | refineV {Γ v T} {refinements : List Pred} :
      HasTy Γ v T → v.IsVal →
      Tm.refinementsHold refinements v = true →
      refinements ≠ [] → (∀ p ∈ refinements, p.elemOnly = true) →
      T.isRefined = false →

...
- **Typing inversions absorb subsumption chains with typing transport, not
  ...
  otherwise lean on transitivity of `Sub`. Instead `HasTy.lam_inv` returns
  implications of the form "whatever is typed at `X` is typed at `Y`" —
  reflexive without `Sub.refl`'s `WF` side condition, and composable across
  a chain link-by-link, each link re-entering the typing via `HasTy.sub`
  directly: the relation carries no environments, so the rename-invariance
  lemma this transport used to need is gone with the machinery it
  compensated for.
-/
...
theorem Ty.TermFrag.peel_refinements : {t : Ty} → t.TermFrag →
...
/-! ## The deficit is refinement containment -/

/-- An empty deficit is refinement containment. -/
...
theorem Sub.to_peel {S W : Ty} (h : Sub S W) :
    Sub S.peel.1 W.peel.1 := by
  cases h with
...
theorem Sub.refinements_mono {S W : Ty} (h : Sub S W) :
...
theorem Sub.fn_src {S : Ty} {n k d c}
    (h : Sub S (.fn n k d c)) (hS : S.isRefined = false) :
...
codomain edge. -/
theorem Sub.fn_inv {n0 n1 : Option String} {k0 k1 : FunKind}
    {d0 d1 c0 c1 : Ty}
    (h : Sub (.fn n0 k0 d0 c0) (.fn n1 k1 d1 c1)) :
    Sub d1 d0 ∧ Sub c0 c1 := by
  cases h with
...
theorem Sub.tuple_inv {S : Ty} {Ts : List Ty}
    (h : Sub S (.tuple Ts)) (hS : S.isRefined = false) :
...
theorem Sub.variant_inv {S : Ty} {tagsW : List (FieldKey × Ty)}
    (h : Sub S (.variant tagsW)) (hS : S.isRefined = false) :
    ∃ tagsS, S = .variant tagsS ∧
      ∀ tg t0, (tg, t0) ∈ tagsS →
        ∃ t1, lookupBy tagsW tg = some t1 ∧ Sub t0 t1 := by
  cases h with
  | variant hcov hpay =>
...
@[simp] theorem shift_cast {c : Nat} {refinements : List Pred} {e : Tm} :
    (Tm.cast refinements e).shift c = .cast refinements (e.shift c) := by
  simp [Tm.shift]
...
@[simp] theorem subst_cast {k : Nat} {v : Tm} {refinements : List Pred} {e : Tm} :
    Tm.subst k v (.cast refinements e) = .cast refinements (Tm.subst k v e) := by
  rw [Tm.subst.eq_def]
...
theorem refinementsHold_shift {v : Tm} {refinements : List Pred} (c : Nat) :
    Tm.refinementsHold refinements (v.shift c) = Tm.refinementsHold refinements v := by
  unfold Tm.refinementsHold
  congr 1
  funext p
  rw [eval_shift]

theorem refinementsHold_subst {v : Tm} (hv : v.IsVal) {refinements : List Pred}
    (k : Nat) (u : Tm) :
    Tm.refinementsHold refinements (Tm.subst k u v) = Tm.refinementsHold refinements v := by
  unfold Tm.refinementsHold
  congr 1
  funext p
  rw [eval_subst hv]
...
theorem HasTy.weaken {Γ : List Ty} {e : Tm} {W : Ty} (h : HasTy Γ e W) :
  ...
  | refineV _ hval hch hne hcl hbase ih =>
      ...
      exact .refineV (ih Γ₁ Γ₂ U hΓ) (hval.shift _)
        (by rw [Tm.refinementsHold_shift]; exact hch) hne hcl hbase
  | sub _ hsub hfrag ih =>
      intro Γ₁ Γ₂ U hΓ
...
theorem HasTy.subst_preserves {Γ : List Ty} {e : Tm} {W : Ty}
  ...
  | refineV _ hval hch hne hcl hbase ih =>
      ...
      exact .refineV (ih Γ₁ Γ₂ T v hΓ hv) (hval.subst _ _)
        (by rw [Tm.refinementsHold_subst hval]; exact hch) hne hcl hbase
  | sub _ hsub hfrag ih =>
      intro Γ₁ Γ₂ T v hΓ hv
...
theorem HasTy.lam_inv {Γ : List Ty} {L : Tm} {W : Ty} (h : HasTy Γ L W) :
  ...
  | @sub Γ' e S W' hty hsub hfrag ih =>
      ...
      have hcod_id : Sub c' cod := hcod_sub
...
theorem progress_aux {Γ : List Ty} {e : Tm} {T : Ty} (h : HasTy Γ e T) :
  ...
  | @cast Γ' e0 T0 refinements _ _ _ _ ih =>
      intro hΓ; subst hΓ
      rcases ih rfl with hve | hste
      · cases hch : Tm.refinementsHold refinements e0 with
...
them checked, `sub` only ever shrinks them (`Sub.refinements_mono`), and no
other rule types a value at a refined type. -/
theorem HasTy.value_refinements {Γ : List Ty} {v : Tm} {W : Ty}
    (h : HasTy Γ v W) :
...
theorem refinement_soundness {e v : Tm} {T : Ty} {refinements : List Pred}
    (h : HasTy [] e (.refined T refinements)) (hs : Tm.Steps e v)
    (hv : v.IsVal) : Tm.refinementsHold refinements v = true := by

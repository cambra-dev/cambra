...
/-!
# M2 — Safety: progress, preservation, and refinement soundness

The proof battery over `Term.lean`'s judgment: weakening and the
substitution lemma (the de Bruijn payoff), `Sub` inversion and canonical
forms modulo refinement peeling, progress (a well-typed closed term is a
value, steps, or is filter-blocked at a cast), preservation, and the two
...
- **`Sub` inversions are case analyses, not inductions.** Every `Sub`
  constructor pins both sides' head constructors, and the `refined` arm
  recurses on fully-peeled bases — so `Sub.to_peel` (peel both sides) plus
  one non-recursive `cases` per head is all the inversion there is.
- **Typing inversions absorb subsumption chains with typing transport, not
  `Sub` composition.** Inverting a derivation that ends in `sub` would
  otherwise need transitivity of `Sub` (M3b's open problem). Instead
  `HasTy.lam_inv` returns implications of the form "whatever is typed at
  `X` is typed at `Y`" — reflexive without `Sub.refl`'s `WF` side
  condition, and composable across a chain link-by-link, each link entering
  the typing via `HasTy.sub` (renames brought back to identity by
  `Sub.rename_invariant`, which is where the fragment earns its keep).
-/

namespace CclFormal

/-! ## Renames fix the non-dependent fragment

`Pred.rename` moves only `Pred.var` nodes, and the fragment has none — so
renames are invisible to it, which is what lets every `Sub` link a typing
inversion produces be brought back to the identity morphisms `HasTy.sub`
demands. -/

theorem Pred.elemOnly_rename (ρ : Ren) (p : Pred) :
    (p.rename ρ).elemOnly = p.elemOnly := by
  induction p <;> simp [Pred.rename, Pred.elemOnly, *]

theorem Pred.rename_of_elemOnly {p : Pred} (h : p.elemOnly = true) (ρ : Ren) :
    p.rename ρ = p := by
  induction p <;> simp_all [Pred.rename, Pred.elemOnly]

@[simp] theorem Ren.apply_nil (x : String) : Ren.apply [] x = x := rfl

theorem Pred.rename_nil (p : Pred) : p.rename [] = p := by
  induction p <;> simp [Pred.rename, *]

/-- For an `elemOnly` predicate, membership in a renamed claim list is
membership in the original: renames neither create nor destroy the match
(they preserve `elemOnly`-ness, and fix `elemOnly` predicates outright). -/
theorem Pred.mem_map_rename_iff {p : Pred} (hp : p.elemOnly = true)
    {l : List Pred} (ρ : Ren) : p ∈ l.map (Pred.rename ρ) ↔ p ∈ l := by
  constructor
  · intro h
    obtain ⟨q, hq, hqe⟩ := List.mem_map.mp h
    have hqo : q.elemOnly = true := by
      rw [← Pred.elemOnly_rename ρ q, hqe]; exact hp
    rw [Pred.rename_of_elemOnly hqo] at hqe
    exact hqe ▸ hq
  · intro h
    have hfix := Pred.rename_of_elemOnly hp ρ
    exact hfix ▸ List.mem_map_of_mem h
...
theorem Ty.TermFrag.peel_fst : {t : Ty} → t.TermFrag → t.peel.1.TermFrag
...
theorem Ty.TermFrag.peel_claims : {t : Ty} → t.TermFrag →
  ...
  | .refined _ _, .refined hps hb => by
      ...
      · exact hps p hp
      · exact hb.peel_claims p hp
  | .base _, h | .uintRange _, h | .dataSource _, h | .txn, h
  | .fn .., h | .tuple _, h | .record _, h | .variant _, h => by
      simp [Ty.peel]

/-! ## The deficit under identity and under the fragment -/

/-- At identity morphisms an empty deficit is claim containment. -/
theorem deficit_nil_mono {l r : List Pred} (h : deficit [] [] l r = []) :
    ∀ p ∈ r, p ∈ l := by
  have hmap : l.map (Pred.rename []) = l := List.map_id'' Pred.rename_nil l
  intro p hp
  unfold deficit at h
  rw [List.filter_eq_nil_iff] at h
  have hc := h p hp
  rw [Pred.rename_nil, hmap] at hc
  simpa [List.contains_iff_mem] using hc

/-- In the fragment the deficit cannot see the morphisms at all. -/
theorem deficit_invariant {lrefs rrefs : List Pred}
    (hr : ∀ p ∈ rrefs, p.elemOnly = true) (ρl ρr ρl' ρr' : Ren) :
    deficit ρl ρr lrefs rrefs = deficit ρl' ρr' lrefs rrefs := by
  unfold deficit
  apply List.filter_congr
  intro p hp
  have hpo := hr p hp
  rw [Pred.rename_of_elemOnly hpo, Pred.rename_of_elemOnly hpo]
  congr 1
  rw [Bool.eq_iff_iff]
  simp only [List.contains_iff_mem]
  rw [Pred.mem_map_rename_iff hpo, Pred.mem_map_rename_iff hpo]

/-! ## Keyed lookup bridges

The typing rules use `List.lookup`, the relation's arms use `lookupBy` —
both are find-first by key, and the proofs cross between them. -/
...
theorem lookupBy_mem {α} [BEq α] [LawfulBEq α] {l : List (α × Ty)} {k : α}
    {t : Ty} (h : lookupBy l k = some t) : (k, t) ∈ l := by
...
/-! ## `Sub` inversion: peel, then one `cases` per head -/

/-- Peel both sides of a subtyping: the `refined` arm *is* this move, and
every other arm pins two unrefined heads that peel to themselves. -/
theorem Sub.to_peel {ρl ρr : Ren} {S W : Ty} (h : Sub ρl ρr S W) :
    Sub ρl ρr S.peel.1 W.peel.1 := by
  cases h with
  | refined hl hr _ _ hbase => rw [hl, hr]; exact hbase
  | base b => simpa [Ty.peel] using Sub.base b
...
claims (across all its peeled layers), the subtype already claimed. -/
theorem Sub.claims_mono {S W : Ty} (h : Sub [] [] S W) :
    ∀ p ∈ W.peel.2, p ∈ S.peel.2 := by
  cases h with
  | refined hl hr _ hdef _ =>
      rw [hl, hr]
      exact deficit_nil_mono hdef
  | base b => simp [Ty.peel]
...
theorem Sub.fn_src {ρl ρr : Ren} {S : Ty} {n k d c}
...
theorem Sub.tuple_inv {ρl ρr : Ren} {S : Ty} {Ts : List Ty}
    (h : Sub ρl ρr S (.tuple Ts)) (hS : S.isRefined = false) :
...
theorem Sub.variant_inv {ρl ρr : Ren} {S : Ty} {tagsW : List (FieldKey × Ty)}
    (h : Sub ρl ρr S (.variant tagsW)) (hS : S.isRefined = false) :
    ∃ tagsS, S = .variant tagsS ∧
      ∀ tg t0, (tg, t0) ∈ tagsS →
        ∃ t1, lookupBy tagsW tg = some t1 ∧ Sub ρl ρr t0 t1 := by
  cases h with
  | variant hcov hpay =>
...
/-! ## Rename invariance on the fragment

The morphisms exist to transport Pi-binder references, and the fragment has
none — so a fragment subtyping holds at *every* pair of morphisms if it
holds at one. This is what re-admits the codomain edge's `codRen`-extended
links into `HasTy.sub`'s identity-morphism subsumption. -/

theorem Sub.rename_invariant {ρl ρr : Ren} {T U : Ty} (h : Sub ρl ρr T U) :
    T.TermFrag → U.TermFrag → ∀ ρl' ρr' : Ren, Sub ρl' ρr' T U := by
  induction h with
  | base b => exact fun _ _ _ _ => .base b
  ...
  | tuple hlen _ ih =>
      intro hT hU ρl' ρr'
      cases hT with | tuple ha =>
      cases hU with | tuple hb =>
      exact .tuple hlen fun i t0 t1 h0 h1 =>
        ih i t0 t1 h0 h1 (ha _ (List.mem_of_getElem? h0))
          (hb _ (List.mem_of_getElem? h1)) _ _
  | record hcov _ ih =>
      intro hT hU ρl' ρr'
      cases hT with | record ha =>
      cases hU with | record hb =>
      exact .record hcov fun n t0 t1 hmem hlk =>
        ih n t0 t1 hmem hlk (ha _ (lookupBy_mem hlk)) (hb _ hmem) _ _
  | variant hcov _ ih =>
      intro hT hU ρl' ρr'
      cases hT with | variant ha =>
      cases hU with | variant hb =>
      exact .variant hcov fun k t0 t1 hmem hlk =>
        ih k t0 t1 hmem hlk (ha _ hmem) (hb _ (lookupBy_mem hlk)) _ _
  | refined hl hr hne hdef _ ihbase =>
      intro hT hU ρl' ρr'
      have hlb := hT.peel_fst
      have hrb := hU.peel_fst
      have hrc := hU.peel_claims
      rw [hl] at hlb
      rw [hr] at hrb hrc
      exact .refined hl hr hne
        ((deficit_invariant hrc _ _ _ _).symm.trans hdef)
        (ihbase hlb hrb _ _)
...
closes the loop. -/
theorem hasTy_frag {Γ : List Ty} {e : Tm} {T : Ty} (h : HasTy Γ e T) :
    (∀ X ∈ Γ, X.TermFrag) → T.TermFrag := by
  induction h with
  ...
  | cast _ _ hclaims _ ih => exact fun hΓ => .refined hclaims (ih hΓ)
  | refineV _ _ _ _ hclaims _ ih => exact fun hΓ => .refined hclaims (ih hΓ)
  | sub _ _ hfrag _ => exact fun _ => hfrag

/-! ## Shift and substitution, constructor-wise

`shift`/`subst` recurse through lists via `attach` (their termination
device); these unfoldings restate them as plain `map`s so the typing
proofs never see a subtype. -/

namespace Tm

@[simp] theorem shift_lit {c : Nat} {l : Lit} :
    (Tm.lit l).shift c = .lit l := by simp [Tm.shift]

@[simp] theorem shift_var {c n : Nat} :
    (Tm.var n).shift c = if n < c then .var n else .var (n + 1) := by
  simp [Tm.shift]

@[simp] theorem shift_lam {c : Nat} {dom : Ty} {body : Tm} :
    (Tm.lam dom body).shift c = .lam dom (body.shift (c + 1)) := by
  simp [Tm.shift]

@[simp] theorem shift_app {c : Nat} {f a : Tm} :
    (Tm.app f a).shift c = .app (f.shift c) (a.shift c) := by
  simp [Tm.shift]

@[simp] theorem shift_letE {c : Nat} {bound body : Tm} :
    (Tm.letE bound body).shift c = .letE (bound.shift c) (body.shift (c + 1)) := by
  simp [Tm.shift]

@[simp] theorem shift_tuple {c : Nat} {es : List Tm} :
    (Tm.tuple es).shift c = .tuple (es.map (Tm.shift c)) := by
  rw [Tm.shift]
  congr 1
  exact List.attach_map_val

@[simp] theorem shift_proj {c : Nat} {e : Tm} {i : Nat} :
    (Tm.proj e i).shift c = .proj (e.shift c) i := by
  simp [Tm.shift]

@[simp] theorem shift_variant {c : Nat} {tag : FieldKey} {e : Tm} :
    (Tm.variant tag e).shift c = .variant tag (e.shift c) := by
  simp [Tm.shift]

@[simp] theorem shift_caseE {c : Nat} {scrut : Tm} {arms : List (FieldKey × Tm)} :
    (Tm.caseE scrut arms).shift c =
      .caseE (scrut.shift c) (arms.map fun a => (a.1, a.2.shift (c + 1))) := by
  rw [Tm.shift]
  congr 1
  show arms.attach.map (fun x => (x.1.1, Tm.shift (c + 1) x.1.2)) = _
  rw [List.map_attach_eq_pmap]
  show arms.pmap (fun a _ => (a.1, Tm.shift (c + 1) a.2)) _ = _
  exact List.pmap_eq_map _

@[simp] theorem shift_cast {c : Nat} {claims : List Pred} {e : Tm} :
    (Tm.cast claims e).shift c = .cast claims (e.shift c) := by
  simp [Tm.shift]

@[simp] theorem subst_lit {k : Nat} {v : Tm} {l : Lit} :
    Tm.subst k v (.lit l) = .lit l := by rw [Tm.subst.eq_def]

@[simp] theorem subst_var {k : Nat} {v : Tm} {n : Nat} :
    Tm.subst k v (.var n) =
      if n = k then v else if n < k then .var n else .var (n - 1) := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_lam {k : Nat} {v : Tm} {dom : Ty} {body : Tm} :
    Tm.subst k v (.lam dom body) =
      .lam dom (Tm.subst (k + 1) (v.shift 0) body) := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_app {k : Nat} {v f a : Tm} :
    Tm.subst k v (.app f a) = .app (Tm.subst k v f) (Tm.subst k v a) := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_letE {k : Nat} {v bound body : Tm} :
    Tm.subst k v (.letE bound body) =
      .letE (Tm.subst k v bound) (Tm.subst (k + 1) (v.shift 0) body) := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_tuple {k : Nat} {v : Tm} {es : List Tm} :
    Tm.subst k v (.tuple es) = .tuple (es.map (Tm.subst k v)) := by
  rw [Tm.subst.eq_def]
  show Tm.tuple (es.attach.map fun x => Tm.subst k v x.1) = _
  congr 1
  exact List.attach_map_val

@[simp] theorem subst_proj {k : Nat} {v e : Tm} {i : Nat} :
    Tm.subst k v (.proj e i) = .proj (Tm.subst k v e) i := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_variant {k : Nat} {v : Tm} {tag : FieldKey} {e : Tm} :
    Tm.subst k v (.variant tag e) = .variant tag (Tm.subst k v e) := by
  rw [Tm.subst.eq_def]

@[simp] theorem subst_caseE {k : Nat} {v scrut : Tm}
    {arms : List (FieldKey × Tm)} :
    Tm.subst k v (.caseE scrut arms) =
      .caseE (Tm.subst k v scrut)
        (arms.map fun a => (a.1, Tm.subst (k + 1) (v.shift 0) a.2)) := by
  rw [Tm.subst.eq_def]
  show Tm.caseE (Tm.subst k v scrut)
      (arms.attach.map fun x => (x.1.1, Tm.subst (k + 1) (Tm.shift 0 v) x.1.2)) = _
  congr 1
  rw [List.map_attach_eq_pmap]
  show arms.pmap (fun a _ => (a.1, Tm.subst (k + 1) (Tm.shift 0 v) a.2)) _ = _
  exact List.pmap_eq_map _

@[simp] theorem subst_cast {k : Nat} {v : Tm} {claims : List Pred} {e : Tm} :
    Tm.subst k v (.cast claims e) = .cast claims (Tm.subst k v e) := by
  rw [Tm.subst.eq_def]

/-! ## Values, claims, and the two term transports -/

theorem IsVal.shift {v : Tm} (hv : v.IsVal) (c : Nat) : (v.shift c).IsVal := by
...
theorem IsVal.subst {v : Tm} (hv : v.IsVal) (k : Nat) (u : Tm) :
    (Tm.subst k u v).IsVal := by
  induction hv generalizing k u with
  | lit l => simpa using .lit l
  | lam dom body => simpa using .lam dom (Tm.subst (k + 1) (u.shift 0) body)
  | tuple _ ih =>
      rw [subst_tuple]
      refine .tuple fun e he => ?_
      obtain ⟨x, hx, rfl⟩ := List.mem_map.mp he
...
theorem eval_shift {v : Tm} (c : Nat) (p : Pred) :
    Pred.eval (v.shift c) p = Pred.eval v p := by
  induction p with
  | elem =>
      cases v with
      | var n => by_cases h : n < c <;> simp [h, Pred.eval]
      | lit l => simp [Pred.eval]
      | lam dom body => simp [Pred.eval]
      | app f a => simp [Pred.eval]
      | letE b body => simp [Pred.eval]
      | tuple es => simp [Pred.eval]
      | proj e i => simp [Pred.eval]
      | variant tag e => simp [Pred.eval]
      | caseE sc arms => simp [Pred.eval]
      | cast cl e => simp [Pred.eval]
  | unop op a iha => simp [Pred.eval, iha]
  | binop op a b iha ihb => simp [Pred.eval, iha, ihb]
  | proj a k iha => simp [Pred.eval]
  | app f a _ _ => simp [Pred.eval]
  | _ => simp [Pred.eval]

/-- Same for substitution into a *value*: values have no free variables at
their literal-observable surface. -/
theorem eval_subst {v : Tm} (hv : v.IsVal) (k : Nat) (u : Tm) (p : Pred) :
    Pred.eval (Tm.subst k u v) p = Pred.eval v p := by
  induction p with
  | elem => cases hv <;> simp [Pred.eval]
  | unop op a iha => simp [Pred.eval, iha]
  | binop op a b iha ihb => simp [Pred.eval, iha, ihb]
  | proj a k iha => simp [Pred.eval]
  | app f a _ _ => simp [Pred.eval]
...
theorem claimsHold_shift {v : Tm} {claims : List Pred} (c : Nat) :
    Tm.claimsHold claims (v.shift c) = Tm.claimsHold claims v := by
  unfold Tm.claimsHold
  congr 1
  funext p
  rw [eval_shift]

theorem claimsHold_subst {v : Tm} (hv : v.IsVal) {claims : List Pred}
    (k : Nat) (u : Tm) :
    Tm.claimsHold claims (Tm.subst k u v) = Tm.claimsHold claims v := by
  unfold Tm.claimsHold
  congr 1
  funext p
  rw [eval_subst hv]
...
theorem HasTy.weaken {Γ : List Ty} {e : Tm} {W : Ty} (h : HasTy Γ e W) :
    ∀ (Γ₁ Γ₂ : List Ty) (U : Ty), Γ = Γ₁ ++ Γ₂ →
      HasTy (Γ₁ ++ U :: Γ₂) (e.shift Γ₁.length) W := by
  induction h with
  | lit l =>
      intro Γ₁ Γ₂ U hΓ
      rw [Tm.shift_lit]; exact .lit l
  | @var Γ n T hn =>
      intro Γ₁ Γ₂ U hΓ
      subst hΓ
      rw [Tm.shift_var]
  ...
  | refineV _ hval hch hne hcl hbase ih =>
      ...
      exact .refineV (ih Γ₁ Γ₂ U hΓ) (hval.shift _)
        (by rw [Tm.claimsHold_shift]; exact hch) hne hcl hbase
  | sub _ hsub hfrag ih =>
      intro Γ₁ Γ₂ U hΓ
...
No value restriction and no fragment hypothesis: the fragment premises ride
the rules untouched (types contain no terms in this fragment), and the
substituend only needs the type the cut assumed. -/

theorem HasTy.subst_preserves {Γ : List Ty} {e : Tm} {W : Ty}
    (h : HasTy Γ e W) :
    ∀ (Γ₁ Γ₂ : List Ty) (T : Ty) (v : Tm), Γ = Γ₁ ++ T :: Γ₂ →
      HasTy (Γ₁ ++ Γ₂) v T →
      HasTy (Γ₁ ++ Γ₂) (Tm.subst Γ₁.length v e) W := by
  induction h with
  | lit l =>
      intro Γ₁ Γ₂ T v hΓ hv
      rw [Tm.subst_lit]; exact .lit l
  | @var Γ n T' hn =>
      intro Γ₁ Γ₂ T v hΓ hv
      subst hΓ
      rw [Tm.subst_var]
  ...
  | refineV _ hval hch hne hcl hbase ih =>
      ...
      exact .refineV (ih Γ₁ Γ₂ T v hΓ hv) (hval.subst _ _)
        (by rw [Tm.claimsHold_subst hval]; exact hch) hne hcl hbase
  | sub _ hsub hfrag ih =>
      intro Γ₁ Γ₂ T v hΓ hv
...
one binder. -/
theorem HasTy.subst_one {Γ : List Ty} {body v : Tm} {T U : Ty}
    (hbody : HasTy (T :: Γ) body U) (hv : HasTy Γ v T) :
    HasTy Γ (Tm.subst 0 v body) U := by
  simpa using hbody.subst_preserves [] Γ T v rfl (by simpa using hv)

/-! ## Canonical forms (modulo refinement peeling)

Each is an induction over the typing derivation: the head rule supplies the
shape, `refineV` peels a layer, and `sub` crosses one relation link via
`Sub.to_peel` plus the head inversion. `IsVal` dismisses every non-value
rule. -/

theorem HasTy.canonical_fn {Γ : List Ty} {v : Tm} {W : Ty}
    (h : HasTy Γ v W) :
...
theorem HasTy.canonical_tuple {Γ : List Ty} {v : Tm} {W : Ty}
  ...
  | @sub Γ' e S W' _ hsub hfrag ih =>
      ...
      have hWp : (Ty.tuple Ts).TermFrag := hW ▸ hfrag.peel_fst
...
theorem HasTy.canonical_variant {Γ : List Ty} {v : Tm} {W : Ty}
  ...
  | @sub Γ' e S W' _ hsub hfrag ih =>
      ...
      have hWp : (Ty.variant tags).TermFrag := hW ▸ hfrag.peel_fst
      have hTfrag : T.TermFrag := by
...
`Y`. The inversion's slack — reflexive without `Sub.refl`'s `WF` side
condition, and composable across a subsumption chain without transitivity
of `Sub`. -/
def TyImp (X Y : Ty) : Prop := ∀ (Δ : List Ty) (a : Tm), HasTy Δ a X → HasTy Δ a Y

theorem HasTy.lam_inv {Γ : List Ty} {L : Tm} {W : Ty} (h : HasTy Γ L W) :
    ∀ {d body}, L = .lam d body → (∀ X ∈ Γ, X.TermFrag) →
      ∀ n k dom cod, W.peel.1 = .fn n k dom cod →
        ∃ c₀, HasTy (d :: Γ) body c₀ ∧ TyImp dom d ∧ TyImp c₀ cod := by
  induction h with
...
theorem progress_split {es : List Tm}
    (h : ∀ e ∈ es, e.IsVal ∨ (∃ e', Tm.Step e e') ∨ Tm.Blocked e) :
    (∀ e ∈ es, e.IsVal) ∨
      ∃ pre e post, es = pre ++ e :: post ∧ (∀ x ∈ pre, x.IsVal) ∧
        ((∃ e', Tm.Step e e') ∨ Tm.Blocked e) := by
  induction es with
...
theorem progress_aux {Γ : List Ty} {e : Tm} {T : Ty} (h : HasTy Γ e T) :
    Γ = [] → e.IsVal ∨ (∃ e', Tm.Step e e') ∨ Tm.Blocked e := by
  induction h with
  ...
  | @tuple Γ' es Ts hlen _ ih =>
      intro hΓ; subst hΓ
      have hprog : ∀ e ∈ es, e.IsVal ∨ (∃ e', Tm.Step e e') ∨ Tm.Blocked e := by
        intro e he
  ...
  | @cast Γ' e0 T0 claims _ _ _ _ ih =>
      intro hΓ; subst hΓ
      rcases ih rfl with hve | hste
      · cases hch : Tm.claimsHold claims e0 with
...
theorem progress {e : Tm} {T : Ty} (h : HasTy [] e T) :
    e.IsVal ∨ (∃ e', Tm.Step e e') ∨ Tm.Blocked e :=
  progress_aux h rfl

/-! ## Preservation -/

theorem preservation_aux {Γ : List Ty} {e : Tm} {T : Ty} (h : HasTy Γ e T) :
    ∀ e', Tm.Step e e' → (∀ X ∈ Γ, X.TermFrag) → HasTy Γ e' T := by
  induction h with
...
theorem preservation {Γ : List Ty} {e e' : Tm} {T : Ty}
    (hΓ : ∀ X ∈ Γ, X.TermFrag) (h : HasTy Γ e T) (hs : Tm.Step e e') :
    HasTy Γ e' T :=
...
inductive Tm.Steps : Tm → Tm → Prop
  | refl (e : Tm) : Tm.Steps e e
  | head {e e' e''} : Tm.Step e e' → Tm.Steps e' e'' → Tm.Steps e e''

theorem preservation_star {Γ : List Ty} {e e' : Tm}
    (hs : Tm.Steps e e') (hΓ : ∀ X ∈ Γ, X.TermFrag) :
    ∀ {T : Ty}, HasTy Γ e T → HasTy Γ e' T := by
  induction hs with
  | refl _ => exact fun h => h
  | head hstep _ ih => exact fun h => ih (preservation hΓ h hstep)

/-- A value's ascribed claims all evaluate true on it: `refineV` supplies
them checked, `sub` only ever shrinks them (`Sub.claims_mono`), and no
other rule types a value at a refined type. -/
theorem HasTy.value_claims {Γ : List Ty} {v : Tm} {W : Ty}
    (h : HasTy Γ v W) :
  ...
  | sub _ hsub _ ih =>
      intro hv p hp
      exact ih hv p (hsub.claims_mono p hp)

/-- **Refinement soundness**: a closed term of refined type that evaluates
to a value satisfies every claim — the cast is the only door, and the
`castV` step checks exactly this set. -/
theorem refinement_soundness {e v : Tm} {T : Ty} {claims : List Pred}
    (h : HasTy [] e (.refined T claims)) (hs : Tm.Steps e v)
    (hv : v.IsVal) : Tm.claimsHold claims v = true := by
  have hty := preservation_star hs (by simp) h
  have hcl := hty.value_claims hv
  unfold Tm.claimsHold
  rw [List.all_eq_true]
  intro p hp
...
well-typed variant value really has the type the scrutinee's tag table
assigns — so `caseV`'s substitution feeds each arm's binder a value of the
bound it was typed under. The Rust's `case _:` payload-binder defect lives
in *wildcard* arms, which this model does not yet have; refuting the naive
statement there needs that extension (recorded in `Term.lean`'s module
docs as a later increment). -/
theorem case_binder_sound {Γ : List Ty} {v : Tm} {tag : FieldKey}
    {tags : List (FieldKey × Ty)}
    (h : HasTy Γ (.variant tag v) (.variant tags))
    (hv : (Tm.variant tag v).IsVal) :

...
- `App` and `Below` — type-function applications reduce during inference and
  ...
  sense as `Infer`.
- `FunKind::Var` — kind unknowns resolve at coalesce; a ground kind is one of
...
/-- Mirror of the ground part of `ccl::ty::FunKind` (`Var` excluded): two
...
reserved refinement binder (`REFINEMENT_BINDER`, `__elem`); `piBound`
references an enclosing `Fun` Pi binder as a de Bruijn index — the number of
`fn` codomains crossed between the reference and the function that binds it,
mirroring `Name::PiBound` (`src/ccl/design/type-inference.md`, "A binder
reference is stored in one of two forms"). A ground type is closed:
construction converts a binder reference to its index (`close_pi_binder`), so
two α-variant function types are structurally identical and the relation needs no rename
environments. `var` remains for the *free* references a refinement may keep — a
`let`-bound name a user-written refinement mentions, or a source — which are
globally unique and compare structurally. The Rust→JSON emitter must map
...
`refined` carries a **refinement set**, exactly like
`Type::Refinement(base, RefinementSet)`: a base narrowed by the conjunction of
its refinements, with `Ty.WF` requiring the same two invariants `Type::refined`
establishes — the refinements are non-empty, and the base is not itself refined, so
layers never nest.

The refinements are *represented* as a `List Pred` and `Ty.beq` compares them
...
is containment, never list equality, so `Sub` cannot observe refinement order —
stated and proved as `sub_refinements_perm` rather than left as a reading of the
rules.
...
inductive Ty where
  ...
  | refined (base : Ty) (refinements : List Pred)
deriving Repr
...
def Ty.mkRefined (base : Ty) (refinements : List Pred) : Ty :=

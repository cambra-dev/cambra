...
declarative relation, giving `Decidable (Sub lhs rhs)` — the ground
...
theorem subSeq_iff :
  ...
  | [], _ :: _ => by simp [subSeq]
  | t0 :: a, t1 :: b => by
      simp only [subSeq, Bool.and_eq_true, subSeq_iff a b,
        List.length_cons]
      constructor
...
theorem subFields_iff (a : List (String × Ty)) :
    ∀ b, subFields a b = true ↔
      ((∀ n t1, (n, t1) ∈ b → (lookupBy a n).isSome) ∧
        ∀ n t0 t1, (n, t1) ∈ b → lookupBy a n = some t0 →
          subCheck t0 t1 = true)
  | [] => by simp [subFields]
  | (n1, t1) :: rest => by
      simp only [subFields, Bool.and_eq_true, subFields_iff a rest]
...
theorem subTags_iff (b : List (FieldKey × Ty)) :
    ∀ a, subTags b a = true ↔
  ...
  | [] => by simp [subTags]
  | (k0, t0) :: rest => by
      simp only [subTags, Bool.and_eq_true, subTags_iff b rest]
...
theorem sub_of_subCheck :
    ∀ (lhs rhs : Ty),
      subCheck lhs rhs = true → Sub lhs rhs
  | lhs, rhs, h => by
    ...
    · rename_i n0 k0 d0 c0 n1 k1 d1 c1
      ...
      have hcodS := sub_of_subCheck c0 c1 hcod
      split at hdom
      · -- data-data: invariant domains.
        ...
        exact Sub.fnData
          (sub_of_subCheck d1 d0 hdom.1)
          (sub_of_subCheck d0 d1 hdom.2) hcodS
      · rename_i hdd
        have hnd : ¬(k0 = .data ∧ k1 = .data) := by
          rintro ⟨rfl, rfl⟩
          exact hdd (by simp)
        exact Sub.fnCompute (kindOkB_iff.mp hok) hnd
          (sub_of_subCheck d1 d0 hdom) hcodS
    -- Tuple.
    · rename_i a b
      obtain ⟨hlen, hpt⟩ := (subSeq_iff a b).mp h
      ...
      exact sub_of_subCheck t0 t1 (hpt i t0 t1 h0 h1)
    -- Record.
    · rename_i a b
      obtain ⟨hsome, hsub⟩ := (subFields_iff a b).mp h
      ...
      have hs1 : sizeOf t1 < sizeOf b := by
        ...
        omega
      exact sub_of_subCheck t0 t1 (hsub n t0 t1 hm hlk)
    -- Variant.
    · rename_i a b
      obtain ⟨hsome, hsub⟩ := (subTags_iff b a).mp h
      ...
      have hs0 : sizeOf t0 < sizeOf a := by
        ...
        rw [Prod.mk.sizeOf_spec] at h'
        omega
      exact sub_of_subCheck t0 t1 (hsub k t0 t1 hm hlk)
    ...
    · split at h
      ...
      · rename_i hne
        ...
        refine Sub.refined rfl rfl ?_ ?_
          (sub_of_subCheck lhs.peel.1 rhs.peel.1 hbase)
...
theorem subCheck_of_sub {lhs rhs : Ty}
    (h : Sub lhs rhs) : subCheck lhs rhs = true := by
  induction h with
  ...
  | tuple hlen hpt ihpt =>
      simp only [subCheck]
      exact (subSeq_iff _ _).mpr ⟨hlen, ihpt⟩
  | record hsome hsub ihsub =>
      simp only [subCheck]
      exact (subFields_iff _ _).mpr ⟨hsome, ihsub⟩
  | variant hsome hsub ihsub =>
      simp only [subCheck]
      exact (subTags_iff _ _).mpr ⟨hsome, ihsub⟩
...
theorem subCheck_iff_sub (lhs rhs : Ty) :
    subCheck lhs rhs = true ↔ Sub lhs rhs :=
  ⟨sub_of_subCheck lhs rhs, subCheck_of_sub⟩

/-- **The ground subtype relation is decidable.** -/
instance (lhs rhs : Ty) : Decidable (Sub lhs rhs) :=
  decidable_of_iff _ (subCheck_iff_sub lhs rhs)

...
// through the store's offset bridge and per-stage indices:
//   - hover tooltips: hovered pos -> tightest IR node -> label/type(s)/provenance
//     (anchored on the post-inference stage, which carries resolved types)
...
// reseeds every stage's tightest node from that offset, so all panes light up.
...
import type { RewriteTag } from "./types";
...
export interface HoverPayload {
  label: string;
  types: string[];
  // The node's rewrite tag (schema 3), or null for a directly-lowered source
  // node.
...
export class SourceView {
  ...
  // The post-inference stage's indices, used for hover types + goto-def. May be
  // undefined on a degraded snapshot (no stages); interactions then degrade.
  private readonly anchor: Indices | undefined;

  constructor(parent: HTMLElement, store: Store) {
    this.store = store;
    const { offsets, snapshot } = store;
    this.anchor = store.sourceAnchorStageId
      ? store.indicesFor(store.sourceAnchorStageId)
      : undefined;
    ...
    const hover = hoverTooltip((_view, pos): Tooltip | null => {
      ...
      const def = anchor.definitionAt(byte);
      const start = offsets.byteToChar(node?.span ? node.span.start : byte);
      const end = node?.span ? offsets.byteToChar(node.span.end) : pos;
      ...
      return {
        ...
        create() {
          ...
          if (payload.rewritten) {
            ...
            // label (schema 3 replaced the old flat provenance string).
    ...
    //     stage's tightest node), without suppressing the editor's own cursor.
  ...
  private renderSelection(resolved: Resolved): void {
    ...
    // The primary span(s): the source spans of the per-stage primary anchors.
    const primaryKeys = new Set<string>();
    for (const [stageId, nodeId] of resolved.primaryByStage) {
      if (nodeId === null) continue;
      const span = this.store.indicesFor(stageId)?.nodeById.get(nodeId)?.span;

...
// NOTE: the exact type strings these tests pin (`"_"`, `"((Int, Int) ⇒ Int)"`,
// `"(Int, Int)"`, `"(Int ⇒ (Int, Int))"`, ...) are CCL's *canonical* type
// rendering — `Display for Type` in `src/ccl/mod.rs`. The coupling is
// intentional: if that formatting changes, these expectations must be updated in
// lockstep (the frontend reads the type as an opaque display string from
// `node.type`, so the only contract is the rendered text).
describe("T1: B5 hole -> downstream-type stitch (resolvedTypesFor)", () => {
  it("arithmetic: an identity hole resolves to its single downstream type", () => {
    ...
    expect(store.resolvedTypesFor("pre-inference", lit.nodeId)).toEqual(["Int"]);
  ...
  it("polymorphic: a monomorphized hole resolves to the downstream type *set*", () => {
    ...
    expect(sorted(store.resolvedTypesFor("pre-inference", lambda.nodeId))).toEqual(
      sorted(["(Int ⇒ (Int, Int))", "(Bool ⇒ (Bool, Bool))"]),
    );
  });
...
describe("T2: cross-pane resolution (setSelection -> getResolved)", () => {
  ...
  it("an inline-fan-out node click reaches the post-desugar copies (window 2)", () => {
    ...
    // duplicates its body, and the post-inference -> post-desugar stageLinks
    // window carries the (copy, origin) edges (RT-3b at the store level).
    const window2 = udfFanout.stageLinks.find(
      (l) => l.from === "post-inference" && l.to === "post-desugar",
    )!;
    expect(window2.edges.length).toBeGreaterThan(0);

    const store = new Store(udfFanout);
    const [origin, copy] = window2.edges[0];

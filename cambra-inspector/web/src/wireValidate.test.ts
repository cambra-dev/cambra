// Tests for the runtime wire-shape validator (schema 2). Every real fixture
...
describe("validateSnapshot: real fixtures", () => {
  ...
  it("the failed (degraded) fixture validates with empty stages", () => {
    ...
    expect(snap.stageLinks).toEqual([]);
...
describe("validateSnapshot: rejects malformed payloads with a path", () => {
  ...
  // windows the schema-2 contract requires.
  ...
  it("throws naming meta.schema when it is missing or not the supported version", () => {
    ...
    (stale.meta as Record<string, unknown>).schema = 1;
    expect(() => validateSnapshot(stale)).toThrow(/meta\.schema.*exactly 2/);
  });
  ...
  it("throws when a successful payload's stages deviate from the pinned list", () => {
    ...
    (wrongWindows.stageLinks as unknown[]).pop();
    expect(() => validateSnapshot(wrongWindows)).toThrow(/stageLinks.*windows/);
  });
  ...
  it("throws on a provenance label outside the pinned vocabulary", () => {
    const bad = minimalSuccess();
    ((bad.stages as { ir: Record<string, unknown> }[])[0].ir as Record<string, unknown>).provenance =
      "Derived(Planning)"; // a pass not yet on the wire
    expect(() => validateSnapshot(bad)).toThrow(/provenance/);
  });

  it("throws when a stageLinks edge is not a [number, number] pair", () => {
    const bad = minimalSuccess();
    (bad.stageLinks as { edges: unknown[] }[])[0].edges = [[1]];
    expect(() => validateSnapshot(bad)).toThrow(/stageLinks\[0\]\.edges\[0\].*pair/);
  });

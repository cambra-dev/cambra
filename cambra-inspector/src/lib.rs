...
/// (`web/src/wireValidate.ts`). Asserting the schema-2 contract in one place
...
pub(crate) mod test_support {
    ...
    /// stage list a schema-2 successful payload ships.
    ...
    /// The adjacent stage windows, in order — the exact `stageLinks` from/to
    /// pairs a schema-2 successful payload ships.
    const STAGE_WINDOWS: [(&str, &str); 2] = [
        ("pre-inference", "post-inference"),
        ("post-inference", "post-desugar"),
    ];
    /// The observed `Provenance.via` vocabulary on the wire (the `Pass` names
    /// that appear inside `Derived(_)`/`Synthetic(_)` provenance labels). Pinned
    /// so a new `via` reaching the wire is a deliberate, reviewed event. `Source`
    /// (no via) is handled separately in [`assert_provenance_vocabulary`].
    const ALLOWED_VIA: &[&str] = &["Mono", "Inline", "Desugar"];

    /// Assert `v` is a structurally-valid **successful** `/api/snapshot` payload
    /// (schema 2). Pins the full stage contract: schema 2, the retired top-level
    /// `ir`/`spanIndex` absent, the three pipeline stages in order with their
    /// kinds, each stage's `spanIndex` non-empty, the two windowed `stageLinks`
    /// with matching from/to ids, and every node's provenance `via` in the
    /// observed vocabulary. Panics naming the offending path otherwise. Does not
    /// assert program-specific content — that is each caller's job.
    pub(crate) fn assert_snapshot_shape(v: &Value) {
        ...
        // merely null — a schema-2 client reads the stages, never the top level.
        assert!(v.get("ir").is_none(), "schema 2 has no top-level ir");
        assert!(
            v.get("spanIndex").is_none(),
            "schema 2 has no top-level spanIndex"
        );
        ...
        for (i, st) in stages.iter().enumerate() {
            ...
            // Every provenance label a stage's tree carries uses the pinned via
            // vocabulary.
            assert_provenance_vocabulary(&st["ir"], &at);
        }
        ...
        let windows: Vec<(&str, &str)> = links
            ...
            .map(|l| {
                (
                    l["from"].as_str().expect("stageLink.from is a string"),
                    l["to"].as_str().expect("stageLink.to is a string"),
                )
            })
            .collect();
        assert_eq!(
            ...
            "stageLinks are the two adjacent stage windows in pipeline order"
        );
        for (i, link) in links.iter().enumerate() {
            let at = format!("stageLinks[{i}]");
            ...
            for (j, e) in edges.iter().enumerate() {
                ...
                assert!(
                    ...
                    "{at}.edges[{j}] entries are numbers"
                );
            }
        }
    }

    /// Assert `v` is a structurally-valid **degraded** payload (schema 2): the
    /// retired top-level `ir`/`spanIndex` absent, empty `stages`/`stageLinks`,
    /// `snapshotKind: "failed"`.
    pub(crate) fn assert_degraded_snapshot_shape(v: &Value) {
        ...
        assert!(
            v["stageLinks"].as_array().expect("array").is_empty(),
            "degraded stageLinks is empty"
        );
    ...
    /// Recursively assert every node's `provenance` label (when present) is
    /// `"Source"` or `Derived(<via>)`/`Synthetic(<via>)` with `<via>` in
    /// [`ALLOWED_VIA`] — the wire's pinned `Provenance.via` vocabulary.
    fn assert_provenance_vocabulary(node: &Value, at: &str) {
        if let Some(p) = node["provenance"].as_str() {
            let via_ok = |wrapped: &str| {
                wrapped
                    .strip_suffix(')')
                    .is_some_and(|via| ALLOWED_VIA.contains(&via))
            };
            let ok = p == "Source"
                || p.strip_prefix("Derived(").is_some_and(via_ok)
                || p.strip_prefix("Synthetic(").is_some_and(via_ok);
            assert!(
                ok,
                "{at}: provenance {p:?} is outside the pinned vocabulary \
                 (Source, or Derived/Synthetic over {ALLOWED_VIA:?})"
            );
        }
        for c in node["children"].as_array().into_iter().flatten() {
            assert_provenance_vocabulary(&c["node"], at);
        }
    }
...
";
    ...
    /// The structural wire-shape contract (schema 2) — the Rust twin of the
    ...
    /// post-inference stage tree (schema 2 retired the top-level `ir`).
    #[test]
    ...
    /// For a let-polymorphic program (`dup` used at two types), `stageLinks[0].edges`
    /// is non-empty — the monomorphization fan-out produces non-identity cross-stage
    /// edges.
    #[test]
    fn stage_links_edges_non_empty_for_polymorphic_program() {
        let code = "\
dup = lambda x: (x, x)
a = dup(1)
b = dup(2 == 2)
...
";
        ...
        let stage_links = v["stageLinks"].as_array().expect("stageLinks is an array");
        assert!(!stage_links.is_empty(), "stageLinks is non-empty");

        let link = &stage_links[0];
        ...
        assert!(
            !edges.is_empty(),
            "a let-polymorphic def used at two types must yield non-empty stageLinks edges"
        );
    }

    /// RT-3b — for a scalar UDF called at two sites (inline fan-out), the wire
    /// payload's `post-inference → post-desugar` stageLinks entry has non-empty

...
// TODO(type-feed): this filter-feed-inside-defer case fails on a deep
// interaction between the desugar-after-inference filter-feed refined-source
// channel and the immutable-predicate refinement representation — `retype`'s
// generic reconstruction can't re-derive the refined source's predicate term
// (its embedded compose keeps a `Hole` slot the strict post-desugar typecheck
// rejects). Reconcile the filter-feed refined-source emission with the
// refinement repr to restore it (it was also ignored under the legacy
// desugar-before-inference order, as a simple-sub predicate gap).
#[ignore = "filter-feed refined-source channel: retype cannot re-derive the predicate term"]
#[case::feed_with_if(
r#"x = defer()
for i in [0,1,2,3]:
  if i // 2 == 0:

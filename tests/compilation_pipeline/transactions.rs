...
/// A load binds a plain value, so `Mut(…)` on its annotation is a contradiction and is
/// named as one. Lowering the annotation without the check reports `Mut` as a type
/// application nothing defines, about a type constructor the language has.
#[test]
fn load_from_with_a_mut_annotation_rejected() {
    check_compile_error(
        indoc! {r#"
            @LoadFrom(n)
            held: Mut(Int, Txn)
            held
        "#},
        "declares a mutable, but a load binds a plain value",
    );
}
...
/// (`m := box(map(…))`) is not a keyed write, and does not compile, as the same write in the
/// loop body does not. Transactional pass-by-reference is refused at lowering: a
/// `with begin():` block takes writes, bindings, guards and feeds, not a call.
#[test]

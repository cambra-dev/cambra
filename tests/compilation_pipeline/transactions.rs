...
///
/// `Typing::apply` peels the handle for `Typing::as_function` and then passes
/// `constrain_argument` the function it was given, so before `Type::feed_read_view` was
/// taken there too the two disagreed about whether a channel is a function and the
/// argument check asserted on a caller that had passed one.
///
/// An aggregate applied to the channel directly (`sum(out)`, as
/// [`await_final_and_as_of_read_the_same_mut_var`] reads it) consumes the collection
/// without applying it, which is why the suite held no program that reached this. The
/// transaction is beside the point — a bare feeding loop reaches it too.
#[rstest]
#[timeout(Duration::from_secs(10))]
...
/// A **filtered** comprehension over a channel, which the argument check now admits and
/// `channelize` then stops. The filter refines the comprehension's source domain and that
/// domain is the channel's, so a `ChanDom` sits inside the refinement's predicate —
/// where channelize's erasure never reaches, because `Type::walk_children_mut` visits a
...
#[test]
#[should_panic(expected = "feed handle type survived channelize")]
fn a_filtered_comprehension_over_a_feed_channel_is_not_reachable() {
    check_scalar(
        indoc! {r"
            out = defer()
            for v in [1, 2]:

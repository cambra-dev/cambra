...
// The committed value is read by a trailing standalone read-only transaction
// (`with begin(): out << pool`) and fed to `out` — an as-of read latched to the
// singleton trigger at the read transaction's commit position, so `out` is the
// one-element stream at position 0. Under the batch scheduler the read commits
// after the draws drain, so it observes the drained value.
#[rstest]
...
/// its store snapshot. The end-to-end value is checked in
/// `live_reply_combines_request_and_store`.
#[test]
fn live_read_combining_request_and_store_compiles() {
    let code = "set_reqs, set_resps = http_serve(\"0\", \"POST\", \"/set\")\n\
    ...
    compile_program(&mut ctx, code, consumer).expect(
        "a live reply combining the request with a store read should compile to a zip read",
    );
}

/// End-to-end live check of the request-plus-store reply (3b): a batch loop
...
/// `Mut[V, Txn]`. A fully-inferred bare `:=` store is an *induction* store, so
...
/// Stage-1 interop, store side: the register accumulates transactionally across
...
/// worked-example form. `transact_phase` partitions the block by store domain —
...
/// Stage-2 interop, store side: with the induction write inside the block, the
...
// A heterogeneous multi-key register store: a `Mut[str, Txn]` and a
// `Mut[int, Txn]` committed together in one block. Regression for the
// store-wide value extent (`build_commit_store`), which is the union of the

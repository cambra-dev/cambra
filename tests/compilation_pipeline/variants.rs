...
///
/// The variant elimination stack covers that law at the `ExtractLast` boundary (its
...
///   typecheck has a stamped type to resolve against.
...
// The nullary arm both ways: bare, and with its `Unit` payload spelled out.
#[case("x: {`a} = `a\nx", union("a", Value::Unit))]
#[case("x: {`a{Unit}} = `a\nx", union("a", Value::Unit))]
// `None` is `Unit`'s surface spelling, so it names the same nullary arm.
#[case("x: {`a{None}} = `a\nx", union("a", Value::Unit))]

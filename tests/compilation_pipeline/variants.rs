...
/// consistency wall.
///
/// Pinned rather than fixed: the cause is **order-dependent specialization**, on
/// `main` and independent of this surface. The first clone of a generalized
/// definition gets a domain widened by the join of *every* use's demand (later
/// clones get their own — reverse the uses and the widening follows whichever comes
/// first). A too-wide domain is normally invisible under contravariance; a two-arm
/// `Case` is what makes it observable, because the dead arm's binder takes its type
/// from that widened payload, enters the arms' join, and so disagrees with a
/// codomain that *is* per-instance. Hence `expected Int, found 1`.
///
/// Four conditions are each necessary, which is what places the defect there rather
/// than in the surface:
///
/// - **two or more arms** — with one there is no join for the widened payload to
///   pollute ([`test_single_arm_match_udf_takes_two_call_sites`]); the one-arm case
///   is widened identically and simply escapes notice;
/// - **two or more call sites** — and not because they *differ*: two calls with the
///   same literal fail too, at `p + q`;
/// - **a singleton-typed payload** — a computed one (`` `a(1 + 0) ``, an `Int`
///   because computing drops the claim) compiles, since the join is then a no-op;
/// - **both results consumed** — dropping one compiles.
///
/// Annotating the parameter does not help, and a non-variant `if`/`else` UDF called
/// twice is unaffected.
///

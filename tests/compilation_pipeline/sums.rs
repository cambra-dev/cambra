...
/// The same missing iteration source, reached by a `for` loop rather than a
/// comprehension, and reported by a different check.
///
...
/// op-conversion is reached. Both doors want the source that takes the collection as an
/// input and emits the domain the value holds
/// ([`iterating_a_witness_domained_collection_is_unimplemented`]); only one of them says
/// so, which is worth fixing when that source lands.
#[test]

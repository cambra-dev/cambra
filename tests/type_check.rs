...
fn a_single_type_fault_prints_no_demand() {
    ...
    assert!(
        missing.contains("missing field") && missing.contains("found"),
        "expected a missing-field fault naming the record, got: {missing}"
    );
    assert!(
...
mod annotation_kinds {
    ...
    fn a_bound_on_a_register_bounds_its_value_type() {
        // Compared by register *value* type: each run mints its own domain variable,
        // so the whole types differ only by variable id.
        let register_value = |code: &str| {
            infer_program(code)
                .as_register()
                .cloned()
                .expect("a trailing register read is typed by its register")
        };

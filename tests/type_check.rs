...
// ---------------------------------------------------------------------------
// Defer / Feed / Define typing rules (pre-desugar trees)
// ---------------------------------------------------------------------------
//
// These tests run inference on lowered-but-NOT-desugared trees, exercising
// the `Defer`/`Feed`/`Define` typing rules directly: a defer binding types
// as `Feed(ρ)` (its post-desugar result type), feeds contribute
// `Fun(?, elem)` channel shapes into ρ, defines set ρ outright, and reads
// discharge transparently through the feed handle. Channel *domains* are
// unknowable before desugaring, so fed payloads carry `Type::Infer`
// domains here — desugar later replaces them with concrete channel domains.

/// Destructure `ty` as `Feed(payload)`, panicking otherwise.
fn feed_payload(ty: &Type) -> &Type {
    match ty {
        Type::Feed(p) => p,
...
/// Assert `ty` is a channel shape `Fun(_, elem)` and return the codomain.
fn channel_elem(ty: &Type) -> &Type {
    match ty {
        Type::Fun { codomain, .. } => codomain,
        _ => panic!("expected a channel (function) payload, got {ty}"),
    }
}

#[test]
fn test_defined_defer_is_feed_of_value() {
    let ty = infer_program("x = defer()\nx <<= 1\nx");
    assert_eq!(*feed_payload(&ty), int());
}

#[test]
fn test_fed_defer_is_feed_of_channel() {
    let ty = infer_program("x = defer()\n[x << i for i in [1,2,3]]\nx");
    assert_eq!(*channel_elem(feed_payload(&ty)), int());
}
...
#[test]
fn test_defined_defer_reads_transparently() {
    // The read `x + 1` consumes the feed handle as its payload (Int).
    assert_eq!(infer_program("x = defer()\nx <<= 5\nx + 1"), int());
}

#[test]
fn test_fed_defer_reads_through_aggregate() {
    // `sum` consumes the feed handle as its channel payload `(α → γ)`.
    assert_eq!(infer_program("x = defer()\nx << 1\nx << 2\nsum(x)"), int());
}

#[test]
fn test_defer_chain_nests_feeds() {
    // `x <<= y` sets x's payload to y's whole feed handle; reads discharge
    // through both layers (rule 2 recursively), and desugar later binds x
    // to y's channel directly.
    let ty = infer_program("x = defer()\ny = defer()\nx <<= y\ny <<= [0, 1]\nx");
    let inner = feed_payload(&ty);
    assert_eq!(*channel_elem(feed_payload(inner)), int());
}
...
(a, b)"#,
    ...
    assert_eq!(*channel_elem(feed_payload(&elems[0])), int());
    assert_eq!(*channel_elem(feed_payload(&elems[1])), string());
}
...
/// `src/ccl/design-mut-txn-feed.md` lands it later).
mod letrec_typing {

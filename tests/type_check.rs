...
// as a feed history `feed(δ ⇒ value)` (its post-desugar result type), feeds
// contribute `Fun(?, elem)` channel shapes into it, defines set the whole
// stream outright, and reads discharge transparently through the handle as
// that stream. Channel *domains* are unknowable before desugaring, so a fed
// history's `domain` carries `Type::Infer` here — desugar later replaces it
// with the concrete channel domain.

...
/// (`src/ccl/design/collections.md`, "The kind is declared, not read off the shape").
...
// `Option(group)`, pinned by `checked_lookup_is_application_modulo_membership` — and it is
// not `match`, which has landed, nor a runtime presence test, which absence-as-an-empty-tile
// does not need (`src/ccl/design/collections.md`, "How `𝑐[𝑘]?` compiles [Planned]"). What is missing is
// that compilation: the lookup's argument encoding has no runtime reading. When it lands this
// becomes `match groups[1]? { some(g) -> sum(g), none -> 0 }`, a different assertion than the
// one here, so it is restated rather than un-ignored.
#[ignore = "superseded: `groups(1)` is rejected by design; restate as a checked lookup once \
            `c[k]?` compiles"]
#[test]
...
/// binder's scope.
///
/// And the **key's base type is still checked.** Only the membership refinement is dropped, so
...
fn checked_lookup_is_application_modulo_membership() {
    ...
    assert!(
        !infer_program_err(&format!("{g}g[\"nope\"]?")).is_empty(),

...
fn probe_dead() {
    ...
    let pane = snap["panes"]
        ...
        .unwrap()
        .iter()
        .find(|p| p["id"] == "post-conversion")
        .unwrap();
    let mut kind_of: HashMap<u64, String> = HashMap::new();
    for n in pane["nodes"].as_array().unwrap() {
        kind_of.insert(
            n["nodeId"].as_u64().unwrap(),
            n["label"].as_str().unwrap_or("?").to_string(),
        );
    }
    ...
    let harvest = |h: &Host, seen: &mut HashSet<u64>| {
        ...
        for n in f["nodes"].as_array().unwrap() {
            seen.insert(n["nodeId"].as_u64().unwrap());
        }
        for s in f["sources"].as_array().unwrap() {
            if let Some(id) = s["nodeId"].as_u64() {
                seen.insert(id);
            }
        }
    };
    ...
    push(
        &mut host,
        "cart_changes",
        ticker_row("qty", "BTC-USD", 2),
        &mut seen,
    );
    push(
        &mut host,
        "cart_changes",
        ticker_row("qty", "ETH-USD", 5),
        &mut seen,
    );
    push(
        &mut host,
        "cart_changes",
        ticker_row("qty", "SOL-USD", 7),
        &mut seen,
    );
    for (i, t) in [
        "BTC-USD", "ETH-USD", "SOL-USD", "DOGE-USD", "XRP-USD", "BTC-USD",
    ]
    .iter()
    .enumerate()
    {
        push(
            &mut host,
            "price_updates",
            ticker_row("price", t, (80_000 + i as i64) * SCALE),
            &mut seen,
        );
    ...
    for (id, kind) in &kind_of {
        if !seen.contains(id) {
            *never.entry(kind.clone()).or_default() += 1;
            never_ids.push(*id);
        }
    }
    ...
    println!(
        "EVER IN A FRAME: {}",
        kind_of.keys().filter(|k| seen.contains(k)).count()
    );
